...
class Bundle(object):
  ...
  def Kind(platform, extension):
    # Need to check using stratswith because ios SDK freeze mechanism
    # adds the SDK version to the end of the platform string
    if platform.startswith('iphoneos') or platform.startswith('iphonesimulator'):
      return 'ios'
    if platform == 'macosx':

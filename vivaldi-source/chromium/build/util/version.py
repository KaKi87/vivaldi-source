...
def BuildParser():
  ...
  parser.add_argument('--vivaldi', action='store_true')
...
def GenerateOutputContents(options, values):
  ...
  elif options.vivaldi:
    # Generate a default set of Vivaldi version information.
    return """MAJOR=%(VIVALDI_MAJOR)s
MINOR=%(VIVALDI_MINOR)s
BUILD=%(VIVALDI_NIGHTLY)s
PATCH=%(VIVALDI_BUILD)s
VIVALDI_MAJOR=%(VIVALDI_MAJOR)s
VIVALDI_MINOR=%(VIVALDI_MINOR)s
VIVALDI_NIGHTLY=%(VIVALDI_NIGHTLY)s
VIVALDI_BUILD=%(VIVALDI_BUILD)s

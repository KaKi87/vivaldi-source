      ...
      [chrome, &browser, isolate, v8_context,
       set_accessor_on_browser](std::string_view accessor_name) {
        ...
        if (feature && feature->IsVivaldiFeature()) {
          v8::Local<v8::Object> vivaldi = GetOrCreateGlobalObjectProperty(v8_context, "vivaldi");
          v8::Maybe<bool> vivaldi_success = vivaldi->SetLazyDataProperty(
            v8_context, api_name, &BindingAccessor, api_name);
          return vivaldi_success.IsJust() && vivaldi_success.FromJust();
        } else { // Vivaldi

...
ExecuteCodeFunction::InitResult WebViewInternalExecuteCodeFunction::Init() {
  ...
  if (!guest_src_.is_valid()) {
    ...
    WebViewGuest* guest = WebViewGuest::FromInstanceID(
...
ExtensionFunction::ResponseAction WebViewInternalSetPermissionFunction::Run() {
  ...
  // Vivaldi: VB-114658: Handle new window requests that bypass permission
  // helper.
  if (auto result = GetGuest().VivaldiHandleNewWindowSetPermission(
          params->request_id,
          action == WebViewPermissionHelper::ALLOW,
          user_input)) {

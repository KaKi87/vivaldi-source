...
void EventRouter::DispatchEventToProcess(
  ...
  if (extension && event.filter_non_vivaldi != Event::NO_FILTERING) {
    bool is_vivaldi = ::vivaldi::IsVivaldiApp(extension->id());
    switch(event.filter_non_vivaldi) {
      case Event::VIVALDI_ONLY:
        if (!is_vivaldi)
          return;
        break;
      case Event::EXCLUDE_VIVALDI:
        if (is_vivaldi)
          return;
        break;
      default:

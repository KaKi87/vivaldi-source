...
MV2ExperimentStage CalculateCurrentExperimentStage() {
  ...
  // NOTE(andre@vivaldi.com): Stepping stone for our users to get away from MV2
  // extensions.
  // 1. When the user first starts with a unsupported extension the extension is
  // disabled.
  // 2. We show a dialog describing what happened.
  // 3. User can enable the extension in the extension manager.
  if (base::FeatureList::IsEnabled(

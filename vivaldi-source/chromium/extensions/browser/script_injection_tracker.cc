...
bool CanExtensionScriptsAffectFrame(content::RenderFrameHost& frame,
  ...
  // Avoid blocking this for extensions in Vivaldi. VB-101685.
  if (VivaldiTabCheck::IsVivaldiTab(
          content::WebContents::FromRenderFrameHost(&frame))) {
    guest = nullptr;
  }

  if (VivaldiTabCheck::IsVivaldiPanel(
          content::WebContents::FromRenderFrameHost(&frame))) {
    guest = nullptr;
  }
...
bool DoWebViewScripstMatch(const Extension& extension,
  ...
  // only done for non-guest content, which will of ourse fail in Vivaldi. Was
  // VB-85805.

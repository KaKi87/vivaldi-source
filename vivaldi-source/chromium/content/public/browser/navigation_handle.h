...
class CONTENT_EXPORT NavigationHandle : public base::SupportsUserData {
 ...
 public:
  ...
  // Whether the navigation was initiated by a trusted event.
  // Note WRT chromium updates: We want this one to trigger compile errors if
  // a new class implementing it shows up as a default implementation could lead
  // to false positives in the adblocker popup blocking code. Therefore, this
  // has to be pure virtual.
  virtual bool VivaldiIsFromTrustedEvent() = 0;
};

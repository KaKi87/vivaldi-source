...
void DownloadManagerImpl::StartDownloadItem(
  ...
  if (vivaldi::IsVivaldiRunning()) {
    ...
    if (web_contents && web_contents->GetDelegate()) {
      ...
      info->open_when_finished = downloadinfo->open_when_done;
      info->save_info->prompt_for_save_location = downloadinfo->ask_for_target;
    }
  }
#endif // !IS_ANDROID
...
void DownloadManagerImpl::InterceptNavigation(
  ...
  if (web_contents) {
    content::DownloadInformation* downloadinfo =
      web_contents->GetDelegate()->GetDownloadInformation();
    downloadinfo->mime_type = response_head->mime_type;
    downloadinfo->size =
        response_head->content_length ? response_head->content_length : -1;
    std::string default_file_name = "file";

    std::string content_disposition;
    if (response_head->headers) {
      content_disposition = response_head->headers->GetNormalizedHeader(
          "Content-Disposition").value_or(std::string());
    }
    ...
    std::u16string file_name = net::GetSuggestedFilename(
        ...
        std::string(),  // mime_type, do not pass this as it will lead to
                        // blocking behavior.
        default_file_name);
...
void DownloadManagerImpl::DownloadUrl(
  ...
  if (vivaldi::IsVivaldiRunning()) {
    // NOTE(andre@vivaldi.com): Added for VB-40929.
    WebContents* web_contents = WebContentsImpl::FromRenderFrameHostID(
    ...
    if (web_contents && web_contents->GetDelegate()) {
      ...
      if (!params->suggested_name().empty()) {
        ...
        std::string content_disposition;
        for (auto it = params->request_headers().begin();
             it != params->request_headers().end(); ++it) {
          if (it->first == "content_disposition") {
            content_disposition = it->second;
            break;
          }
        }
        std::u16string file_name = net::GetSuggestedFilename(
            params->url(), content_disposition,
            std::string(),  // referrer_charset
            std::string(),  // suggested_name
            std::string(),  // mime_type, do not pass this as it will lead to
                            // blocking behavior.
      ...
      downloadinfo->open_when_done = false;
    }
  }

...
void RenderWidgetHostImpl::ForwardKeyboardEventWithCommands(
  ...
  if (KeyPressListenersHandleEvent(key_event)) {
    ...
    if (key_event.GetType() == WebKeyboardEvent::Type::kRawKeyDown) {
      suppress_events_key_code_ = key_event.windows_key_code;
    }
    return;
  }

  if (suppress_events_key_code_) {
    ...
    if (key_event.GetType() == WebKeyboardEvent::Type::kKeyUp ||
      ...
      // Suppress only events for the same key that was handled. Events for
      // other keys (e.g. modifier keyups) should pass through.
      if (key_event.windows_key_code == suppress_events_key_code_) {
        return;
      }
      // Event is for a different key; continue processing normally.
    } else {
    CHECK(key_event.GetType() == WebKeyboardEvent::Type::kRawKeyDown ||
          key_event.GetType() == WebKeyboardEvent::Type::kKeyDown);
      suppress_events_key_code_ = 0;
    }
  }
  ...
  if (delegate_ && !key_event.skip_if_unhandled) {
    // We need to track the key code of handled RawKeyDown events so
    // follow-up KeyUp/Char events for that key are suppressed, but this
    // object may already be destroyed at that time. So set the key code
    // here, then clear it afterwards when necessary.
    if (key_event.GetType() == WebKeyboardEvent::Type::kRawKeyDown) {
      suppress_events_key_code_ = key_event.windows_key_code;
    }
  ...
  if (VivaldiEventHooks::HandleKeyboardEvent(this, key_event))
    return;
...
input::RenderWidgetHostViewInput* RenderWidgetHostImpl::GetPointerLockView() {
  // Vivaldi
  if (!delegate() || !delegate()->GetPointerLockWidget())
    return nullptr;
  // End Vivaldi
...
void RenderWidgetHostImpl::OnLocalSurfaceIdChanged(
  ...
  if (auto_resize_enabled_ && delegate_) {
    ...
    // NOTE(andre@vivaldi.com) : VB-87162
    // If the conversion exceed |max_size_for_auto_resize_|  webviews will not
    // resize. So we need to constrain.
    if (viewport_size_in_dip.width() > max_size_for_auto_resize_.width()) {
      viewport_size_in_dip.set_width(max_size_for_auto_resize_.width());
    }
    if (viewport_size_in_dip.height() > max_size_for_auto_resize_.height()) {
      viewport_size_in_dip.set_height(max_size_for_auto_resize_.height());
    }
...
void RenderWidgetHostImpl::SetAutoscrollSelectionActiveInMainFrame(
  ...
  // NOTE(andre@vivaldi.com) : This introduces an unwanted sideeffect where
  // mouseup is sent to the embedding document. Visible in VB-104719.
  RenderViewHostImpl* rvh = RenderViewHostImpl::From(this);
  content::WebContents* web_contents =
      content::WebContents::FromRenderViewHost(rvh);
  content::WebContents* outer_web_contents =
      web_contents ? web_contents->GetOuterWebContents() : nullptr;
  if (!vivaldi::IsVivaldiRunning() || (outer_web_contents &&
      outer_web_contents->GetURL().host() != vivaldi::kVivaldiAppId)) {

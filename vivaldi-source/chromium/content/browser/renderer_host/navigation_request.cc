...
void AddAdditionalRequestHeaders(
  ...
  if (vivaldi::IsVivaldiRunning()) {
    ...
    if (user_agent_override.empty()) {
      vivaldi_user_agent::UpdateAgentString(false, ua_string, &url);
    }
...
std::unique_ptr<NavigationRequest> NavigationRequest::Create(
  ...
  auto navigation_params = blink::mojom::BeginNavigationParams::New(
      ...
      true /*vivaldi_from_trusted_event*/);
...
void NavigationRequest::OnRedirectChecksComplete(
  ...
  if (client_hints_delegate) {
    ...
    if (!devtools_user_agent_override_) {
      // Since we have a boundary here from content to UI we cannot use
      // ScopedVivaldiThreadURL.
      std::string final_ua = ComputeUserAgentValue(GetUserAgentOverride());
      vivaldi_user_agent::UpdateAgentString(/*reduced=*/false, final_ua,
                                            &common_params_->url);
      headers_update_params.modified_headers.SetHeader(
          net::HttpRequestHeaders::kUserAgent,
          final_ua);
...
// Vivaldi
bool NavigationRequest::VivaldiIsFromTrustedEvent() {
  return begin_params().vivaldi_from_trusted_event;
}
// End Vivaldi

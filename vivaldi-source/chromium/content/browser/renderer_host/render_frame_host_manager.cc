...
RenderFrameHostManager::GetReplacementFrameToken(
  ...
  if (existing_proxy) {
    ...
    // frames.
// TEMP : VB-127181    CHECK(frame_tree_node_->parent());
    if (current_frame_host()->IsRenderFrameLive()) {
      ...
      // a subframe.
// TEMP : VB-127181         CHECK(render_frame_host->GetSiteInstance() !=
//                current_frame_host()->GetSiteInstance() ||
//            render_frame_host_
//                ->ShouldChangeRenderFrameHostOnSameSiteNavigation());
// TEMP : VB-127181      CHECK_NE(render_frame_host, current_frame_host());
...
// TEMP : VB-127181   CHECK(current_frame_host()->must_be_replaced_for_crash());
...
void RenderFrameHostManager::CommitPending(
  ...
  // NOTE(andre@vivaldi.com): Opening a tab from an extension in Vivaldi would
  // end up here without a RenderWidgetHostView. Create it here as a fallback as
  // non-guest content get it's view created in
  // WebContentsImpl::CreateNewWindow. See VB-54634, Pocket extension.
  if (!new_view) {
    delegate_->CreateRenderWidgetHostViewForRenderManager(new_rvh);
    new_view = render_frame_host_->GetView();
  }
  ...
  if (clear_proxies_on_commit) {
    ...
    for (auto& it :
      ...
      if (!is_outer_delegate_proxy && render_frame_host_->GetSiteInstance()
               ->group()
  ...
  if (proxy_to_parent_or_outer_delegate && proxy_to_parent_or_outer_delegate->cross_process_frame_connector()) {

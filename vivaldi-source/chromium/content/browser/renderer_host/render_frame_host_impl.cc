...
#if !BUILDFLAG(IS_ANDROID)
  // NOTE(andre@vivaldi.com) : We need to use the top level guest as this holds
  // the DownloadInfo and does the permission handling. VB-40929
  int process_host_id = GetProcess()->GetDeprecatedID();
  int frame_host_routing_id = GetRoutingID();
  if (vivaldi::IsVivaldiRunning()) {
    // Loop through the tree and find the webviewguest.
    const RenderFrameHostImpl* guest = nullptr;
    const RenderFrameHostImpl* current = this;
    while (!guest && current) {
      RenderFrameHostImpl* parent =
          current->GetParentOrOuterDocumentOrEmbedder();
      if (parent && !parent->GetParentOrOuterDocumentOrEmbedder())
        guest = current;
      current = parent;
    };
    if(guest) {
      process_host_id = guest->GetProcess()->GetDeprecatedID();
      frame_host_routing_id = guest->GetRoutingID();
    }
  }
#endif // !IS_ANDROID // End Vivaldi
...
#else // Vivaldi
      process_host_id, frame_host_routing_id, traffic_annotation);
#endif // IS_ANDROID // End Vivaldi
}
...
bool RenderFrameHostImpl::CanExecuteJavaScript() {
  ...
  return !GetLastCommittedURL().is_valid() ||
         ...
         (vivaldi::IsVivaldiApp(GetLastCommittedURL().host()));
}

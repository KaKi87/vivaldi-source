...
void WebContentsImpl::WebContentsTreeNode::DetachUnownedInnerWebContents(
  ...
  inner_web_contents_node.outer_contents_frame_tree_node_id_ =
      FrameTreeNodeId();
  // NOTE(andre@vivaldi.com) : Added for VB-118520, reported upstream.
  current_web_contents_->GetPrimaryFrameTree().RemoveFrame(
...
void WebContentsImpl::WebContentsTreeNode::OnFrameTreeNodeDestroyed(
  ...
  // Make sure we can reattach with a new ProxyHost if the same unowned
  // WebContents is reattached. Was VB-117316 due to upstream change:
  // https://source.chromium.org/chromium/chromium/src/+/b5adf39c0bd6c08318ec9a58b09a6ea2a1732a1b
...
void WebContentsImpl::SetDelegate(WebContentsDelegate* delegate) {
  ...
  // note(ondrej@vivaldi) VB-114624 - When WebViewGuest attaches the
  // delegate must exchange the download info and the postponed callbacks.
  if (delegate && delegate_) {
    delegate->download_info_ = delegate_->download_info_;
    delegate->vivaldi_postponed_calls_ =
      std::move(delegate_->vivaldi_postponed_calls_);
  }
...
void WebContentsImpl::AttachInnerWebContents(
  ...
  bool should_take_ownership =
      !VivaldiTabCheck::IsOwnedByTabStripOrDevTools(inner_web_contents.get());
...
#if defined(USE_AURA)
  // VB-119695: Make sure the newly created childview get overscroll support, if
  // enabled.
  // VB-126068: Set overscroll only for Aura - macOS can cause crashes.
  inner_web_contents_impl->view_->SetOverscrollControllerEnabled(
      inner_web_contents_impl->CanOverscrollContent());
...
#if BUILDFLAG(IS_MAC)
  bool create_guest_for_vivaldi =
      inner_web_contents_impl->view_->IsWebContentsViewChildFrame();
#endif
...
#if BUILDFLAG(IS_MAC)
  if (create_guest_for_vivaldi) {
    // NOTE(tomas@vivaldi):
    // VB-109565 make sure the new view is the same as the old view
    inner_web_contents_impl->view_ = std::make_unique<WebContentsViewChildFrame>(
        inner_web_contents_impl,
        GetContentClient()->browser()->GetWebContentsViewDelegate(
            inner_web_contents_impl),
...
void WebContentsImpl::RecursivelyUnregisterRenderWidgetHostViews() {
  ...
  for (auto* view : GetRenderWidgetHostViewsInWebContentsTree()) {
    ...
    // In Vivaldi's case we want to unregister the cached TextInputManager
    // because the lifecycle are different. We keep the view until we re-attach
    // when a new view is created. VB-118319.
    auto* text_input_manager_for_view = view->GetTextInputManager();
    if (text_input_manager_for_view) {
      text_input_manager_for_view->Unregister(view);
    }
...
void WebContentsImpl::EnterFullscreenMode(
  ...
  // In Vivaldi ContainsOrIsFocusedWebContents and
  // IsFullscreenAllowedForUnfocusedWebContents will return false. If we have
  // devtools inspecting a tab/webview this check will wrongly trigger. The
  // structure behind seems flaky and not possible to tweak for this at the
  // moment without breaking other things, like permissions. See VB-128549.
...
#if !BUILDFLAG(IS_ANDROID)
      if (vivaldi::IsVivaldiRunning()) {
        create_params.context = gfx::NativeView();
#if BUILDFLAG(IS_MAC)
        // NOTE(konrad@vivaldi.com): VB-99252
        if (params.disposition !=
            WindowOpenDisposition::NEW_PICTURE_IN_PICTURE) {
          // NOTE(andre@vivaldi.com) : This is necessary on MAC because the
          // initial non-guest-view is not tore down before attaching to the
          // parent. Was VB-44298 & VB-109565.
...
void WebContentsImpl::DragSourceEndedAt(float client_x,
  ...
  bool canceled =
#if BUILDFLAG(IS_WIN)
      vivaldi::g_cancelled_drag;
#else
      operation == ui::mojom::DragOperation::kNone;
#endif
  VivaldiEventHooks::HandleDragEnd(this, client_x, client_y, screen_x,
                                   screen_y, canceled);
...
void WebContentsImpl::RunJavaScriptDialog(
  ...
  if (vivaldi::IsVivaldiRunning()) {
    ...
    if (!vivaldi::IsVivaldiApp(url.host()) && !GetOuterWebContents()) {
      // NOTE(ondrej@vivaldi.com): VB-91963 - Show native dialogs in PWA.
      const bool is_web_app = delegate_ && delegate_->IsWebApp();
      if (!is_web_app) {
...
void WebContentsImpl::OnDialogClosed(int render_process_id,
  ...
  observers_.NotifyObservers(&WebContentsObserver::OnJavascriptDialogClosed,
                             success);
}

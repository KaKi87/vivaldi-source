...
- (void)dragEnterHitTestDidCompleteForView:
  ...
  if (vivaldi::IsVivaldiRunning() && vivaldi::IsTabDragInProgress()) {
    // Always direct the tab drag to Vivaldi UI
    DCHECK(!_webContents->GetOuterWebContents());
    targetRWH = content::RenderWidgetHostImpl::From(
        _webContents->GetRenderViewHost()->GetWidget());
  } // End Vivaldi

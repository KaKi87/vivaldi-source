...
void RenderFrameImpl::UpdateSubresourceLoaderFactories(
  ...
  if (loader_factories_->IsHostChildURLLoaderFactoryBundle()) {
    ...
    // opener/parent).
    // NOTE(andre@vivaldi.com) : SynchronouslyCommitAboutBlankForBug778318 set
    // navigation_commit_state_ to NavigationCommitState::kNone.
...
RenderFrameImpl::UserAgentMetadataOverride() {
  bool override_metadata = ShouldUseUserAgentOverride();
  if (vivaldi::IsVivaldiRunning() || override_metadata) {
    return GetWebView()
        ->GetRendererPreferences()
        .user_agent_override.GetUaMetaDataOverride(
            GURL(frame_->GetDocument().Url()).host(), override_metadata);
  }
  return std::nullopt;
}
...
void RenderFrameImpl::BeginNavigationInternal(
  ...
  blink::mojom::BeginNavigationParamsPtr begin_params =
      blink::mojom::BeginNavigationParams::New(
          ...
          info->has_rel_opener, info->triggering_event_info == blink::mojom::TriggeringEventInfo::kFromTrustedEvent);

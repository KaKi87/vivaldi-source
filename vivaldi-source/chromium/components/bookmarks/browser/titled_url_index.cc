...
// Vivaldi: Have to be kept at this location due to dependency
// on a function in the anonymous namespace
...
TitledUrlIndex::RetrieveNicknameNodesMatchingAnyTerms(
  ...
  TitledUrlIndex::TitledUrlNodeSet prefix_matches;
  for (const std::u16string& term : terms) {
    ...
    while (i != nickname_index_.end() && IsPrefix(term, i->first)) {
      auto vector = TitledUrlNodes(i->second.begin(), i->second.end());
      for (auto i1 : vector) {
        prefix_matches.insert(prefix_matches.end(), i1);
      }
      ++i;
    }
  }

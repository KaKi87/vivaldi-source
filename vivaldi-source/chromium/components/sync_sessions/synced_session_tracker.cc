...
void SerializeTrackerToSpecifics(
  ...
  for (const SyncedSession* session :
    ...
    // VB-129443 - this was missing, causing "unreliable" behavior.
    const VivaldiSpecific& vivaldi_specific = session->GetVivaldiSpecific();
    if (vivaldi_specific.panels || vivaldi_specific.workspaces) {
      session_tag_to_tab_node_ids[session_tag].insert(
          TabNodePool::kVivaldiTabNodeID);
    }
  }

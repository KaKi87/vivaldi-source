...
constexpr auto kDefaultSupportedLanguages =
    base::MakeFixedFlatSet<LanguageTag>({
        ...
        GetKnownLanguageTag("zh-Hans"),   // Chinese (Simplified)
        GetKnownLanguageTag("zh-Hant"),   // Chinese (Traditional)
#endif // Vivaldi
...
TranslateLanguageList::TranslateLanguageList(
  ...
  if (vivaldi::IsVivaldiRunning()) {
    language_list_fetcher_.reset(); // Not needed for Vivaldi
  }
}
...
void TranslateLanguageList::SetLanguageList(std::vector<std::string> list) {
  ...
  SortAndUnique(supported_languages_from_service);
  base::flat_set<LanguageTag>(base::sorted_unique,
                              std::move(supported_languages_from_service));
  ...
  NotifyEvent(__LINE__, "Language list set by Vivaldi: " +
                            base::JoinString(languages_as_strings, ", "));

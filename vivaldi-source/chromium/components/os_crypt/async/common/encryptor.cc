...
bool Encryptor::DecryptString16WithRawKey(const std::string& ciphertext,
  ...
  if (ciphertext.empty()) {
    *plaintext = std::u16string();
    return true;
  }

  mojom::Algorithm algorithm =
      key.size() == Key::kAES256GCMKeySize ? mojom::Algorithm::kAES256GCM
                                            : mojom::Algorithm::kAES128CBC;

  // Build a temporary keyring with the raw key under all known provider tags.
  // The DecryptData() logic matches the tag prefix in the ciphertext to find
  // the right key, so we cover all possibilities here.
  KeyRing keyring;
  for (const auto& tag : {std::string("v10"), std::string("v11"),
                          std::string("k2")}) {
    keyring.emplace(tag, Encryptor::Key(key, algorithm));
  }

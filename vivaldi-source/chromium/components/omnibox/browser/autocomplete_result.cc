...
size_t AutocompleteResult::GetMaxMatches(
  ...
  size_t desktop_max_matches =
      client && vivaldi::IsVivaldiRunning()
          ? client->GetPrefs()->GetInteger(
                vivaldiprefs::kAddressBarOmniboxDropdownSize)
          : VIVALDI_DESKTOP_DEFAULT_MAX_MATCHES;
  size_t vivaldi_max_matches =
      is_android_any ? 10 : (is_ios ? 10 : desktop_max_matches);
  size_t vivaldi_zero_suggest_max_matches =
      is_android_any ? 15 : (is_ios ? 20 : desktop_max_matches);
#endif
  constexpr size_t kDefaultMaxAutocompleteMatches =
      is_android_mobile ? 10 : (is_ios ? 6 : 8);
...
// When the user types '@', show 9, instead of the usual 8, matches on desktop.
  ...
  if (is_zero_suggest) {
#if defined(VIVALDI_BUILD)
    return vivaldi_zero_suggest_max_matches;
#else // Vivaldi
...
#if defined(VIVALDI_BUILD)
  size_t field_trial_value = vivaldi_max_matches;
#else // Vivaldi
...
void AutocompleteResult::SortAndCull(
  ...
  // Suggest maximum 42 items for zero-suggest dropdown. This is desktop only.
  max_search_suggestions = 42u;
  max_suggestions = 42u;
#endif
  ...
  if (is_zero_suggest) {
    ...
    if (is_android_any && omnibox::IsAndroidHub(page_classification)) {
      ...
      if (omnibox::IsLensSearchbox(page_classification)) {
        ...
        bool add_iph_section =
            ...
            has_iph_match;
        // Vivaldi: Increase from 8 to 42 to get more results
        // when showing RECENT_TYPED_HISTORY.
        sections.push_back(std::make_unique<DesktopNTPZpsSection>(
            suggestion_groups_map_, add_iph_section ? 7u : 42u, mia_enabled));
#if BUILDFLAG(ENABLE_EXTENSIONS)
    ...
    const size_t num_matches =
        CalculateNumMatches(is_zero_suggest, input.GetFeaturedKeywordMode(),
                            matches_, comparing_object, client);
    ...
    #if defined(VIVALDI_BUILD)
          // Limit the number of search history items to 5.
          int nb_search_history_item = 0;
          for (auto it = matches_.rbegin(); it != matches_.rend();) {
            if (it->type == AutocompleteMatchType::SEARCH_HISTORY) {
              nb_search_history_item++;
              if (nb_search_history_item > 5) {
                matches_.erase(std::next(it).base());
              }
            }
            ++it;
          }
...
size_t AutocompleteResult::CalculateNumMatches(
  ...
  if (!is_zero_suggest &&
    ...
    return CalculateNumMatchesPerUrlCount(matches, comparing_object, client);
  }
...
size_t AutocompleteResult::CalculateNumMatchesPerUrlCount(
  ...
  size_t base_limit = GetMaxMatches(
      false, AutocompleteInput::FeaturedKeywordMode::kFalse, client);
  size_t increased_limit = GetDynamicMaxMatches(client);
...
void AutocompleteResult::DeduplicateMatches(
  ...
  const TemplateURL* search_url =
      input.search_engine_guid.empty()
          ? template_url_service->GetDefaultSearchProvider()
          : template_url_service->GetTemplateURLForGUID(
                input.search_engine_guid);
  bool use_post = !search_url->search_url_post_params().empty();
  // Erase duplicate matches.
  std::erase_if(*matches, [&url_to_matches, &search_url,
                           &use_post](const AutocompleteMatch& match) {
    auto match_comparison_fields = GetMatchComparisonFields(match);
    // Vivaldi
    bool keep_suggestion_using_post = false;
    if (search_url) {
      // If the search URL uses POST, search-suggestions URLs will be
      // considered as duplicates. We keep search-suggestions in that case.
      keep_suggestion_using_post =
          (match.type != ACMatchType::SEARCH_SUGGEST || !use_post);
    }  // End Vivaldi

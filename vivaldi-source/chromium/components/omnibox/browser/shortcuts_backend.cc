...
bool ShortcutsBackend::Init(int number_of_days_to_keep_visits) {
  ...
  // NOTE(arnar@vivaldi.com) Vivaldi overrides delete duration of old visits
  if (number_of_days_to_keep_visits < 0)
    number_of_days_to_keep_visits =
        history::HistoryBackend::kExpireDaysThreshold;
  number_of_days_to_keep_visits_ = number_of_days_to_keep_visits;
...
void ShortcutsBackend::ComputeDatabaseMetrics() {
  ...
  for (const auto& shortcut_pair : shortcuts_map_) {
    if (now - shortcut_pair.second.last_access_time >
        base::Days(number_of_days_to_keep_visits_)) {
      num_old_shortcuts++;
    }
  }

...
void SearchProvider::Start(const AutocompleteInput& input,
  ...
  const TemplateURL* default_provider =
    input.from_search_field && !input.search_engine_guid.empty() ?
        model->GetTemplateURLForGUID(input.search_engine_guid) :
...
void SearchProvider::StartOrStopSuggestQuery(bool minimal_changes) {
  ...
  const bool query_is_private = IsQueryPotentiallyPrivate() || keyword_turl;

  client()->SetFromSearchField(input_.from_search_field);

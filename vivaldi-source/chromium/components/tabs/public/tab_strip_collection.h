...
class TabStripCollection : public TabCollection {
 public:
  ...
  // Vivaldi
  size_t IndexOfFirstVivaldiTab() const;
  ...
  // NOTE(ondrej@vivaldi.com): Support for the vivaldi panels. They are placed
  // in vivaldi_collection_ and they should not move.
  bool AreMovable(const std::vector<int>&) const;
 ...
 private:
  ...
  // NOTE(ondrej@vivaldi.com): vivaldi=true, the tab goes to vivaldi_collection_
  ...
  // Vivaldi panels and widgets go there.
  raw_ptr<VivaldiTabCollection> vivaldi_collection_ = nullptr;

...
TabStripCollection::TabStripCollection()
  ...
  if (vivaldi::IsVivaldiRunning()) {
    vivaldi_collection_ =
        AddCollection(std::make_unique<VivaldiTabCollection>(), 2);
  }
}
...
void TabStripCollection::AddTabRecursiveImpl(
  ...
  auto [tab_collection_ptr, insert_index] = GetInsertionDetails(
      index, new_pinned_state, new_group_id, tab->IsVivaldiPanel());
...
void TabStripCollection::MoveTabsRecursive(
  ...
  auto [tab_collection_ptr, insert_index] =
      ...
      // NOTE(ondrej@vivaldi.com): Only the regular tabs are moving. Those
      // are not panels nor widgets.
      false);
...
TabGroupTabCollection* TabStripCollection::AddTabGroup(
  ...
  const int dst_index =
      (index == static_cast<int>(VivaldiTabCountRecursive()))
...
void TabStripCollection::InsertTabCollectionAt(
  ...
  std::pair<tabs::TabCollection*, int> insertion_details =
      GetInsertionDetails(index, pinned, parent_group, false);
  auto [tab_collection_ptr, insert_index] = insertion_details;
...
std::pair<tabs::TabCollection*, int> TabStripCollection::GetInsertionDetails(
  ...
  if (vivaldi) {
    ...
    // Vivaldi panels/widgets go to the vivaldi collection.
    // The order within the collection is void.
    insert_collection = vivaldi_collection_;
    direct_dst_index = 0;
  } else if (pinned) {

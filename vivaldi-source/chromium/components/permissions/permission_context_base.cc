...
content::PermissionResult PermissionContextBase::GetPermissionStatus(
  ...
  // NOTE(andre@vivaldi.com) : We do not use OverridePermissionResult or
  // IsPermissionRequestable. Do the regular tab content handling for
  // permissions for guests attached to our window. VB-120859
  // NOTE(volca@vivaldi.com) : Check if owner_rfh is valid before accessing
  // GetOwnerSiteURL(). During mass tab closures, the owner RenderFrameHost
  // may be destroyed while guest permission observers are still active,
  // causing a null pointer dereference crash. VB-122160
  if (guest && guest->owner_rfh() &&
      guest->GetOwnerSiteURL().host() != vivaldi::kVivaldiAppId) {
...
void PermissionContextBase::NotifyPermissionSet(
  ...
  if (vivaldi::IsVivaldiRunning()) {
    vivaldi::permissions::NotifyPermissionSet(
        request_data.id, content_settings_type_,
        PermissionUtil::PermissionDecisionToContentSetting(
            decision.overall_decision));
  }
#endif

...
class SyncAuthManager : public signin::IdentityManager::Observer {
 public:
  ...
  // internals UI.
  virtual SyncTokenStatus GetSyncTokenStatus() const;
  ...
  virtual void ConnectionStatusChanged(ConnectionStatus status);
 ...
 private:
  friend class vivaldi::VivaldiSyncAuthManager;

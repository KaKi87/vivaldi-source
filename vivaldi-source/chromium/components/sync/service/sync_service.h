...
// This enum is parsed by java_cpp_enum.py, which doesn't accept &&ing flags.
  ...
  // Vivaldi methods
  virtual void ClearSyncData() {}
  virtual bool is_clearing_sync_data() const { return false; }
  virtual std::optional<std::string> GetEncryptionBootstrapTokenForBackup()
      const {
    return "";
  }
  virtual bool ResetEncryptionBootstrapTokenFromBackup(

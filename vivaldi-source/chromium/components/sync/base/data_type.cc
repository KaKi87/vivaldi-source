...
constexpr std::array<DataTypeInfo, syncer::GetNumDataTypes()>
    kDataTypeInfoTable = {{
        ...
        {
            .type = NOTES,
            .specifics_field_number = sync_pb::EntitySpecifics::kNotes,
            .debug_string = "Notes",
            .histogram_suffix = "NOTES",
            .stable_lowercase_string = "notes",
            // Nigori has built-in encryption and powers the encryption of other
            // datatypes.
            .encryption_policy =
                EncryptionPolicy::kEncryptedIfCustomPassphraseSet,
...
static_assert(GetNumDataTypes() == 63 + 1 /* Notes */,
...
DataTypeSet EncryptableUserTypes() {
  ...
  for (const auto& info : kDataTypeInfoTable) {
    // Vivaldi
    // NOTE(julien): Chromium disables encryptions for a number of types
    // that it uses for communicating with the server. We do not need most of
    // them and they could lead to privacy leaks, therefore we implement our own
    // list here.
    //
    if (UserTypes().Has(info.type) && info.type != DEVICE_INFO &&
        info.type != NIGORI) {
    // End Vivaldi

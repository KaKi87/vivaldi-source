...
class GuestViewBase::OwnerContentsObserver : public WebContentsObserver {
 public:
  ...
  void WebContentsDestroyed() override {
    ...
    // The comment below is not accurate in Vivaldi as we do not destroy tab and
    // devtool WebContents. See |WebViewGuest::VivaldiCreateWebContents| where
    // the old guest is destroyed via WebContentsDestroyed.
...
#if BUILDFLAG(ENABLE_EXTENSIONS)
    // NOTE(andre@vivaldi.com) : We can set muting on a tab, which is a guest.
    // So
...
void GuestViewBase::WebContentsDestroyed() {
  ...
  // tabstrip. VB-93668.
  // We need to invalidate the id so any lookup through
  // GuestViewManager::GetGuestByInstanceID does not look up a guest without a
  // webcontents. It will not be recreated. See
  // https://source.chromium.org/chromium/chromium/src/+/04ba266f5e8b7be1959682addf22d6727eba64ab
  // for further details.
  bool invalidate_id = false;
...
void GuestViewBase::CompleteInit(base::DictValue create_params,
  ...
  if (base::FeatureList::IsEnabled(features::kGuestViewMPArch)) {
    ...
    // Vivaldi
    // web_contents() will not be set in some cases (like when we activate an
    ...
    if (vivaldi::IsVivaldiRunning() && guest_web_contents) {
      ...
      static_cast<content::WebContentsImpl*>(guest_web_contents.get())
...
#if defined (VIVALDI_BUILD) && BUILDFLAG(ENABLE_EXTENSIONS)
  // NOTE(andre@vivaldi.com) : If the webcontents belong to a tabstrip don't
  // touch it. This caused VB-99800.
  // When the window is closed TabStripModel::CloseAllTabs is called which
  // destroys any webcontents, which leads us to here. Just release any
  // owned_guest_contents_ that is attached to any tabstrip since it must own
  // it.
  if (owned_guest_contents_) {
    bool contents_is_present_in_tabstrip =
        owned_guest_contents_->WasDiscarded() ||
        VivaldiTabCheck::IsVivaldiTab(owned_guest_contents_.get());
    if (contents_is_present_in_tabstrip) {
      owned_guest_contents_.release();
      return;
    }
  }
#endif  // ENABLE_EXTENSIONS && VIVALDI_BUILD
...
void GuestViewBase::SetUpSizing(const base::DictValue& params) {
  ...
  gfx::Size defsize = GetDefaultSize();
  // If element size has not been set, get default size. The owner.
  if (normal_height == 0) {
    normal_height = defsize.height();
  }
  if (normal_width == 0) {
    normal_width = defsize.width();
  }

...
// everything.
const int kEntriesPerReset = 1;
        // NOTE(andre@vivaldi.com): Was |40|; we need to update the file
        // contents more often because we show the full list of entries where
        // Chrome only restore the last closed entry.
...
bool TabRestoreServiceImpl::PersistenceDelegate::ConvertSessionWindowToWindow(
  ...
  for (auto& i : session_window->tabs) {
    ...
    // Vivaldi
    tab.viv_ext_data = i->viv_ext_data;
    tab.viv_page_action_overrides = i->viv_page_action_overrides;
  }

  window->viv_ext_data = session_window->viv_ext_data;

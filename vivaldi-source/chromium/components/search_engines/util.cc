...
void MergeIntoEngineData(const TemplateURLData& original_turl,
  ...
  data_to_update.is_active = original_turl.is_active;
  data_to_update.vivaldi_position = original_turl.vivaldi_position;
}
...
ActionsFromCurrentData CreateActionsFromCurrentPrepopulateData(
  ...
  for (auto& turl : existing_urls) {
    ...
    if (MatchesDefaultSearchProvider(turl.get(), default_search_provider[0],
                                     /*check_keyword=*/false)) {
      if (!dsp_match ||
  ...
  if (base::FeatureList::IsEnabled(switches::kKwdbRefreshDebugging)) {
    ...
    SCOPED_CRASH_KEY_BOOL("KwdbRefresh", "has_no_preloaded_dsp",
                          default_search_provider[0] == nullptr);
    ...
    SCOPED_CRASH_KEY_BOOL("KwdbRefresh", "is_dsp_prepopulated",
                          default_search_provider[0] &&
                              default_search_provider[0]->prepopulate_id() > 0);

    SCOPED_CRASH_KEY_BOOL("KwdbRefresh", "is_dsp_from_policy",
                          default_search_provider[0] &&
                              default_search_provider[0]->enforced_by_policy());

    SCOPED_CRASH_KEY_BOOL(
        "KwdbRefresh", "is_dsp_from_extension",
        default_search_provider[0] &&
            (default_search_provider[0]->type() ==
    ...
    SCOPED_CRASH_KEY_BOOL(
        ...
        default_search_provider[0] &&
            default_search_provider[0]->CreatedByRegulatoryProgram());
  ...
  for (const auto& [id, template_url] : id_to_turl) {
    if (entries_matching_dsp_to_reconcile > 0 &&
        MatchesDefaultSearchProvider(template_url, default_search_provider[0],
                                     /*check_keyword=*/true)) {
      --entries_matching_dsp_to_reconcile;

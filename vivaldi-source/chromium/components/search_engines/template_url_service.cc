...
TemplateURLService::TemplateURLService(
    ...
    : prefs_(prefs),
      ...
      default_search_manager_(
          VivaldiGetDefaultSearchManagers(&prefs, &search_engine_choice_service, prepopulate_data_resolver_.get())
          ),
...
void TemplateURLService::RegisterProfilePrefs(
  ...
  vivaldi::RegisterTemplateURLServiceProfilePrefs(registry);
...
TemplateURL* TemplateURLService::Add(
  ...
  if (IsCreatedByExtension(*template_url.get())) {
    int32_t new_id = -2;
    new_id -= base::PersistentHash(
        template_url->GetExtensionInfo()->extension_id + template_url->url());
    template_url->set_id(new_id);
...
TemplateURLService::GetDefaultSearchManager() {
  return &default_search_manager_;
}

const TemplateURL* TemplateURLService::GetDefaultSearchProvider(DefaultSearchType type, bool ignore_viv_override) const {
  if (!ignore_viv_override && vivaldi_default_override_)
...
void TemplateURLService::RepairPrepopulatedSearchEngines() {
  ...
  for (auto i(actions.edited_engines.begin()); i < actions.edited_engines.end();
       ++i) {
    i->second.is_active = TemplateURLData::ActiveStatus::kUnspecified;
...
void TemplateURLService::OnWebDataServiceRequestDone(
  ...
  if (default_search_provider_[kDefaultSearchMain]) {
    ...
    if (engine_type == SEARCH_ENGINE_OTHER) {
      GURL search_url = GURL(default_search_provider_[kDefaultSearchMain]->url());
    ...
    if (default_search_provider_[kDefaultSearchMain]->CreatedByDefaultSearchProviderPolicy()) {
      base::UmaHistogramEnumeration(
...
std::optional<syncer::ModelError> TemplateURLService::ProcessSyncChanges(
  ...
  for (auto iter = change_list.begin(); iter != change_list.end(); ++iter) {
    ...
    if (iter->change_type() == syncer::SyncChange::ACTION_DELETE) {
      ...
      } else if (!VivaldiIsDefaultSearchProvider(existing_turl)) {
...
void TemplateURLService::StopSyncing(syncer::DataType type) {
  ...
  for (size_t i = 0; i < template_urls_.size();) {
    ...
    if (turl->GetLocalData()) {
      ...
      ++i;
    } else if (!VivaldiIsDefaultSearchProvider(turl)) {
      Remove(turl);
    } else {
...
bool TemplateURLService::ApplyDefaultSearchChangeNoMetrics(
  ...
  if (!data) {
    ...
    if (default_search_provider_[type]) {
      ...
      UpdateData(default_search_provider_[type], update_data);
    } else {
    ...
    if (default_search_provider_[type]) {
      UpdateData(default_search_provider_[type], new_data);
    } else {
...
TemplateURL* TemplateURLService::Add(std::unique_ptr<TemplateURL> template_url,
  ...
  if (template_url->type() == TemplateURL::NORMAL &&
    ...
    if (position.IsValid())
      template_url->set_vivaldi_position(syncer::UniquePosition::After(
          position, vivaldi::VivaldiGetPositionSuffix(template_url.get())));
    else
      template_url->set_vivaldi_position(
  ...
  for (TemplateURL* conflicting_turl : local_duplicates) {
    ...
    if (base::FeatureList::IsEnabled(
      ...
      const bool is_default_search_provider =
          VivaldiIsDefaultSearchProvider(conflicting_turl);
      base::UmaHistogramBoolean(
    ...
    if (VivaldiIsDefaultSearchProvider(conflicting_turl) ||
      ...
      DCHECK(!VivaldiIsDefaultSearchProvider(conflicting_turl));
  ...
  if (local_duplicates.empty() &&
    ...
    if (conflicting_built_in_turl &&
      ...
      if (default_search_provider != default_search_provider_.end()) {
        type = std::distance(default_search_provider_.begin(), default_search_provider);
      }

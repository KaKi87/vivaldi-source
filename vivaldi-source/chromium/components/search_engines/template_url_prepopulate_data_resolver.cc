...
std::unique_ptr<TemplateURLData> Resolver::GetFallbackSearch(TemplateURLPrepopulateData::SearchType search_type) const {
  return TemplateURLPrepopulateData::GetPrepopulatedFallbackSearch(
      ...
      // only where it needs to be used.
      regional_capabilities_->GetCountryId().GetRestricted(CountryAccessKey(
          CountryAccessReason::kTemplateURLPrepopulateDataResolution)), search_type);
}

...
std::optional<TemplateURLData> KeywordTable::GetKeywordDataFromStatement(
  ...
  data.featured_by_policy = s.ColumnBool(26 + 1 /* vivaldi position field */);
...
// desktop window manager.
  ...
  if (!encryptor()->IsDecryptionAvailable()) {
    ...
    const auto hash = encryptor()->DecryptData(s.ColumnBlob(27 + 1 /* vivaldi position field */));
    if (!hash) {
...
void KeywordTable::BindURLToStatement(const TemplateURLData& data,
  ...
  s->BindBool(starting_column + 25 + 1 /* vivaldi position field */, data.featured_by_policy);
  if (encryptor()->IsEncryptionAvailable()) {
    const std::vector<uint8_t> url_hash = data.GenerateHash();
    ...
    s->BindBlob(starting_column + 26 + 1 /* vivaldi position field */, *std::move(encrypted_hash));
  } else {
    s->BindNull(starting_column + 26 + 1 /* vivaldi position field */);
  }
}
...
bool KeywordTable::UpdateKeyword(const TemplateURLData& data) {
  ...
  BindURLToStatement(data, &s, /*id_column=*/27 + 1 /* vivaldi position field */, /*starting_column=*/0);
  return s.Run();
}

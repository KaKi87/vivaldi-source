...
SyncedBookmarkTrackerEntity* SyncedBookmarkTracker::AddInternal(
  ...
  // vivaldi
  entity->set_vivaldi_synced_file_store(SyncedBookmarkTrackerEntity::PassKey(),
...
SyncedBookmarkTracker::InitEntitiesFromModelAndMetadata(
  ...
  for (sync_pb::BookmarkMetadata& bookmark_metadata :
    ...
    if (metadata->IsDeleted()) {
      ...
      // vivaldi
      tombstone_entity->set_vivaldi_synced_file_store(
          SyncedBookmarkTrackerEntity::PassKey(), vivaldi_synced_file_store_);
      // end vivaldi
    ...
    // vivaldi
    entity->set_vivaldi_synced_file_store(
        SyncedBookmarkTrackerEntity::PassKey(), vivaldi_synced_file_store_);
    // end vivaldi
...
void SyncedBookmarkTracker::CheckAllNodesTracked(
  ...
  if (vivaldi::IsVivaldiRunning())
    DCHECK(GetEntityForBookmarkNode(bookmark_model->trash_node()));

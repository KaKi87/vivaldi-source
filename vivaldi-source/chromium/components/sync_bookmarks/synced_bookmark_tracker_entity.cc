...
void SyncedBookmarkTrackerEntity::RecordAcceptedRemoteUpdate(
  ...
  if (vivaldi_synced_file_store_) {
    auto thumbnail_checksum = VivaldiGetSyncedThumbnailChecksumFromSpecifics(
        update.entity.specifics.bookmark());
    if (thumbnail_checksum)
      vivaldi_synced_file_store_->SetSyncFileRef(
          metadata().server_id(), syncer::BOOKMARKS, *thumbnail_checksum);
    else
      vivaldi_synced_file_store_->RemoveSyncRef(metadata().server_id(),
                                                syncer::BOOKMARKS);
  }

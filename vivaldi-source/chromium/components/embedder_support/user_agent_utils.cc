...
blink::UserAgentBrandList GenerateBrandVersionList(
  ...
  std::string major_version_brand = version;
  std::string full_version_brand = version;
  vivaldi::SelectClientHintsBrand(brand, major_version_brand,
                                  full_version_brand);

  vivaldi::UpdateBrands(additional_brand_version);
  ...
  if (brand) {
    brand_version_list.emplace_back(brand.value(), brand_version);
  }

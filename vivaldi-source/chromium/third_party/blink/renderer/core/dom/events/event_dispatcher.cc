...
EventDispatcher::DispatchEventLegacyPreActivationBehavior(
  ...
  // Vivaldi
  const bool disable_canditate =
      (event_->IsMouseEvent() &&
       (event_->type() == event_type_names::kClick ||
        event_->type() == event_type_names::kMousedown ||
        event_->type() == event_type_names::kMouseup)) ||
      (event_->IsPointerEvent() &&
       (event_->type() == event_type_names::kPointerdown ||
        event_->type() == event_type_names::kPointerup));

  if (disable_canditate && view_ && view_->GetPage() &&
      view_->GetPage()->GetSettings().GetHasVivaldiFollowerTab()) {
    Node* node = node_;
    while (node) {
      if (node->IsLink()) {
        if (auto* element = DynamicTo<Element>(node)) {
          const AtomicString& href_value =
              element->getAttribute(html_names::kHrefAttr);

          KURL target_url = element->GetDocument().CompleteURL(href_value);
          if (target_url.IsValid()) {
            Document& document = element->GetDocument();
            KURL current_url = document.Url();

            if (!EqualIgnoringFragmentIdentifier(target_url, document.Url()) &&
                target_url.ProtocolIsInHttpFamily()) {

...
SoftNavigationHeuristics::MaybeCreateEventScopeForEvent(const Event& event) {
  ...
  // Vivaldi VB-113263 BEGIN
  blink::LocalDOMWindow* dom_window = window_.Get();
  if (dom_window) {
    blink::LocalFrame* frame = dom_window->GetFrame();
    if (frame) {
      auto &world = blink::DOMWrapperWorld::MainWorld(dom_window->GetIsolate());
      auto *proxy = frame->GetWindowProxyMaybeUninitialized(world);
      if (proxy) {
        if (proxy->NeedInitialize()) {
          return std::nullopt;
        }
      }
    }

...
const i18nStringVivaldi = i18n.i18n.getVivaldiLocalizedString.bind(undefined, str_);
...
const DEFAULT_VIEW = (input: ViewInput, output: ViewOutput, target: HTMLElement): void => {
  const renderSettingCheckboxIfNeeded = (): Lit.LitTemplate => {
    ...
    const warningText = input.warningType === WarningType.SYNC_DISABLED ? i18nStringVivaldi(UIStrings.syncDisabled) : i18nStringVivaldi(UIStrings.preferencesSyncDisabled);
  ...
  const renderAccountInfo = (): Lit.LitTemplate => {
    if (!input.syncInfo.accountEmail) {
      return html`
        <div class="not-signed-in">${i18nStringVivaldi(UIStrings.notSignedIn)}</div>
      `;
    }
    ...
    return html`
      <div class="account-info">
        ...
        <div class="account-email">
          <span>${i18nStringVivaldi(UIStrings.signedIn)}</span>

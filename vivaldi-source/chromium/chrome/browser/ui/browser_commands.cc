...
content::WebContents* DuplicateTabAt(BrowserWindowInterface* browser,
  ...
  ::vivaldi::TabExtDataImpl::Create(raw_contents_dupe);
  ...
  if (browser->GetBrowserForMigrationOnly()->CanSupportWindowFeature(
    ...
    // Vivaldi: Override to allow for duplicating many tabs in one go while only
    // activating the last (gives better behavior in UI).
    if (vivaldi_add_types) {
      add_types = *vivaldi_add_types;
    }
    // End Vivaldi
    const auto old_group = tab_strip_model->GetTabGroupForTab(index);

    // NOTE(ondrej@vivaldi.com)
    std::optional<int> vivaldi_index =
        vivaldi::tab_positioning::DetermineDuplicateIndex(
            tab_strip_model, contents, contents_dupe.get(), add_types);

    if (vivaldi_index) {
      dst_index = *vivaldi_index;
    }
  ...
  if (vivaldi::IsVivaldiRunning()) {
    bool is_discarded =
        resource_coordinator::TabLifecycleUnitExternal::FromWebContents(
            contents) &&
        resource_coordinator::TabLifecycleUnitExternal::FromWebContents(
            contents)->GetTabState() == ::mojom::LifecycleUnitState::DISCARDED;
    bool is_active = vivaldi_add_types
        ? (*vivaldi_add_types & ADD_ACTIVE)
        : true;
    if (is_discarded || !is_active) {
      // Note: Setting discarded if active has no effect. Page will be loaded
      // in any case.
      resource_coordinator::TabLifecycleUnitExternal::FromWebContents(
          raw_contents_dupe)->SetIsDiscarded();
    }
  }
...
WebContents* GetTabAndRevertIfNecessaryHelper(BrowserWindowInterface* browser,
  ...
  switch (disposition) {
    ...
    default:
      if (!browser->GetBrowserForMigrationOnly()->is_vivaldi())
...
void ReloadInternal(BrowserWindowInterface* browser,
  ...
  for (WebContents* const tab : tabs_to_reload) {
    ...
    if (tab == active_contents && !new_tab->FocusLocationBarByDefault() && canFocus) { // Vivaldi
      new_tab->Focus();
    }
...
BrowserWindowInterface* OpenEmptyWindow(Profile* profile,
  ...
  if (vivaldi::IsVivaldiRunning()) {
    // VB-114710 Address Bar not focusing when no windows are open
    // Don't add a tab, on macOS it has the wrong url
    // Vivaldi app will handle the rest.
    return browser;
  }
...
content::WebContents& NewTab(BrowserWindowInterface* browser,
  ...
  if (!displayer_browser->is_vivaldi())
...
BrowserWindowInterface* OpenInChrome(
  ...
  if (target_browser->is_vivaldi() != hosted_app_browser->is_vivaldi()) {
    ...
    WebContents::CreateParams params(hosted_app_browser->GetProfile());
    params.always_create_guest = target_browser->is_vivaldi();

    std::unique_ptr<WebContents> dest_contents(WebContents::Create(params));
    TabStripModel* source_tabstrip = hosted_app_browser->GetTabStripModel();
...
// Vivaldi: Need to be here, due to using an anonymous namespace function
content::WebContents* VivaldiDuplicateTabAt(
    BrowserWindowInterface* browser, int index, int dst_index, int add_types) {
  return ::DuplicateTabAt(browser, index, dst_index, &add_types);
}
// End Vivaldi

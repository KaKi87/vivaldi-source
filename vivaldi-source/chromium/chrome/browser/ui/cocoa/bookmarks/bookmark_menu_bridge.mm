...
void BookmarkMenuBridge::BuildRootMenu(bool recurse) {
  ...
  if (vivaldi::IsVivaldiRunning()) {
    ...
    vivaldi::AddExtraBookmarkMenuItems(menu_root_, model, bookmark_bar_folder,
        true);
    std::vector<bookmarks::BookmarkNode*> nodes;
    vivaldi::GetBookmarkNodes(model, bookmark_bar_folder, nodes);
...
void BookmarkMenuBridge::AddChildrenToMenu(const BookmarkParentFolder& folder,
  ...
  if (vivaldi::IsVivaldiRunning()) {
    BookmarkModel* model = bookmark_service_->bookmark_model();
    vivaldi::AddExtraBookmarkMenuItems(menu, model, folder, true);
    std::vector<bookmarks::BookmarkNode*> nodes;
    vivaldi::GetBookmarkNodes(model, folder, nodes);
    for (const BookmarkNode* node : nodes) {
      AddNodeToMenu(node, menu, recurse);
    }
    vivaldi::AddExtraBookmarkMenuItems(menu, model, folder, false);
  } else {
...
void BookmarkMenuBridge::AddNodeToMenu(const BookmarkNode* node,
  ...
  if (vivaldi::IsVivaldiRunning()) {
    ...
    if (::vivaldi_bookmark_kit::IsSeparator(node)) {
      ...
      AddSubmenu(menu, BookmarkParentFolder::FromFolderNode(node),
                 folder_image_, recurse);
    } else {
      // TODO: Test properly for problems wrt ARC transition
      NSMenuItem* item =
          [[NSMenuItem alloc] initWithTitle:MenuTitleForNode(node)
                                     action:nil
                              keyEquivalent:@""];
      bookmark_nodes_[node] = item;
      ConfigureMenuItem(node, item);

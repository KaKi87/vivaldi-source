...
void BrowserWindowFeatures::Init(BrowserWindowInterface* browser) {
  ...
  */ // Vivaldi
  ...
  if (vivaldi::IsVivaldiRunning()) {
    ...
    vivaldi_browser_ui_data_ =
        std::make_unique<vivaldi::VivaldiBrowserUiData>(browser);
  }  // End Vivaldi
...
void BrowserWindowFeatures::InitPostWindowConstruction(Browser* browser) {
  ...
  VivaldiBrowserWindow* vivaldi_browser_window =
      VivaldiBrowserWindow::FromBrowser(browser);
  ...
  } else if (vivaldi_browser_window) {
    focus_manager = vivaldi_browser_window->GetWidget()->GetFocusManager();
  }
  ...
  synced_window_delegate_ =
      ...
      vivaldi::IsVivaldiRunning()
          ? std::make_unique<VivaldiBrowserSyncedWindowDelegate>(
                browser, browser->GetTabStripModel(), browser->GetSessionID(),
                browser->GetType()) :
#endif
	std::make_unique<BrowserSyncedWindowDelegate>(
      browser, browser->GetTabStripModel(), browser->GetSessionID(),
      browser->GetType());
  ...
  if (vivaldi::IsVivaldiRunning() &&
    ...
    // Note this must only be set for browser windows that needs
    // extension_keybinding_registry_.

    browser_window_ = BrowserWindow::FromBrowser(browser);
  }  // IsVivaldiRunning

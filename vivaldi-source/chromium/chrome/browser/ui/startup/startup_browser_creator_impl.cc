...
Browser* StartupBrowserCreatorImpl::OpenTabsInBrowser(
  ...
  for (auto& tab : tabs) {
    ...
    // add_types |= AddTabTypes::ADD_FORCE_INDEX; // Disabled by vivaldi
...
StartupBrowserCreatorImpl::DetermineStartupTabs(
  ...
  if (vivaldi::IsVivaldiRunning() &&
      !profile_->GetPrefs()->GetBoolean(
          vivaldiprefs::kStartupHasSeenWelcomePage)) {
    profile_->GetPrefs()->SetBoolean(vivaldiprefs::kStartupHasSeenWelcomePage,
                                     true);

...
base::WeakPtr<content::NavigationHandle> NavigateImpl(
  ...
  if (params->contents_to_insert) {
    ...
    if (params->source_contents &&
        ((params->tabstrip_add_types & AddTabTypes::ADD_INHERIT_OPENER) ||
         params->user_gesture)) {
      // vivaldi, VB-126815
      ::vivaldi::tab_positioning::SetOpener(
          tab_to_insert.get(), params->source_contents,
          params->browser->GetBrowserForMigrationOnly()->tab_strip_model());
    }
  }
  ...
  // tab.
  bool made_new_contents = false; // Vivaldi
...
#endif
      made_new_contents = true; // Vivaldi
    } else {
  ...
  if (tab_to_insert) {
    ...
    if (::vivaldi::IsVivaldiRunning()) {
      if (params->positioning_params) {
        vivaldi::TabExtData::Get(tab_to_insert->GetContents())
             ->SetPositioningParams(*params->positioning_params);
      }
    }
  ...
  if (params->source_contents == contents_to_navigate_or_insert) {
    ...
    if (::vivaldi::IsVivaldiRunning() && !params->viv_ext_data.empty()) {
      vivaldi::TabExtData::Get(contents_to_navigate_or_insert)
          ->Merge(params->viv_ext_data);
    }

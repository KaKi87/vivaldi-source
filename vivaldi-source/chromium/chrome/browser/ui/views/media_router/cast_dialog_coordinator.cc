...
void CastDialogCoordinator::ShowDialogCentered(
  ...
  if (vivaldi::IsVivaldiRunning()) {
    ...
    gfx::Rect rect(api->GetDialogPosition(active_browser->session_id().id(),
      "chromecast", &flow_direction));
    Show(views::BubbleAnchor(anchor_view), views::BubbleBorder::FLOAT,
         controller, profile, start_time, activation_location, nullptr,
         std::move(after_shown_callback));
    GetCastDialogView()->SetAnchorRect(rect);
  } else { // Vivaldi

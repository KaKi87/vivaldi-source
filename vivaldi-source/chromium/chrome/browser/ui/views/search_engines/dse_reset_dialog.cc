...
void MaybeShowSearchEngineResetNotification(
  ...
  if (!NeedsDseResetNotification(profile, default_search_engine[0].data())) {
    return;
  }
  ...
  if (!template_url_service->GetDefaultSearchProvider() &&
    ...
    default_search_engine[0].data()->SetUnacknowledgedDefaultSearchEngineReset(
        false);
    return;
  }

  ShowSearchEngineResetNotification(browser, default_search_engine[0].data());
#endif  // BUILDFLAG(IS_WIN) || BUILDFLAG(IS_MAC)
}

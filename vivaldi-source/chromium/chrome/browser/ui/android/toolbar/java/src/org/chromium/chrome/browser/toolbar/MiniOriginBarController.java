...
public class MiniOriginBarController implements Observer {
    ...
    protected void showMiniOriginBar() {
        if (mControlContainer.getView().findViewById(R.id.find_toolbar) != null // VAB-12946 Crash fix
                && mControlContainer.getView().findViewById(R.id.find_toolbar).getVisibility() == View.VISIBLE) return; // VAB-12933 Do not show on Find in Page
    ...
    private @MiniOriginState int getNewMiniOriginState(@MiniOriginEvent int miniOriginEvent) {
        switch (mMiniOriginBarState) {
            case MiniOriginState.NOT_READY -> {
                if ((BuildConfig.IS_VIVALDI || mIsFormFieldFocusedSupplier.get() || mIsCobrowseSheetOpen)

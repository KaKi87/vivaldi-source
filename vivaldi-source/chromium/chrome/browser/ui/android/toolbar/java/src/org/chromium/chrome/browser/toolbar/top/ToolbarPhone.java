...
public class ToolbarPhone extends ToolbarLayout
    ...
    public void onFinishInflate() {
        try (TraceEvent te = TraceEvent.scoped("ToolbarPhone.onFinishInflate")) {
            ...
            mBackButton.setImageDrawable(
                    getResources().getDrawable(R.drawable.vivaldi_bottom_nav_back_56dp));
            mForwardButton.setImageDrawable(
                    getResources().getDrawable(R.drawable.vivaldi_bottom_nav_forward_56dp));
            // End Vivaldi
    ...
    protected void onNativeLibraryReady() {
        ...
        // TODO CHR142: Replace ForwardButton with the ForwardButtonCoordinator
        if (mForwardButton != null && mForwardButtonCoordinator != null) {
            /*mForwardButton.setClickCallback(metaState
                    -> mForwardButtonCoordinator.forward(metaState, "VivaldiToolbarPhoneForward"));
    ...
    public boolean onInterceptTouchEvent(MotionEvent ev) {
        ...
        // views.
        if (BuildConfig.IS_VIVALDI && isInTabSwitcherMode()) return true;
    ...
    private int getBoundsAfterAccountingForLeftButton() {
        ...
        if (BuildConfig.IS_VIVALDI) { // Ref: VAB-12212
            return padding + (int) getResources().getDimension(R.dimen.address_bar_offset);
        }
    ...
    private int getBoundsAfterAccountingForRightButtons() {
        ...
        if (BuildConfig.IS_VIVALDI) { // Vivaldi Ref. VAB-8862
            toolbarButtonsContainerWidth += (int) getResources().getDimension
    ...
    private @ColorInt int getToolbarColorForVisualState(final @VisualState int visualState) {
        ...
        switch (visualState) {
            ...
            case VisualState.NORMAL:
                ...
                else if (!isTabStripOn() // Vivaldi Ref: VAB-12174
                        // Temporarily enable VAB-12693 on Sopranos builds only.
                        || getContext().getPackageName().contains("sopranos")) {
                    return getToolbarColorForVisualState(VisualState.BRAND_COLOR);
                } // Vivaldi End
    ...
    private void updateToolbarAndLocationBarColorForFocusChange() {
        ...
        if ((urlHasFocus() || !isLocationBarShownInNtp()) && mTabSwitcherState == STATIC_TAB) {
            ...
            if (BuildConfig.IS_VIVALDI) isInGeneralNtp = false; // Vivaldi VAB-9752
            ...
            if (BuildConfig.IS_VIVALDI && isIncognitoBranded()) { // Vivaldi VAB-10631
                primaryColor = getToolbarDefaultColor(shouldUseFocusColor);
            } // End Vivaldi
    ...
    private void updateProgressBarVisibility() {
        getProgressBar().setVisibility(mTabSwitcherState != STATIC_TAB || !isNativeLibraryReady() /* Vivaldi */ ? INVISIBLE : VISIBLE);
    }
    ...
    private void updateVisualsForLocationBarState() {
        ...
        // Vivaldi VAB-12163
        if (BuildConfig.IS_VIVALDI && getLocationBarColorForToolbarColor(currentPrimaryColor) != -1)
    ...
    // Vivaldi
    void drawLocationBarButton(ChromeImageButton button, Canvas canvas, float alpha) {
    ...
    private void displayNavigationPopup(boolean isForward, View anchorView) {
        ...
        mNavigationPopup = new NavigationPopup(tab.getProfile(),
    ...
    public void onBottomToolbarVisibilityChanged(boolean isVisible, int orientation) {
        ...
        int endPadding =
                getResources().getDimensionPixelSize(R.dimen.location_bar_margin_start);
        mHomeButton.setPadding(mHomeButton.getPaddingLeft(), mHomeButton.getPaddingTop(),
                endPadding, mHomeButton.getPaddingBottom());

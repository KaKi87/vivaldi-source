    ...
    public void populateModel(
        ...
        if (!isSearchSuggestion) {
            if (!suggestion.getUrl().isEmpty()
                ...
                // Vivaldi VAB-12922: replace chrome-native:// with vivaldi-native://.
                if (BuildConfig.IS_VIVALDI) {
                    SuggestionSpannable str = new SuggestionSpannable(
                            VivaldiUrlConstants.replaceInternalScheme(
        ...
        if (suggestion.getType() == DIRECT_MATCH && BuildConfig.IS_VIVALDI) { // Vivaldi VAB-10000
            ...
            Drawable hintBackground = ResourcesCompat.getDrawable(mContext.getResources(),
                    model.get(SuggestionCommonProperties.COLOR_SCHEME)
                                    == BrandedColorScheme.INCOGNITO
                            ? R.drawable.direct_match_label_dark
                            : R.drawable.direct_match_label_light,
                    mContext.getTheme());
            if (hintBackground != null) {
                hintBackground.setBounds(
                        ...
                        /* top= */ 0,
                        /* right */
                        (int) mContext.getResources().getDimension(R.dimen.direct_match_hint_width),
                        /* bottom */

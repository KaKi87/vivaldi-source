...
class LocationBarMediator
    ...
    private boolean mVivaldiMiniBarActive;
    private final ButtonToolbarWidthConsumer mReloadButtonToolbarWidthConsumer;
    private final ButtonToolbarWidthConsumer mQrCodeButtonToolbarWidthConsumer;
    ...
    /*package */ LocationBarMediator(
        ...
        // Vivaldi
        mReloadButtonToolbarWidthConsumer =
                new ButtonToolbarWidthConsumer(
                        mContext,
                        mIsTablet,
                        this::shouldShowReloadButton,
                        this::setReloadButtonVisibility);
        mQrCodeButtonToolbarWidthConsumer =
                new ButtonToolbarWidthConsumer(
                        mContext,
                        mIsTablet,
                        this::shouldShowQrCodeButton,
                        this::setQrCodeButtonVisibility);
        // End Vivaldi
    }
    ...
    /*package */ void onUrlFocusChange(UrlBarFocusChangeInfo info) {
        ...
        // Vivaldi VAB-13131: Clear the one-shot focus-on-new-tab signal once the URL bar is
        // focused, so later tabs (e.g. opened from a link) do not inherit it. Done last so the
        // focus handling above still observes it.
        if (hasFocus) {
    ...
    /* package */ void onUrlTextRichChanged(UrlBarTextChangeInfo info) {
        ...
        // Vivaldi VAB-12793: suppress keyword mode text.
        if (!BuildConfig.IS_VIVALDI && shouldTriggerSiteSearch(info) && mAutocompleteCoordinator != null) {
    ...
    /* package */ void onSuggestionsChanged(
        ...
        if (mScrimHandler != null) {
            ...
            mScrimHandler.setVisibility(
                    mCurrentInput.getAutocompleteState() == AutocompleteState.ENABLED
                            && vivaldiAllowsFocusScrim()); // Vivaldi VAB-13364
        }
        ...
        if (mUrlCoordinator.shouldAutocomplete()) {
            ...
            if (!BuildConfig.IS_VIVALDI
                    && defaultMatch != null
                    && defaultMatch.getAssociatedKeyword() != null) {
                TemplateUrlService templateUrlService = mTemplateUrlServiceSupplier.get();
                if (templateUrlService != null) {
                    siteSearchLabel =
    ...
    /* package */ void finishUrlFocusChange(boolean showExpandedState, boolean shouldShowKeyboard) {
        ...
        if (shouldShowKeyboard) {
            mUrlCoordinator.setUrlBarData(mUrlCoordinator.getUrlBarData(),
                    UrlBar.ScrollType.NO_SCROLL, TextSelection.SELECT_ALL);
        }
        // End Vivaldi
    }
    ...
    void beginOrResumeInput(boolean activateNewSession) {
        ...
        // Vivaldi VAB-12819/VAB-13137: Only on the NTP, suppress zero suggest so the speed dials
        // show.
        if (BuildConfig.IS_VIVALDI && mAutocompleteCoordinator != null
            ...
            mAutocompleteCoordinator.setSuppressNextZeroSuggest(true);
    ...
    /* package */ void updateBrandedColorScheme() {
        ...
        mLocationBarLayout.setQrCodeButtonTint(
                ThemeUtils.getThemedToolbarIconTint(mContext, mBrandedColorScheme));
        mLocationBarLayout.setReloadButtonTint(
                ThemeUtils.getThemedToolbarIconTint(mContext, mBrandedColorScheme));
    }
    ...
    boolean shouldShowMicButton() {
        // Vivaldi
        // [Kia/Hyundai] For Kia PV5, the microphone button should not be visible. Ref. AUTO-306.
        final String DEVICE_NAME_KIA_PV5 = "pbvivi";
        if (BuildConfig.IS_OEM_GAS_BUILD && android.os.Build.DEVICE.equals(DEVICE_NAME_KIA_PV5))
        ...
        if (mIsTablet && mShouldShowButtonsWhenUnfocused) {
            ...
            // Vivaldi Instantly remove mic button on unfocus
            if (BuildConfig.IS_VIVALDI){
    ...
    public void onUrlChanged(boolean isTabChanging) {
        if (isTabChanging) {
            ...
            if (currentSession == null || !currentSession.isSessionActive()) {
                ...
                // underneath it.
                // Vivaldi VAB-11992: FOCUS_ADDRESS_BAR_ON_NEW_TAB is set only on new tab creation.
                boolean skipEndInput = BuildConfig.IS_VIVALDI
                        && VivaldiPreferences.getSharedPreferencesManager()
                                .readBoolean(VivaldiPreferences.FOCUS_ADDRESS_BAR_ON_NEW_TAB,
                                        false);
                if (!skipEndInput) {
                    endInput();
                } else if (mUrlHasFocus) {
                    beginOrResumeInput(/* activateNewSession= */ true);
                } else if (mContext.getResources().getConfiguration().keyboard
                        == Configuration.KEYBOARD_QWERTY) {
                    // Focus the URL bar immediately on hardware keyboard so no keystrokes are lost.
                    showUrlBarCursorWithoutFocusAnimations();
                } // End Vivaldi VAB-11992
    ...
    private static class ButtonToolbarWidthConsumer implements ToolbarWidthConsumer {
        ...
        public boolean hasSpaceToShow() {
            // NOTE(jarle@vivaldi.com): Always show reload button.
    ...
    /** Vivaldi */
    private void setQrCodeButtonVisibility(boolean shouldShowQrCodeButton) {
        mLocationBarLayout.setQrCodeButtonVisibility(
                shouldShowQrCodeButton && mQrCodeButtonToolbarWidthConsumer.hasSpaceToShow());
    }
    ...
    boolean shouldShowReloadButton() {
        ...
        return currentTab != null
                ...
                && !mVivaldiMiniBarActive;
    }

    /** Vivaldi. Updates the display of the Reload button. */
    private void updateReloadButtonVisibility() {
        setReloadButtonVisibility(shouldShowReloadButton());
    }

    // Vivaldi VAB-12859: called when mini origin bar shows/hides.
    void setVivaldiMiniBarActive(boolean active) {
        mVivaldiMiniBarActive = active;
        updateReloadButtonVisibility();
    }

    /** Vivaldi */
    private void setReloadButtonVisibility(boolean shouldShowReloadButton) {
        mLocationBarLayout.setReloadButtonVisibility(
                shouldShowReloadButton && mReloadButtonToolbarWidthConsumer.hasSpaceToShow());
    }
    ...
    private void maybeUpdateStatusBarVisibility() {
        ...
        boolean isAddressBarAtBottom = ChromeSharedPreferences.getInstance().readBoolean(
    ...
    /** VAB-13364: the focus scrim is over the content, so it dims the Speed Dials. */
    private boolean vivaldiAllowsFocusScrim() {

...
void TabStripModel::ActivateTabAt(int index,
  ...
  // Vivaldi panels don't move, stays in vivaldi collection.
  if (!IsMovable(index)) {
    return;
  }
...
void TabStripModel::AddTab(std::unique_ptr<tabs::TabModel> tab,
  ...
  if (index_by_vivaldi) {
    if (index_by_vivaldi->pinned) { // vivaldi VB-128789
      ...
      add_types &= ~ADD_PINNED; // VB-129634
    }
...
int TabStripModel::ConstrainInsertionIndex(int index,
                                           bool pinned_tab) const {
  int non_vivaldi_tabs =
      static_cast<int>(contents_data_->IndexOfFirstVivaldiTab());
  return pinned_tab
             ? std::clamp(index, 0, IndexOfFirstNonPinnedTab())
             : std::clamp(index, IndexOfFirstNonPinnedTab(), non_vivaldi_tabs);
}
...
int TabStripModel::ConstrainMoveIndex(int index,
  ...
  return pinned_tab
             ...
             : std::clamp(index, IndexOfFirstNonPinnedTab(), non_vivaldi_tabs - 1);
}
...
void TabStripModel::MoveTabToIndexImpl(
  ...
  if (!contents_data_->IsMovable(initial_index)) {
    return;
  }
...
bool TabStripModel::MoveTabsWithNotifications( // Vivaldi
    std::vector<int> tab_indices,
    int destination_index,
    base::OnceClosure execute_tabs_move_operation) {

  if (!contents_data_->AreMovable(tab_indices)) {
    // NOTE(ondrej@vivaldi.com): This check ensures nobody would move the
    // vivaldi panels out of the vivaldi container.
    return false;
  }

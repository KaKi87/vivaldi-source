...
#if !BUILDFLAG(IS_ANDROID) // Vivaldi: TODO(Chr142): Those flags cause linker errors.
...
#if !BUILDFLAG(IS_ANDROID) // Vivalid
...
#if defined(VIVALDI_BUILD)
#include "browser/features/vivaldi_flags.inc"
#endif
...
bool ShouldSkipConditionalFeatureEntry(const flags_ui::FlagsStorage* storage,
  ...
  if (vivaldi::IsVivaldiRunning() &&
      vivaldi::VivaldiShouldSkipConditionalFeatureEntry(storage, entry)) {
    return true;
  }

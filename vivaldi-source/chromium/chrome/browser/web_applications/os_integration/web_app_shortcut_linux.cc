...
// Vivaldi: Desktop file override for SNAP.
#include "extraparts/vivaldi_snap_utils.h"
...
base::FilePath GetDesktopPath() {
  ...
  // Vivaldi: Fixes the path for .desktop files in SNAP (where XDG_DATA_HOME is
  // local to snap directories)
  if (vivaldi::GetSnapDesktopPathOverride(&desktop_path))
    return desktop_path;

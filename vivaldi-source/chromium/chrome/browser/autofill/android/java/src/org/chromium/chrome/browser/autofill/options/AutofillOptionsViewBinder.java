...
// Vivaldi
import static org.chromium.chrome.browser.autofill.options.AutofillOptionsProperties.AUTOFILL_SETTING_ON;
import static org.chromium.chrome.browser.autofill.options.AutofillOptionsProperties.ON_AUTOFILL_SETTING_TOGGLED;
...
class AutofillOptionsViewBinder {
    ...
    public static void bind(PropertyModel model, AutofillOptionsFragment view, PropertyKey key) {
        // Vivaldi
        view.getThirdPartyFillingOption().setVisible(false); // Hide third party option.
        ChromeSwitchPreference autofillToggleSwitch = view.getAutofillToggleSwitch();
        autofillToggleSwitch.setTitle(R.string.settings_autofill_toggle_title);
        autofillToggleSwitch.setSummary(R.string.settings_autofill_toggle_summary);
        // End Vivaldi
        ...
        // Vivaldi
        } else if (key == AUTOFILL_SETTING_ON) {
            boolean platformAutofillEnabled = model.get(AUTOFILL_SETTING_ON);
            autofillToggleSwitch.setChecked(platformAutofillEnabled);
        } else if (key == ON_AUTOFILL_SETTING_TOGGLED) {
            autofillToggleSwitch
                    .setOnPreferenceChangeListener(
                            (preference, newValue) -> {
                                model.get(ON_AUTOFILL_SETTING_TOGGLED)
                                        .onResult((boolean) newValue);
                                return true;
                            });
        // End Vivaldi

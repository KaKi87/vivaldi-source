...
class AutofillOptionsViewBinder {
    ...
    public static void bind(PropertyModel model, AutofillOptionsFragment view, PropertyKey key) {
        ...
        if (key == FRAGMENT_TITLE) {
            ...
            // Vivaldi
        } else if (key == AutofillOptionsProperties.AUTOFILL_SETTING_ON) {
            boolean platformAutofillEnabled =
                    model.get(AutofillOptionsProperties.AUTOFILL_SETTING_ON);
            autofillToggleSwitch.setChecked(platformAutofillEnabled);
        } else if (key == AutofillOptionsProperties.ON_AUTOFILL_SETTING_TOGGLED) {
            autofillToggleSwitch.setOnPreferenceChangeListener((preference, newValue) -> {
                model.get(AutofillOptionsProperties.ON_AUTOFILL_SETTING_TOGGLED)
                        .onResult((boolean) newValue);
                return true;
            });
            // End Vivaldi

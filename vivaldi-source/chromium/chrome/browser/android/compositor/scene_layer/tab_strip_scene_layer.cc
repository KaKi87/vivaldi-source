...
void TabStripSceneLayer::UpdateLoadingState(
  ...
  if (title_resource) {
    ...
    // Vivaldi VAB-13164: tab_strip_layer_ (which carries the strip color) is
    // hidden while loading, so back the loading text with background_layer_.
    background_layer_->SetBackgroundColor(
        should_show_loading ? SkColor4f::FromColor(background_color)

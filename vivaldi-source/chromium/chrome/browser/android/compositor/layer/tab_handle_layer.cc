...
void TabHandleLayer::SetProperties(
  ...
  // Vivaldi VAB-13228: these resources can be null while their drawables are
  // still loading. Skip this frame instead of dereferencing them.
  if (vivaldi::IsVivaldiRunning() &&
      (!tab_handle_resource || !tab_handle_outline_resource)) return;
  ...
  if (media_indicator_size > 0) {
    ...
    // Vivaldi VAB-13172: Center the media indicator on favicon-only tabs.
    if (vivaldi::IsVivaldiRunning() && is_shown_as_favicon &&
        close_button_alpha == 0.f) {
      media_x = (width / 2.f) - (media_indicator_size / 2.f);
    }
    float media_y =

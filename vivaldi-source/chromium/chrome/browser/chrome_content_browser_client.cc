...
#if BUILDFLAG(IS_ANDROID)
  // Vivaldi AUTO-348: automotive embeds media apps (YouTube) as Custom Tabs.
  // Autoplay without a user gesture so ads and the next video keep playing.
...
ChromeContentBrowserClient::ChromeContentBrowserClient() {
  ...
  extra_parts_.push_back(std::make_unique<VivaldiContentBrowserClientParts>());
}
...
bool ChromeContentBrowserClient::CanCreateWindow(
  ...
  *no_javascript_access = false;
  ...
  bool can_open_window = !blocked_content::ConsiderForPopupBlocking(disposition) ||
...
ChromeContentBrowserClient::CreateURLLoaderThrottles(
  ...
  if (vivaldi::IsVivaldiRunning() && request.url.is_valid() &&
    ...
    result.push_back(std::make_unique<vivaldi::VivaldiURLLoaderThrottle>(
        frame_tree_node_id));
  }
...
void AddChromeSchemeFactories(
  ...
  if (vivaldi::IsVivaldiApp(extension->id())) {
    // Vivaldi webuis loads chrome://vivaldi-data, thumb and favicon urls.
    allowed_webui_hosts.emplace_back(vivaldi::kVivaldiUIDataHost);
    // Prevent double instances, this can be registered above. vector :-/
    bool addFaviconHost = true;
    for (std::string host : allowed_webui_hosts) {
      if (host == chrome::kChromeUIFaviconHost) {
        addFaviconHost = false;
        break;
      }
    }
    if (addFaviconHost) {
    ...
    allowed_webui_hosts.emplace_back(chrome::kChromeUIFavicon2Host);
    allowed_webui_hosts.emplace_back(chrome::kChromeUIThemeHost);
  }
...
void ChromeContentBrowserClient::WillCreateURLLoaderFactory(
  ...
  // Vivaldi
  mojo::PendingRemote<network::mojom::TrustedURLLoaderHeaderClient>
      request_filter_manager_forwarding_header_client;
  mojo::PendingRemote<network::mojom::TrustedURLLoaderHeaderClient>*
      request_filter_manager_header_client = header_client;
  std::optional<int64_t> request_filter_navigation_id;
  if (vivaldi::IsVivaldiRunning()) {
    if (navigation_id)
      request_filter_navigation_id = navigation_id;
    header_client = &request_filter_manager_forwarding_header_client;
  }
  // End vivaldi
  ...
  if (vivaldi::IsVivaldiRunning()) {
    ...
    request_filter_manager->ProxyURLLoaderFactory(
        browser_context, frame, render_process_id, type,
        std::move(request_filter_navigation_id), factory_builder,
        request_filter_manager_header_client,
        std::move(request_filter_manager_forwarding_header_client),
...
void ChromeContentBrowserClient::CreateWebSocket(
    ...
    mojo::PendingRemote<network::mojom::WebSocketHandshakeClient>
        handshake_client,
    content::ContentBrowserClient::WebSocketOptions options) {
  vivaldi::RequestFilterManager* vivaldi_request_filter_manager = nullptr;
  if (vivaldi::IsVivaldiRunning() && frame) {
    vivaldi_request_filter_manager =
      vivaldi::RequestFilterManagerFactory::GetForBrowserContext(
  ...
  if (vivaldi_request_filter_manager) {
    factory = vivaldi_request_filter_manager->ForwardProxyWebSocket(
        frame->GetProcess()->GetID().value(), frame->GetRoutingID(),
        frame->GetLastCommittedOrigin(), std::move(factory), site_for_cookies,
        user_agent);
  }
  ...
  if (!vivaldi_request_filter_manager)
    return;
  vivaldi_request_filter_manager->ProxyWebSocket(
      frame->GetProcess()->GetID().value(), frame->GetRoutingID(),
      frame->GetLastCommittedOrigin(), std::move(factory),
      site_for_cookies, user_agent, url,
      std::vector<network::mojom::HttpHeaderPtr>(), std::move(handshake_client),
      mojo::NullRemote(), std::move(options.header_client));
#endif
}
...
void ChromeContentBrowserClient::WillCreateWebTransport(
  ...
  if (vivaldi::IsVivaldiRunning()) {
    callback = vivaldi_request_filter_manager->ForwardProxyWebTransport(
        render_process_host->GetID().value(), frame_routing_id, url,
  ...
  vivaldi_request_filter_manager->ProxyWebTransport(
      *render_process_host, frame_routing_id, url, initiator_origin,

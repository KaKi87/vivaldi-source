...
void PopulateExtensionItems(content::BrowserContext* browser_context,
  ...
  int index = 0;
  auto ids = menu_manager->ExtensionIds(); //Vivaldi
  std::set<MenuItem::ExtensionKey>::iterator iter = ids.begin(); // Vivaldi
  for (const auto& title : sorted_menu_titles) {
    DCHECK(iter != ids.end()); // Vivaldi
    const std::vector<const Extension*>& extensions =
        title_to_extensions_map[title];
    for (const Extension* extension : extensions) {
      MenuItem::ExtensionKey extension_key(extension->id());
      const MenuItem::ExtensionKey extension_key_w_webviewid = *iter++; // Vivaldi
      bool has_webview = (extension_key_w_webviewid.webview_instance_id != guest_view::kInstanceIDNone); // Vivaldi
      matcher.AppendExtensionItems(has_webview ? extension_key_w_webviewid : extension_key, printable_selection_text,
                                   &index,

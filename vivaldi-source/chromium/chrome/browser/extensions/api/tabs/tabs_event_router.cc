...
void TabsEventRouter::TabEntry::NavigationEntryCommitted(
  ...
  // VB-91252: GetLastCommittedURL() is what is sent, need to use that as state
  ...
  if (::vivaldi::IsVivaldiRunning() &&
    ::vivaldi::IsWorkspacesEnabled(web_contents()) &&
    !is_in_pwa &&
    !ignoreLinkRouting &&
    ...
    for (base::Value& elm : ::vivaldi::getLinkRoutes(web_contents())) {
      ...
      std::optional<double> workspace_id = dict->FindDouble("workspaceId");
    ...
    if (target_workspace_id > -1) {
      ...
      Browser* target_browser =
          VivaldiBrowserComponentWrapper::GetInstance()->GetWorkspaceBrowser(
              target_workspace_id);
...
void TabsEventRouter::OnZoomChanged(
  ...
  DispatchEvent(profile, events::TABS_ON_ZOOM_CHANGE,
                ...
                SuggestFiltering(data.web_contents));
}
...
void TabsEventRouter::DispatchTabInsertedAt(TabStripModel* tab_strip_model,
  ...
  object_args.Set(kDiscardedKey,
                          is_discarded || is_lazy_loaded_tab);

  ::vivaldi::AddVivaldiTabItemsToEvent(contents, object_args);
...
void TabsEventRouter::DispatchTabSelectionChanged(
  ...
  for (int index : new_selection) {
    ...
    if (!contents)
      break;
    int tab_id = ExtensionTabUtil::GetTabId(contents);
    if (!SuggestFiltering(contents)) {
    all_tabs.Append(tab_id);
    } // vivaldi
    viv_tabs.Append(tab_id);
  }
  ...
  DispatchEvent(profile, events::TABS_ON_HIGHLIGHT_CHANGED,
                ...
                EventRouter::UserGestureState::kUnknown, Event::EXCLUDE_VIVALDI);
  DispatchEvent(profile, events::TABS_ON_HIGHLIGHTED,
                api::tabs::OnHighlighted::kEventName, std::move(args),
                EventRouter::UserGestureState::kUnknown, Event::EXCLUDE_VIVALDI);

  // The same content with the vivaldi panels goes to vivaldi only.
  args.clear();
  select_info.Set(
      tabs_constants::kWindowIdKey,
      ExtensionTabUtil::GetWindowIdOfTabStripModel(tab_strip_model));

  select_info.Set(kTabIdsKey, std::move(viv_tabs));
  args.Append(std::move(select_info));

  // The onHighlighted event replaced onHighlightChanged.
  profile = tab_strip_model->profile();
  DispatchEvent(profile, events::TABS_ON_HIGHLIGHT_CHANGED,
                api::tabs::OnHighlightChanged::kEventName, args.Clone(),
                EventRouter::UserGestureState::kUnknown, Event::VIVALDI_ONLY);
  DispatchEvent(profile, events::TABS_ON_HIGHLIGHTED,
                api::tabs::OnHighlighted::kEventName, std::move(args),
                EventRouter::UserGestureState::kUnknown, Event::VIVALDI_ONLY);
}
...
void TabsEventRouter::TabCreatedAt(WebContents* contents,
  ...
  event->filter_non_vivaldi = SuggestFiltering(contents);
...
void TabsEventRouter::DispatchEvent(
  ...
  event->filter_non_vivaldi = filter;

...
int MoveTabToWindow(ExtensionFunction* function,
  ...
  if (!tabs_internal::GetTabById(tab_id, function->browser_context(),
                                 ...
                                 error, /* Vivaldi */ function->extension()) ||
      !source_window) {
    return -1;
  }
...
ui::PageTransition HistoryExtensionTransitionToUiTransition(
  ...
  switch (transition) {
    case extensions::api::history::TransitionType::kLink:
      return ui::PAGE_TRANSITION_LINK;
    case extensions::api::history::TransitionType::kTyped:
      return ui::PAGE_TRANSITION_TYPED;
    case extensions::api::history::TransitionType::kAutoBookmark:
      return ui::PAGE_TRANSITION_AUTO_BOOKMARK;
    case extensions::api::history::TransitionType::kAutoSubframe:
      return ui::PAGE_TRANSITION_AUTO_SUBFRAME;
    case extensions::api::history::TransitionType::kManualSubframe:
      return ui::PAGE_TRANSITION_MANUAL_SUBFRAME;
    case extensions::api::history::TransitionType::kGenerated:
      return ui::PAGE_TRANSITION_GENERATED;
    case extensions::api::history::TransitionType::kAutoToplevel:
      return ui::PAGE_TRANSITION_AUTO_TOPLEVEL;
    case extensions::api::history::TransitionType::kFormSubmit:
      return ui::PAGE_TRANSITION_FORM_SUBMIT;
    case extensions::api::history::TransitionType::kReload:
      return ui::PAGE_TRANSITION_RELOAD;
    case extensions::api::history::TransitionType::kKeyword:
      return ui::PAGE_TRANSITION_KEYWORD;
    case extensions::api::history::TransitionType::kKeywordGenerated:
...
bool GetTabById(int tab_id,
  ...
  // Vivaldi
  content::WebContents * contents_tmp = nullptr;
  if (!contents_out) {
    // But we need contents_out!
    contents_out = &contents_tmp;
  }

  int orig_tab_index = index_out ? *index_out : 0;
  WindowController* orig_window = window_out ? *window_out : nullptr;
  // End Vivaldi

  if (ExtensionTabUtil::GetTabById(tab_id, context, include_incognito,
                                   window_out, contents_out, index_out)) {
    if (::vivaldi::CanTouchTheTab(extension, *contents_out)) {
    return true;
    } else { // Vivaldi
      // Keep the original values unchanged. (This may not be necesary.)
      if (index_out)
        *index_out = orig_tab_index;
      if (window_out)
        *window_out = orig_window;
    } // End Vivaldi
  }
...
ExtensionFunction::ResponseAction WindowsCreateFunction::Run() {
  ...
  if (extension() && is_vivaldi && ::vivaldi::IsVivaldiApp(extension_id()) &&
      ...
      incognito_result == windows_util::IncognitoResult::kRegular) {
  ...
  if (create_data_ && create_data_->viv_ext_data) {
    create_params.viv_ext_data = *create_data_->viv_ext_data;
  } // End Vivaldi
...
ExtensionFunction::ResponseValue WindowsCreateFunction::OnBrowserWindowCreated(
  ...
  if (extension() && ::vivaldi::IsVivaldiApp(extension()->id())) {
    ...
    // set them.
    transition =
        HistoryExtensionTransitionToUiTransition(create_data_->transition);
    if (create_data_->from_url_field && *create_data_->from_url_field) {
  ...
  auto create_nav_params = [&](const GURL& url, bool is_first_nav) {
    NavigateParams navigate_params(new_window, url, transition /* Vivaldi */);
  ...
  if (new_window->GetType() == BrowserWindowInterface::TYPE_NORMAL ||
    ...
    if (create_data_ && create_data_->tab_id) {
      ...
      // NOTE(ondrej@vivaldi.com): ensure the first tab in the window is not
      // member of any workspace.
      if (::vivaldi::IsVivaldiRunning()) {
        TabStripModel* tab_strip = new_window->GetTabStripModel();
        CHECK(tab_strip);
        for (int i = 0; i < tab_strip->count(); ++i) {
          content::WebContents* contents = tab_strip->GetWebContentsAt(i);
          ::vivaldi::TabExtData::Get(contents)->Remove(
              ::vivaldi::TabExtKey::kWorkspaceId);
        }
      }
...
bool TabsQueryFunction::MatchesTab(::tabs::TabInterface* candidate_tab,
  ...
  if (!::vivaldi::CanTouchTheTab(extension(), web_contents))
    return false;

  if (!MatchesBool(query_info_.panel_or_widget,
                   ::vivaldi::GetVivPanelId(web_contents).has_value())) {
    return false;
  }
  // End Vivaldi
...
ExtensionFunction::ResponseAction TabsCreateFunction::Run() {
  ...
  // Vivaldi, need to store these until the window has been created.
  ignore_link_routing_ = std::move(params->create_properties.ignore_link_routing);
  viv_ext_data_ = std::move(params->create_properties.viv_ext_data);
  transition_ = params->create_properties.transition;
  from_url_field_ = std::move(params->create_properties.from_url_field);
  // End Vivaldi
  ...
  if (::vivaldi::IsVivaldiRunning()) {
    if (::vivaldi::IsVivaldiApp(extension()->id())) {
      positioning_params_ =
          ::vivaldi::GetTabPositioningParamsFromCreateProperties(
              params->create_properties);
    } else {
      positioning_params_.invoked_by =
          ::vivaldi::TabInvokedBy::kChromiumExtension;
    }
  } // End Vivaldi
...
void TabsCreateFunction::OpenTabInBrowser(BrowserWindowInterface& browser,
  ...
  // Vivaldi VB-128442
  options.positioning_params = positioning_params_;
  options.ignore_link_routing = ignore_link_routing_;

  if (extension() && ::vivaldi::IsVivaldiApp(extension()->id())) {
    // Vivaldi-specific properties, no extension are allowed to
    // set them.
    options.viv_ext_data = viv_ext_data_.value_or(std::string());
...
ExtensionFunction::ResponseAction TabsUpdateFunction::Run() {
  ...
  if (::vivaldi::IsVivaldiRunning() &&
      params->update_properties.viv_ext_data.has_value()) {
    ::vivaldi::TabExtData::Get(original_contents)
        ->Merge(params->update_properties.viv_ext_data.value());
  }  // End Vivaldi
...
bool TabsRemoveFunction::RemoveTab(int tab_id, std::string* error) {
  ...
  if (!is_loaded)
    window->GetBrowser()->DoCloseContents(contents);
  return true;
}
...
ExtensionFunction::ResponseAction TabsDiscardFunction::Run() {
  ...
  if (params->tab_id) {
    ...
    // NOTE(ondrej@vivaldi.com): VB-126999 Ensure the object has extData.
    if (::vivaldi::IsVivaldiRunning()) {
      ::vivaldi::TabExtDataImpl::Create(contents);
    }
...
int VivaldiMoveTabToWindow(ExtensionFunction* function,
                           ...
                           bool allow_other_window_types,
                           std::string* error) {
  return MoveTabToWindow(function, tab_id, target_browser, new_index,
                         allow_other_window_types, error);
}

...
bool GetTabById(int tab_id,
  ...
  if (!contents) {
    // But we need contents!
    contents = &contents_tmp;
  }

  int orig_tab_index = tab_index ? *tab_index : 0;
  WindowController* orig_window = window ? *window : nullptr;

  if (ExtensionTabUtil::GetTabById(tab_id, context, include_incognito, window,
                                   contents, tab_index)) {
    if (::vivaldi::CanTouchTheTab(extension, *contents)) {
    return true;
    } else { // Vivaldi
      // Keep the original values unchanged. (This may not be necesary.)
      if (tab_index)
        *tab_index = orig_tab_index;
      if (window)
        *window = orig_window;
    } // End Vivaldi
  }
...
content::WebContents* GetTabsAPIDefaultWebContents(ExtensionFunction* function,
  ...
  if (tab_id != -1) {
    ...
    GetTabById(tab_id, function->browser_context(),
               ...
               /*tab_index=*/nullptr, error, function->extension());
  } else {
...
int MoveTabToWindow(ExtensionFunction* function,
  ...
  if (!GetTabById(tab_id, function->browser_context(),
                  ...
                  nullptr, &source_index, error, /* Vivaldi */ function->extension()) ||
      !source_window) {
    return -1;
  }
...
#define GetTabById(tab_id, context, include_incognito, browser, contents, tab_index, error_message) \
  GetTabById(tab_id, context, include_incognito, browser, contents, tab_index, error_message, extension())
...
ExtensionFunction::ResponseAction TabsCreateFunction::Run() {
  ...
  return RespondNow([&] {
    ...
    options.ignore_link_routing = params->create_properties.ignore_link_routing;

    if (extension() && ::vivaldi::IsVivaldiApp(extension()->id())) {
      // Vivaldi-specific properties, no extension are allowed to
      // set them.
      options.viv_ext_data = params->create_properties.viv_ext_data;
...
ExtensionFunction::ResponseAction TabsUpdateFunction::Run() {
  ...
  if (params->update_properties.opener_tab_id) {
    ...
    if (!GetTabById(opener_id, browser_context(),
                                      include_incognito_information(), nullptr,
                                      &opener_contents, nullptr, nullptr)) {
  ...
  if (params->update_properties.viv_ext_data.has_value()) {
    web_contents_->SetVivExtData(params->update_properties.viv_ext_data.value());
  }

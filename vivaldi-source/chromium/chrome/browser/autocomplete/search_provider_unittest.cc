...
TEST_F(SearchProviderTest, SendDataToSuggestAtAppropriateTimes) {
  ...
  struct {
    ...
    const bool expect_to_send_to_default_provider;
      ...
      // Vivaldi: We block search suggestions for url's with all schemes.
      {"http://hostname", false},
      {"http://hostname/path", false},
...
// Vivaldi: Added more searches as our default maximum number of matches is 12.
// (see VIVALDI_DESKTOP_DEFAULT_MAX_MATCHES in autocomplete_result.h)
TEST_F(SearchProviderTest, DontCrowdOutSingleWords) {
  ...
  AddSearchToHistory(default_t_url_, u"five searches fff", 1);
  AddSearchToHistory(default_t_url_, u"five searches ggg", 1);
  AddSearchToHistory(default_t_url_, u"five searches hhh", 1);
  AddSearchToHistory(default_t_url_, u"five searches iii", 1);
  AddSearchToHistory(default_t_url_, u"five searches jjj", 1);
  AddSearchToHistory(default_t_url_, u"five searches kkk", 1);
  AddSearchToHistory(default_t_url_, u"five searches lll", 1);
  profile_->BlockUntilHistoryProcessesPendingRequests();

  AutocompleteMatch wyt_match;
  ASSERT_NO_FATAL_FAILURE(QueryForInputAndSetWYTMatch(u"fi", &wyt_match));
  ASSERT_EQ(provider_->provider_max_matches(), provider_->matches().size());
  AutocompleteMatch term_match;
  EXPECT_TRUE(FindMatchWithDestination(term_url, &term_match));
  EXPECT_GT(term_match.relevance, wyt_match.relevance);
  EXPECT_TRUE(term_match.allowed_to_be_default_match);
...
TEST_F(SearchProviderTest, SuggestRelevance) {
  ...
  AutocompleteMatch verbatim, match_a1, match_a2, match_a3;
...
TEST_F(SearchProviderTest, DefaultProviderSuggestRelevanceScoringUrlInput) {
  ...
  for (auto& test_case : kCases) {
    ...
    while (j < matches.size()) {
      if (test_case.output[j].match_type == AutocompleteMatchType::NAVSUGGEST ||

  ...
  if (vivaldi::IsVivaldiRunning()) {
    ...
    if (update_default_location_to_saved &&
      ...
      base::FilePath target_directory =
          selected_file_info.local_path.DirName();
...
#if !BUILDFLAG(IS_ANDROID)
  base::FilePath download_path =
      vivaldi::GetMailAttachmentDownloadPath(download);

  if (!download_path.empty()) {
    download::DetermineLocalPath(download, download_path, std::move(callback));
    return;
  }
#endif // Android
...
bool ChromeDownloadManagerDelegate::DownloadWithExternalDownloadManager(
    ...
    download::DownloadItem* download) {
  DCHECK(download_dialog_bridge_);
  return download_dialog_bridge_->DownloadWithExternalDownloadManager(
      native_window, dialog_type, suggested_path, download);
}
#endif  // BUILDFLAG(IS_ANDROID)

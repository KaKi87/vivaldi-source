...
class Writer : public base::RefCountedThreadSafe<Writer> {
 public:
  Writer(const bookmarks::BookmarkModel* model,
    ...
    local_bookmarks_ =
        codec.Encode(model->bookmark_bar_node(), model->other_node(),
                     model->mobile_node(), model->trash_node(), /*sync_metadata_str=*/std::string());
    ...
    if (model->account_bookmark_bar_node()) {
      ...
      account_bookmarks_ = codec.Encode(
          ...
          model->account_mobile_node(), model->trash_node(), /*sync_metadata_str=*/std::string());
    } else {
      CHECK(!model->account_other_node());
      CHECK(!model->account_mobile_node());
    }
  ...
  bookmark_html_writer::Result DoWrite() {
    ...
    // VIvaldi
    base::DictValue* trash_folder_value = local_permanent_folders->FindDict(
        BookmarkCodec::kTrashBookmarkFolderNameKey);
    CHECK(trash_folder_value);
    // End Vivaldi
    ...
    base::DictValue* account_trash_folder_value = nullptr; // Vivaldi
    ...
    if (account_permanent_folders) {
      ...
      // Vivaldi
      account_trash_folder_value = account_permanent_folders->FindDict(
          BookmarkCodec::kTrashBookmarkFolderNameKey);
      CHECK(account_trash_folder_value);
      // End Vivaldi
    }
 ...
 private:
  ...
  [[nodiscard]] bool WriteFolderStart(const base::DictValue& value,
    ...
    switch (folder_type) {
      ...
      case BookmarkNode::TRASH: // Vivaldi
        NOTREACHED();
    }

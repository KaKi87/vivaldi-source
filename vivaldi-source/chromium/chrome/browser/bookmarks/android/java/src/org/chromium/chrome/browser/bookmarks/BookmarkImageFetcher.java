...
public class BookmarkImageFetcher {
    ...
    // Vivaldi
    public void requestIconFromPath(String imagePath, int size, Callback<Drawable> callback) {
        new AsyncTask<Bitmap>() {
            ...
            protected final Bitmap doInBackground() {
                ...
                try {
                    ...
                    Log.i("TAG", "File not found " + imagePath);
                    return null;
                } finally {
            ...
            protected final void onPostExecute(Bitmap result) {
                if (result != null && callback != null)
                    callback.onResult(new BitmapDrawable(mContext.getResources(),
                            Bitmap.createScaledBitmap(result, size, size, false)));
            }
        }.executeOnExecutor(AsyncTask.THREAD_POOL_EXECUTOR);
    }

    public void fetchFaviconForBookmark(
            BookmarkItem item, String imagePath, Callback<Drawable> callback) {
        int faviconDisplaySize = BookmarkViewUtils.getFaviconDisplaySize(mContext.getResources());
        if (imagePath != null) {

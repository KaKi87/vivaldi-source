...
public interface BookmarkManagerOpener {
    ...
    // Vivaldi
    /**
     * Starts a {@link VivaldiBookmarkEditActivity} for the given {@link BookmarkId}.
     *
     * @param activity A context to start the manager with, if null then the application context
     *     will be used instead.
     * @param profile The profile associated with the bookmarks.
     * @param bookmarkId The bookmark to open.
     */
    void startVivaldiEditFolderActivity(Context context, Profile profile, BookmarkId bookmarkId);

    /**
     * Starts an {@link VivaldiBookmarkFolderSelectActivity} for the given {@link BookmarkId}s.
     *
     * @param context A context to start the manager with, if null then the application context will
     *     be used instead.
     * @param bookmarkIds The bookmarks that are being moved via the selctor.
     */
    void startVivaldiFolderSelectActivity(Context context, BookmarkId... bookmarks);

    void closePanel(Context context);
    boolean isPanelOpen(Context context);
    void vivaldiAddToReadingList(Context context);
    boolean getAddToReadingListButtonVisibility(Context context);
    // End Vivaldi
}
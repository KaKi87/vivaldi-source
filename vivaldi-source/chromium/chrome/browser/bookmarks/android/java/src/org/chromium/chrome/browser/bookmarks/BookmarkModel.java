...
public class BookmarkModel extends BookmarkBridge {
    ...
    public void deleteBookmarks(BookmarkId... bookmarks) {
        ...
        if (BuildConfig.IS_VIVALDI && !isReadingListItem) {
    ...
    public static void deleteThumbnailFiles(List<String> files) {
        for (String file : files) {
            deleteFileAsync(file);
        }
    }

    private static void deleteFileAsync(final String file) {
        new AsyncTask<Boolean>() {
            @Override
            protected Boolean doInBackground() {
                File stateFile = new File(getThumbnailFolderPath(), file + ".jpg");
                if (stateFile.exists()) {

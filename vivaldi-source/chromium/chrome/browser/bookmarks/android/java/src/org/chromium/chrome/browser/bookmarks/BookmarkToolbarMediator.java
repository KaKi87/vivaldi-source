...
class BookmarkToolbarMediator
    ...
    boolean onMenuIdClick(@IdRes int id) {
        ...
        if (id == R.id.create_new_folder_menu_id) {
            ...
            if (BuildConfig.IS_VIVALDI) {
                mBookmarkManagerOpener.closePanel(mContext);
            }
    ...
    public void onFolderStateSet(BookmarkId folder) {
        ...
        if (BuildConfig.IS_VIVALDI) {
            mModel.set(BookmarkToolbarProperties.CLOSE_BUTTON_VISIBLE,
                    mBookmarkManagerOpener.isPanelOpen(mContext));
            mModel.set(BookmarkToolbarProperties.ADD_TO_READING_LIST_BUTTON_VISIBLE,
                    isReadingListFolder && enableAddToReadingListMenu);
    ...
    // Vivaldi
    private static void writeBookmarkSortOrderPref(BookmarkDelegate.SortOrder sorting) {
        ChromeSharedPreferences.getInstance().writeString(PREF_BOOKMARKS_SORT_ORDER, sorting.name());
    }

    public static BookmarkDelegate.SortOrder getSortOrder() {
        return BookmarkDelegate.SortOrder.valueOf(

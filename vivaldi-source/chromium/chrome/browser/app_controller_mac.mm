...
- (ConfirmQuitResult)confirmQuitIfNeeded {
  ...
  if (!event || CommandForKeyEvent(event).chrome_command != IDC_VIV_EXIT) {
    // VB-131099: Don't return notPrompted for viv exit shortcut
...
- (BOOL)validateUserInterfaceItem:(id<NSValidatedUserInterfaceItem>)item {
  ...
  if (action == @selector(commandDispatch:) ||
    ...
    if (_menuState &&  // NULL in tests.
      ...
      if (vivaldi::IsVivaldiRunning()) {
        bool hasWindow = !![NSApp keyWindow];
        bool enabled;
...
#ifndef VIVALDI_SPARKLE_DISABLED
  [[_sparkle_updater_controller updater] checkForUpdateInformation];
#endif
}
...
- (void)commandDispatch:(id)sender {
  ...
  if (vivaldi::IsVivaldiRunning()) {
    BrowserWindowInterface* browser_window_interface =
        GlobalBrowserCollection::GetInstance()->FindBrowserWithWindow(
            gfx::NativeWindow([NSApp mainWindow]));
    if (browser_window_interface) {
      lastProfile = browser_window_interface->GetProfile();
    }
  }
  ...
  if (vivaldi::IsVivaldiRunning()) {
    ...
    if (lastProfile) {
      ...
      browser_window_interface =
          ProfileBrowserCollection::GetForProfile(wanted_profile)
              ->GetLastActiveBrowser();
...
- (void)executeCommand:(id)sender withProfile:(Profile*)profile {
  ...
  if (vivaldi::IsVivaldiRunning()) {
    ...
    Profile* wanted_profile = profile->IsGuestSession()
        ...
        if (!parameter.empty()) {
          if (BrowserWindowInterface* browser = ActivateBrowser(profile)) {
            ...
            extensions::MenubarAPI::HandleActionById(
              browser->GetProfile(), browser->GetSessionID().id(), tag, parameter);
          }
          break;
        }
      }
...
- (void)swipeWithEvent:(NSEvent*)event {
  ...
  if (command != 0) {
    ...
    if (browser_window_interface) {
      ...
      if (chrome::IsCommandEnabled(browser_window_interface, command)) {
        chrome::ExecuteCommand(browser_window_interface, command);
      }
    }
  }
}
...
void OpenStartupTabsInBrowserWithProfile(const StartupTabs& tabs,
  ...
  if (vivaldi::IsVivaldiRunning()) {
    for (auto& tab : tabs) {
      if (tab.url.scheme() == "mailto" &&
          vivaldi::HandleProtocol(profile, tab.url)) {
        return;
      }
    }
  } // end vivaldi

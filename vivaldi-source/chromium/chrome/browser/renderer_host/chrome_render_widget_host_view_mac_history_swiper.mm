...
- (void)onOverscrolled:(const ui::DidOverscrollParams&)params {
  ...
  if (vivaldi::IsVivaldiRunning()) {
    // VB-119412: One finger history swipe does not work with magic mouse
    // This value is only checked in handleMagicMouseWheelEvent
    _magicMouseOverscrolled = YES;
  } // End Vivaldi
}
...
- (BOOL)handleMagicMouseWheelEvent:(NSEvent*)theEvent {
  ...
  if (vivaldi::IsVivaldiRunning() && !_magicMouseOverscrolled) {
    return NO;
  } // End Vivaldi

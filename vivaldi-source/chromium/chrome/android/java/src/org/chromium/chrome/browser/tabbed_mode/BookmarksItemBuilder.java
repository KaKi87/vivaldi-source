...
public class BookmarksItemBuilder implements Destroyable {
    ...
    public ListItem buildReadingListItem(@Nullable Tab currentTab) { // Vivaldi
        if (BuildConfig.IS_VIVALDI) {
            return new ListItem(AppMenuHandler.AppMenuItemType.STANDARD,
                    AppMenuItemUtils.buildModelForStandardMenuItem(mContext, mAppMenuItemTheme,
                            R.id.menu_reading_list_id, R.string.menu_reading_list,
                            R.drawable.vivaldi_menu_reading_list_24dp, true));
        }
        List<ListItem> submenuItems = new ArrayList<>();
        submenuItems.add(

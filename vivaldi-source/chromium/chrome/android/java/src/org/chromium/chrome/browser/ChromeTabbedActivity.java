...
public class ChromeTabbedActivity extends ChromeActivity {
    ...
    // We can have more instances of |ChromeTabbedActivity|, this flag makes sure we only show the
    // lock screen once.
    private static boolean sShouldShowLockScreen = true;
    // NOTE(jarle@vivaldi.com): disable (AAOS) migration prompt for now.
    ...
    protected void onPreCreate() {
        ...
        mPrefslistener = (sharedPrefs, key) -> {
            if (TextUtils.equals(key, VivaldiPreferences.SHOW_TAB_STRIP)
                    ...
                    || TextUtils.equals(key, VivaldiPreferences.SHOW_TAB_SWITCHER_LIST)) {
    ...
    public void finishNativeInitialization() {
        ...
        if (ChromeApplicationImpl.isVivaldi()) {
            VivaldiUtils.maybeShowPartnerSEPrompt(this);
    ...
    public void onResumeWithNative() {
        ...
        if (VivaldiUtils.isVivaldiScreenLockEnabled()) {
            ScreenLock.getInstance().init(this, getSupportFragmentManager());
        }
    ...
    public void performPreInflationStartup() {
        ...
        // Vivaldi
        if (VivaldiUtils.isVivaldiScreenLockEnabled()) {
            // Note(david@vivaldi.com): Init the authentication for the Android pin.
            ScreenLock.getInstance().init(this, getSupportFragmentManager());
            if (ScreenLock.getInstance().isReauthenticateAllowed()
                    && sShouldShowLockScreen) {
                if (!ScreenLock.getInstance().isScreenLockSetUp()) {
                    ScreenLock.getInstance().showScreenLockToast();
                    exitApplication();
                    return;
                }
                ScreenLock.getInstance().reauthenticate(authSucceeded -> {
                    if (!authSucceeded) exitApplication();
                });
                sShouldShowLockScreen = false;
            }
        }
    ...
    public boolean onMenuOrKeyboardAction(
        ...
        if (id == R.id.new_tab_menu_id) {
            ...
            if (currentTab == null || BuildConfig.IS_VIVALDI || (ChromeFeatureList.sAndroidNativePagesInNewTab.isEnabled()
            ...
            BookmarkBarUtils.toggleSettingEnabled(currentTab.getProfile());
        // Vivaldi specific:
            ...
            getCompositorViewHolderSupplier().get().hideKeyboard(() -> {
                mBookmarkManagerOpenerSupplier.get().showBookmarkManager(
                        ChromeTabbedActivity.this, profile);
            });
            if (currentTabIsNtp) {
                NewTabPageUma.recordAction(NewTabPageUma.ACTION_OPENED_BOOKMARKS_MANAGER);
            }
            ...
            mPageActionsSheet = new PageActionsSheet(this, mContentContainer);
            ...
            Tab newTab = getTabCreator(getCurrentTabModel().isIncognito())
                    .createNewTab(params, TabLaunchType.FROM_CHROME_UI, null);
            if (newTab != null)
                newTab.changeWebContents(currentTab.getWebContents(), false, false);
        ...
        } else if (id == R.id.vivaldi_create_new_tab_stack) { // Vivaldi
            LoadUrlParams params = new LoadUrlParams(UrlConstants.NTP_URL);
            TabGroupModelFilter filter = mTabModelSelector.getTabGroupModelFilterProvider()
                                                 .getCurrentTabGroupModelFilter();
            int relatedTabSize = filter.getRelatedTabList(currentTab.getId()).size();
            ChromeTabCreator tabCreator = getTabCreator(getCurrentTabModel().isIncognito());
            // Note(david@vivaldi.com): We create a new tab and merge it with the current selected
            // one. If the current select one has more then one tab relations we simply merge just
            // two empty tabs.
            Tab newTab = tabCreator.createNewTab(params, TabLaunchType.FROM_CHROME_UI, null);
            if (relatedTabSize == 1)
                filter.mergeTabsToGroup(currentTab.getId(), newTab.getId());
            else {
                Tab secondTab = tabCreator.createNewTab(params, TabLaunchType.FROM_CHROME_UI, null);
                filter.mergeTabsToGroup(newTab.getId(), secondTab.getId());
            }
    ...
    public void onResume() {
        if (BuildConfig.IS_OEM_AUTOMOTIVE_BUILD) {
            ...
            if (VivaldiUtils.driverDistractionHandlingEnabled() &&
                    mDriverDistracted && !CarDataProvider.getInstance().isUxRestrictionsActive()) {
        ...
        if (sShouldShowMigrationPrompt &&
                (BuildConfig.IS_OEM_POLESTAR_BUILD || BuildConfig.IS_OEM_RENAULT_BUILD)) {
            VivaldiUtils.showMigrationPopup(this);
    ...
    private void initializeDriverDistractionHandler() {
        ...
        mCarDataObserver = new CarDataProvider.Observer() {
            ...
            public void onDriverDistracted(boolean isDistracted) {
                ...
                if (isDistracted) {
                    Log.i(TAG, "#onDriverDistracted: true, suspend media player(s), mute audio");
                    webContents.suspendAllMediaPlayers();
                    webContents.setAudioMuted(true);
                    mDriverDistracted = true;
                }
            }
        };
    ...
    private void openAddPageToSubMenu() {
        ...
        popupSubMenu.setOnMenuItemClickListener(menuItem -> {
            ...
            if (menuItem.getItemId() == R.id.add_to_bookmarks) {
                ...
                modelLoadedRunnable = () -> {
                    if (url != null) {
                        vivaldiAddToReadingList(url, currentTab.getTitle());
                    }
                };
            ...
            if (mBookmarkModelSupplier.hasValue() && modelLoadedRunnable != null)
                mBookmarkModelSupplier.get().finishLoadingBookmarkModel(modelLoadedRunnable);
            return true;
        });

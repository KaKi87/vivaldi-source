...
public class MainSettings extends ChromeBaseSettingsFragment
    ...
    public void onViewCreated(View view, @Nullable Bundle savedInstanceState) {
        ...
        PaddedItemDecorationWithDivider itemDecoration =
                new PaddedItemDecorationWithDivider(zeroSupplier);
        Drawable dividerDrawable =
                AppCompatResources.getDrawable(
                        Objects.requireNonNull(getContext()), R.drawable.settings_divider);
        assert dividerDrawable != null;
        itemDecoration.setDividerWithPadding(dividerDrawable, zeroSupplier, zeroSupplier);
    ...
    private void createPreferences() {
        ...
        if (ChromeApplicationImpl.isVivaldi()) {
            if (BuildConfig.IS_OEM_AUTOMOTIVE_BUILD) {
                if (VivaldiUtils.driverDistractionHandlingEnabled(getContext())) {
        ...
        if (ChromeApplicationImpl.isVivaldi()) {
            ...
            if (reverseSearchSuggestion != null)
                reverseSearchSuggestion.setOnPreferenceChangeListener((preference, o) -> {
                    ...
                    if (activity instanceof VivaldiRelaunchDialogHost) {
                        ((VivaldiRelaunchDialogHost) activity).showRelaunchDialog(null);
                    }
                    return true;
                });
            ...
            Preference pref = findPreference(VivaldiPreferences.DEFAULT_BROWSER_PROMO);
            if (pref != null) {
                if (VivaldiDefaultBrowserUtils.checkIfVivaldiDefaultBrowser(getContext())
                    ...
                    removePreferenceIfPresent(VivaldiPreferences.DEFAULT_BROWSER_PROMO);
            }
    ...
    private void updateAutofillPreferences() {
        ...
        // Vivaldi AUTO-339: Remove autofill service links on Mercedes (leads to system UI).
        if (BuildConfig.IS_OEM_MERCEDES_BUILD) {

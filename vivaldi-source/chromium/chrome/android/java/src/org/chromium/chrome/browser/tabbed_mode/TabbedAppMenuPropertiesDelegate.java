...
public class TabbedAppMenuPropertiesDelegate extends AppMenuPropertiesDelegateImpl {
    ...
    // Vivaldi
    private @Nullable AppMenuIconRow mVivaldiMenuIconRow;
    ...
    public boolean shouldShowIconRow() {
        ...
        if (mIsTablet) {
            if (BuildConfig.IS_VIVALDI) return false; // Vivaldi VAB-12813
    ...
    private MVCListAdapter.ListItem buildDuplicateTabItem(Tab currentTab) {
        ...
        return new MVCListAdapter.ListItem(AppMenuHandler.AppMenuItemType.STANDARD,
                AppMenuItemUtils.buildModelForStandardMenuItem(mContext, getAppMenuItemTheme(),
                        R.id.vivaldi_clone_tab_menu_id, R.string.clone_tab,
    ...
    private MVCListAdapter.ListItem buildAddPageToItem(Tab currentTab) {
        ...
        return new MVCListAdapter.ListItem(AppMenuHandler.AppMenuItemType.STANDARD,
                AppMenuItemUtils.buildModelForStandardMenuItem(mContext, getAppMenuItemTheme(),
                        R.id.add_page_to_menu_id, R.string.menu_add_to_menu,
                        shouldShowIconBeforeItem() ? R.drawable.plus : 0, isMenuIconAtStart()));
    }
    ...
    private MVCListAdapter.ListItem buildSetAsDefaultBrowserItem() {
        return new MVCListAdapter.ListItem(AppMenuHandler.AppMenuItemType.STANDARD,
                AppMenuItemUtils.buildModelForStandardMenuItem(mContext, getAppMenuItemTheme(),
                        R.id.set_default_browser_menu_id, R.string.menu_set_default_browser,
    ...
    private MVCListAdapter.ListItem buildNotesItem() {
        return new MVCListAdapter.ListItem(AppMenuHandler.AppMenuItemType.STANDARD,
                AppMenuItemUtils.buildModelForStandardMenuItem(mContext, getAppMenuItemTheme(),
                        R.id.vivaldi_notes_menu_id, R.string.menu_notes,
    ...
    private MVCListAdapter.ListItem buildCapturePageItem(Tab currentTab) {
        ...
        return new MVCListAdapter.ListItem(AppMenuHandler.AppMenuItemType.STANDARD,
                AppMenuItemUtils.buildModelForStandardMenuItem(mContext, getAppMenuItemTheme(),
                        R.id.capture_page_id, R.string.menu_capture_page,
    ...
    private MVCListAdapter.ListItem buildPageActionsItem(Tab currentTab) {
        ...
        return new MVCListAdapter.ListItem(AppMenuHandler.AppMenuItemType.STANDARD,
                AppMenuItemUtils.buildModelForStandardMenuItem(mContext, getAppMenuItemTheme(),
                        R.id.page_actions_id, R.string.menu_page_actions,
    ...
    private MVCListAdapter.ListItem buildVivaldiGameItem() {
        return new MVCListAdapter.ListItem(AppMenuHandler.AppMenuItemType.STANDARD,
                AppMenuItemUtils.buildModelForStandardMenuItem(mContext, getAppMenuItemTheme(),
                        R.id.launch_game_menu_id, R.string.menu_launch_game,
    ...
    private MVCListAdapter.ListItem buildExitItem() {
        return new MVCListAdapter.ListItem(AppMenuHandler.AppMenuItemType.STANDARD,
                AppMenuItemUtils.buildModelForStandardMenuItem(mContext, getAppMenuItemTheme(),
                        R.id.vivaldi_exit_id, R.string.menu_vivaldi_exit,
    ...
    private void populateVivaldiPageModeMenu(MVCListAdapter.ModelList modelList) {
        ...
        if (shouldShowIconRow() && !isToolbarAtBottom) {
            List<PropertyModel> iconModels = new ArrayList<>();
        ...
        // Download Page
        if (shouldShowDownloadPageMenuItem(currentTab)) {
            MVCListAdapter.ListItem downloadPageItem =
                    mSaveAndShareItemBuilder.buildDownloadPageItem(true);
        ...
        // New Window
        if (shouldShowNewWindow()) modelList.add(buildNewWindowItem(isIncognitoForced));
        ...
        // New Tab
        MVCListAdapter.ListItem newTabItem = buildNewTabItem(isIncognitoForced);
        ...
        // Add to Group
        if (mTabGroupItemBuilder.shouldShowAddToGroup(currentTab))
            modelList.add(mTabGroupItemBuilder.buildAddToGroupItem(currentTab, true));

        // Recent Tabs
        if (mHistoryItemBuilder.shouldShowRecentTabsItem()) {
            MVCListAdapter.ListItem recentTabsItem = mHistoryItemBuilder.buildRecentTabsItem(true);
        ...
        // Bookmarks
        MVCListAdapter.ListItem bookmarksItem = mBookmarksItemBuilder.buildBookmarksItem(true);
        replaceMenuIcon(bookmarksItem, R.drawable.main_menu_bookmarks_24dp);
        modelList.add(bookmarksItem);

        // History
        MVCListAdapter.ListItem historyItem = mHistoryItemBuilder.buildHistoryItem(true);
        ...
        if (canShowMenuItem(R.id.menu_reading_list_id)) {
            modelList.add(mBookmarksItemBuilder.buildReadingListItem(currentTab));
        }
        ...
        if (shouldShowPageZoomItem(currentTab) && canShowMenuItem(R.id.page_zoom_id)) {
            ...
            replaceMenuIcon(pageZoomItem, R.drawable.magnify_24dp); // VAB-12468
    ...
    private MVCListAdapter.ListItem buildCloseOtherTabsItem() {
        final PropertyModel model = AppMenuItemUtils.buildModelForStandardMenuItem(mContext,
                getAppMenuItemTheme(), R.id.vivaldi_close_other_tabs,
                R.string.tabs_menu_close_other_tabs,
                // TODO(jarle@vivaldi.com): use Vivaldi icon
                shouldShowIconBeforeItem() ? R.drawable.btn_close_white : 0, isMenuIconAtStart());
        if (isIncognitoShowing()) {
            model.set(
    ...
    public void loadingStateChanged(boolean isLoading) {
        ...
        if (mVivaldiMenuIconRow != null)
            mVivaldiMenuIconRow.loadingStateChanged(isLoading);
    }

    // Vivaldi VAB-13115
    @Override
    public @Nullable View buildFooterView(AppMenuHandler appMenuHandler) {
        // Vivaldi - We use the footer to display our menu bar when address bar is at the bottom.
        boolean isToolbarAtBottom = VivaldiPreferences.getSharedPreferencesManager().readBoolean(
                VivaldiPreferences.ADDRESS_BAR_TO_BOTTOM, false);
        Tab currentTab = mActivityTabProvider.get();
        if (BuildConfig.IS_VIVALDI && currentTab != null && isToolbarAtBottom
                && shouldShowIconRow()) {
            mVivaldiMenuIconRow = (AppMenuIconRow) LayoutInflater.from(mContext).inflate(
                    R.layout.icon_row_menu_layout, null);
            mVivaldiMenuIconRow.initialize(
                    appMenuHandler, mBookmarkModelSupplier, currentTab, mAppMenuDelegate);
            return mVivaldiMenuIconRow;
        }
        return null;
    }
    // End Vivaldi

...
//import org.chromium.chrome.browser.glic.GlicButtonDelegate; Vivaldi
...
//import org.chromium.chrome.browser.ui.actions.glic.GlicActionCoordinator; Vivaldi
...
import org.vivaldi.browser.prompts.VivaldiOnMobilePromptHandler;
import org.vivaldi.browser.toolbar.VivaldiMiniOriginBarController;
...
public class ToolbarManager
    ...
    // Vivaldi
    private @Nullable OnClickListener mTabSwitcherClickHandler;
    private @Nullable BottomToolbarCoordinator mBottomToolbarCoordinator;
    private static boolean shouldCheckAddWidgetPrompt = true;
    private static boolean shouldCheckVivaldiOnMobilePrompt = true;
    private ImageButton mReaderViewButton;
    private @Nullable ReaderViewPreferenceCoordinator mReaderViewPreferenceCoordinator;
    // Vivaldi End
    ...
    public ToolbarManager(
            ...
            GlicButtonDelegate toggleGlicCallback Vivaldi*/) {
        ...
        // Vivaldi
        mControlContainer.setIsInOverviewModeSupplier(isInOverviewModeSupplier);
        ...
        mComponentCallbacks =
                new ComponentCallbacks() {
                    ...
                    public void onConfigurationChanged(Configuration configuration) {
                        ...
                        // Note(david@vivaldi.com): Let the |VivaldiTopToolbarCoordinator| know that the
                        // orientation has changed.
                        if (ChromeApplicationImpl.isVivaldi()
                                && mToolbar instanceof VivaldiTopToolbarCoordinator)
                            ((VivaldiTopToolbarCoordinator) mToolbar).onOrientationChange(newOrientation);
                    }
            ...
            LocationBarCoordinator locationBarCoordinator =
                    new LocationBarCoordinator(
                            ...
                            (url) -> {
                                ...
                                return bridge != null && bridge.isBookmarked(url)                                    // Vivaldi ref. VAB-9205 / VAB-10427
                                    && bridge.isBookmarkModelLoaded()
                                    && !bridge.isInsideTrashFolder(
                                          bridge.getMostRecentlyAddedUserBookmarkIdForUrl(url));
                            },
                            () ->
        ...
        mActivityTabTabObserver =
                new ActivityTabProvider.ActivityTabTabObserver(mActivityTabProvider) {
                    ...
                    public void onObservingDifferentTab(@Nullable Tab tab) {
                        ...
                        // Vivaldi
                        updateReaderViewButtonVisibility();
                    }
                    ...
                    public void onUrlUpdated(Tab tab) {
                        ...
                        // Vivaldi
                        mToolbarLayout.updateShieldButtonState(tab.getUrl());
                    ...
                    public void onLoadStopped(Tab tab, boolean toDifferentDocument) {
                        ...
                        // Vivaldi - Add search widget prompt
                        if (shouldCheckAddWidgetPrompt
                            ...
                            if (mActivity instanceof ChromeTabbedActivity
                                ...
                                shouldCheckAddWidgetPrompt = false;
                                new AddWidgetPromptHandler(mActivity);
                            }
                        }

                        // Vivaldi (gabriel@vivaldi.com) show prompt for automotive users if the
                        // requirements have been met.
                        if (shouldCheckVivaldiOnMobilePrompt
                                && VivaldiMainPromptHandler.canShowPrompt()) {
                            if (mActivity instanceof ChromeTabbedActivity
                                    && BuildConfig.IS_OEM_AUTOMOTIVE_BUILD) {
                                shouldCheckVivaldiOnMobilePrompt = false;
                                new VivaldiOnMobilePromptHandler(mActivity);
                            }
                        }

                        // Vivaldi - Update reader view button after a delay of 1 second to get the
                        // correct status after the tab is loaded.
                        new Handler().postDelayed(() -> updateReaderViewButtonVisibility(), 1000);
                        // Vivaldi End
                    }
        ...
        mBrowserControlsObserver =
                new BrowserControlsStateProvider.Observer() {
                    ...
                    // Vivaldi
                    private static boolean isReaderviewSettingsHidden;
                    ...
                    public void onControlsOffsetChanged(
                        ...
                        // Note(david@vivaldi.com): The margins are handled in
                        ...
                        // Vivaldi Note(VAB-11987): Hide the Readerview settings button(if visible)
                        // when the toolbar disappears. i.e. requestNewFrame == false
                        if (!canShowReaderViewSettingsButton()) return;
                        if (requestNewFrame) {
                            // Show the settings button since the toolbars are visible again on the
                            // screen.
                            setReaderViewSettingsButtonVisibility(true);
                            isReaderviewSettingsHidden = false;
                        } else if (!isReaderviewSettingsHidden) {
                            // Hide the settings button since the toolbars scrolled off from on the
                            // screen.
                            setReaderViewSettingsButtonVisibility(false);
                            isReaderviewSettingsHidden = true;
                        } // End VAB-11987
                    }
        ...
        // Vivaldi - Reader view button in the url bar
        View urlBar = (mActivity == null) ? null : mActivity.findViewById(R.id.location_bar);
        if (urlBar != null) mReaderViewButton = urlBar.findViewById(R.id.reader_view_button);
        if (mReaderViewButton != null) {
            mReaderViewButton.setOnClickListener(v -> {
                ...
                if (DomDistillerUrlUtils.isDistilledPage(currentTab.getUrl())
                    ...
                    if (currentTab.getWebContents() != null)
                        DomDistillerTabUtils.distillCurrentPageAndViewIfSuccessful(
                                currentTab.getWebContents(), (success) -> {});
                }
                updateReaderViewButtonVisibility();
            });
        }
    ...
    private boolean shouldSuppressToolbarLongPress() {
        if (ChromeApplicationImpl.isVivaldi()) { // Vivaldi VAB-13103
    ...
    private void initializeToolbarPositionController() {
        ...
        // Vivaldi VAB-12859: use subclass for bottom-toolbar-specific behavior.
        if (BuildConfig.IS_VIVALDI) {
            VivaldiMiniOriginBarController vivaldiController = new VivaldiMiniOriginBarController(
                    mLocationBar,
            ...
            vivaldiController.setMiniBarActiveCallback(active -> {
                if (mLocationBar instanceof LocationBarCoordinator locationBarCoordinator) {
                    locationBarCoordinator.setVivaldiMiniBarActive(active);
                }
            });
            manualFillingComponentSupplier.addSyncObserverAndCallIfNonNull(manualFillingComponent ->
                    manualFillingComponent.setVivaldiMiniBarCallbacks(
                            vivaldiController::isMiniBarActive,
                            vivaldiController::onManualFillingHeight));
            mMiniOriginBarController = vivaldiController;
        } else { // Vivaldi
    ...
    private TopToolbarCoordinator createTopToolbarCoordinator(
            ...
            SnackbarManager snackbarManager) {
        // clang-format off
        VivaldiTopToolbarCoordinator toolbar =
                new VivaldiTopToolbarCoordinator(
                        mActivity, // Vivaldi
                        controlContainer,
                        mToolbarLayout,
                        mLocationBarModel,
    ...
    private void enableTabGroupUiControls() {
        ...
        // |ThemeColorProvider|.
        // Ref. VAB-13120: |mAdjustedToolbarThemeColorProvider| is nullable, so fall back to the
        // bottom UI provider to avoid a NullPointerException when the address bar is at the bottom.
        ThemeColorProvider tabGroupUiThemeProvider;
        if (VivaldiUtils.isTopToolbarOn()) {
            tabGroupUiThemeProvider = mAppThemeColorProvider;
        } else if (mAdjustedToolbarThemeColorProvider != null) {
            tabGroupUiThemeProvider = mAdjustedToolbarThemeColorProvider;
        } else {
            tabGroupUiThemeProvider = mBottomUiThemeColorProvider;
        }
        ...
        mTabGroupUiOneshotSupplier =
                new TabGroupUiOneshotSupplier(
                        ...
                        mTabModelSelector,
                        mActivity,
                        tabGroupUiContainer.findViewById(R.id.tab_group_ui_toolbar_view), // Vivaldi
                        mBrowserControlsSizer,
                        mScrimManager,
                        mOmniboxFocusStateSupplier,
                        mBottomSheetController,
                        ...
                        mLayoutStateProviderSupplier,
                        mModalDialogManagerSupplier.get(),
                        tabGroupUiThemeProvider, // Vivaldi
                        mUndoBarThrottle,
        ...
        // Note(david@vivaldi.com): Create all necessary handlers for the bottom toolbar.
        if (ChromeApplicationImpl.isVivaldi()) {
            mBottomToolbarCoordinator = new BottomToolbarCoordinator((ChromeActivity) mActivity,
                    mActivity.findViewById(
                            org.chromium.chrome.browser.toolbar.R.id.bottom_toolbar_stub),
                    mAppThemeColorProvider, mToolbarTabController::openHomepage,
                    mMenuButtonCoordinator.getMenuButtonHelperSupplier(), mUserEducationHelper,
                    mPromoShownOneshotSupplier, mLayoutStateProviderSupplier,
                    mActivityTabProvider.asObservable(), mTabModelSelectorSupplier,
                    mIncognitoStateProvider);
            mBottomToolbarCoordinator.postInit(mTabSwitcherClickHandler);
            updateButtonStatus();
        }
        // End Vivaldi
    }
    ...
    public void initializeWithNative(
        ...
        // Vivaldi VAB-12859: wire mini bar active state to the tab strip.
        /* TODO(CHR150): Check this
        if (mStripLayoutHelperManager != null
                && mMiniOriginBarController instanceof VivaldiMiniOriginBarController v) {
            mStripLayoutHelperManager.setVivaldiMiniBarActiveSupplier(
                    v.getMiniBarActiveSupplier());
        }*/
        ...
        // Vivaldi
        mTabSwitcherClickHandler = v -> {
            if (openGridTabSwitcherHandler != null) openGridTabSwitcherHandler.run();
        };
        ...
        // Vivaldi: skip for all custom-tab-based activities (CustomTabActivity, WebappActivity,
        // WebApkActivity, etc.); enableBottomControls() asserts non-null undoBarThrottle
        // which those activities pass as null to initializeWithNative.
        if (ChromeApplicationImpl.isVivaldi()
                && !(mActivity instanceof BaseCustomTabActivity)) // Vivaldi VAB-12792 / VAB-12802
    ...
    public void onUrlFocusChange(boolean hasFocus) {
        ...
        // Note(david@vivaldi.com): Apply the correct top margin for the location bar scrim view.
        mLocationBarFocusHandler.setTopMargin(VivaldiUtils.isTopToolbarOn()
        ...
        // Vivaldi - Reader view
        if (hasFocus) {
            if (mReaderViewButton != null) mReaderViewButton.setVisibility(View.GONE);
            if (mReaderViewPreferenceCoordinator != null) mReaderViewPreferenceCoordinator.hide();
        } else updateReaderViewButtonVisibility();
    }
    ...
    public void onThemeColorChanged(int color, boolean shouldAnimate) {
        ...
        if (activity.isTablet()
                || (ChromeSharedPreferences.getInstance().readBoolean(
                        ...
                        && (activity.getActivityType() != ActivityType.WEBAPP)
                        // Temporarily enable VAB-12693 on Sopranos builds only.
                        && !activity.getPackageName().contains("sopranos"))) {
            color = ChromeColors.getDefaultThemeColor(mActivity, mLocationBarModel.isIncognito());
        }
    ...
    private void updateButtonStatus() {
        ...
        // Vivaldi
        boolean toolbarNavButtonsVisible = (mToolbar instanceof VivaldiTopToolbarCoordinator) &&
                ((VivaldiTopToolbarCoordinator) mToolbar).areToolbarNavigationButtonsVisible();
        if (!toolbarNavButtonsVisible && mBottomToolbarCoordinator != null) {
            mBottomToolbarCoordinator.updateBackButtonVisibility(currentTab != null &&
                    currentTab.canGoBack());
            mBottomToolbarCoordinator.updateForwardButtonVisibility(currentTab != null &&
                    currentTab.canGoForward());
            if (currentTab != null) {
                NativePage nativePage = currentTab.getNativePage();
                boolean showSearch = (nativePage instanceof SpeedDialPage
                        || nativePage instanceof IncognitoNewTabPage);
                if (mBottomToolbarCoordinator != null)
                    mBottomToolbarCoordinator.updateCenterButton(showSearch);
            }
        } else {
            if (mBottomToolbarCoordinator != null) {
                mBottomToolbarCoordinator.updateBackButtonVisibility(
                        currentTab != null && currentTab.canGoBack());
                onBackPressStateChanged();
                mBottomToolbarCoordinator.updateForwardButtonVisibility(
    ...
    /** Vivaldi
     * @return The coordinator for the bottom toolbar.
     */
    public @Nullable BottomToolbarCoordinator getBottomToolbarCoordinator() {
        return mBottomToolbarCoordinator;
    }

    // Vivaldi - Reader view
    public void updateReaderViewButtonVisibility() {
        // Update reader view button visibility
        Tab currentTab = mLocationBarModel.getTab();
        if (mReaderViewButton == null || currentTab == null) return;
        mReaderViewButton.setVisibility(canShowReaderViewButton() ? View.VISIBLE : View.GONE);
        if (mReaderViewPreferenceCoordinator == null) return;
        // Update reader view settings button visibility
        if (mReaderViewButton.getVisibility() == View.GONE) {
            mReaderViewPreferenceCoordinator.hide();
            return;
        }
        setReaderViewButtonTint();
        if (canShowReaderViewSettingsButton())
            mReaderViewPreferenceCoordinator.show(currentTab);
        else
            mReaderViewPreferenceCoordinator.hide();
    }

    // Vivaldi
    private boolean canShowReaderViewButton() {
        Tab currentTab = mLocationBarModel.getTab();
        if (mTabModelSelector == null || mReaderViewButton == null || currentTab == null) {
            return false;
        }
        return VivaldiUtils.shouldShowReaderViewButton(currentTab) && !isUrlBarFocused();
    }

    // Vivaldi
    private boolean canShowReaderViewSettingsButton() {
        if (!canShowReaderViewButton() || mLayoutStateProvider == null) return false;
        Tab currentTab = mLocationBarModel.getTab();
    ...
    // Vivaldi
    private void setReaderViewSettingsButtonVisibility(boolean show) {
        Tab currentTab = mLocationBarModel.getTab();
        if (mReaderViewPreferenceCoordinator == null || currentTab == null) return;
        if (show && canShowReaderViewSettingsButton()) {
            mReaderViewPreferenceCoordinator.show(currentTab);
        } else
            mReaderViewPreferenceCoordinator.hide();
    }
    ...
    // Vivaldi
    public void setReaderViewCoordinator(ReaderViewPreferenceCoordinator readerViewCoordinator) {
        mReaderViewPreferenceCoordinator = readerViewCoordinator;
    }

    // Vivaldi
    public boolean isShowingMiniOriginBar() {
        return mMiniOriginBarController != null
                && mMiniOriginBarController.isShowingMiniOriginBar();
    }

    // Vivaldi
    public void onVivaldiHardwareTypingKey() {
        if (mMiniOriginBarController instanceof VivaldiMiniOriginBarController v) {
            v.onHardwareTypingKey();
        }
    }


    // Vivaldi
    private void setReaderViewButtonTint() {
        if (mActivity == null || mActivityTabProvider == null
                || mActivityTabProvider.get() == null) {
            return;
        }
        int brandedColorScheme = OmniboxResourceProvider.getBrandedColorScheme(
                mActivity, mActivityTabProvider.get().isIncognitoBranded(), getPrimaryColor());
        mReaderViewButton.setImageTintList(

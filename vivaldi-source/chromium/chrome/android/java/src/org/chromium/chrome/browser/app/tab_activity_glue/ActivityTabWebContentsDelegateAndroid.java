...
public class ActivityTabWebContentsDelegateAndroid extends TabWebContentsDelegateAndroid {
    ...
    protected boolean isForceDarkWebContentEnabled() {
        ...
        if (BuildConfig.IS_VIVALDI) {
            int websiteThemeAppearance = ChromeSharedPreferences.getInstance().readInt(
                    ThemeSettingsFragment.PREF_WEBSITE_THEME_APPEARANCE);
            boolean systemNightModeOn = SystemNightModeMonitor.getInstance().isSystemNightModeOn();
            switch (websiteThemeAppearance) {
                case ThemeType.DARK -> {
                    return true;
                }
                case ThemeType.SYSTEM_DEFAULT -> {
                    return systemNightModeOn;
                }
                default -> {
                    return false;
                }

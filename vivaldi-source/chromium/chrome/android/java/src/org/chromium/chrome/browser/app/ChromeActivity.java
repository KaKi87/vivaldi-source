...
import org.vivaldi.browser.car.AndroidAutoActivityZoomController;
...
import org.vivaldi.browser.prompts.PrivacyReportNotificationReceiver;
...
public abstract class ChromeActivity extends AsyncInitializationActivity
    ...
    private AndroidAutoActivityZoomController mAndroidAutoActivityZoomController;
        ...
        try (TraceEvent te = TraceEvent.scoped("ChromeActivity.performPostInflationStartup")) {
            ...
            setTabContentManager(
                    new TabContentManager(
                            ...
                            // Vivaldi (gabriel@vivaldi.com) this value determines the display of
                            // thumbnails. We only want to avoid it for low end devices
                            // (ref. VAB-11218)
                            ChromeApplicationImpl.isVivaldi() ? !SysUtils.isLowEndDevice() :
                            ...
                            model.finishLoadingBookmarkModel(() -> {
                                if (model.getUserBookmarkIdForTab(tab) != null) {
                                    ...
                                    if (SpeedDialUtils.thumbnailsEnabled() && item != null
                                        ...
                                        captureThumbnailForSpeedDial(tab, model, item);
                                    }
                                }
                            });
                        } // End Vivaldi
    ...
    public void onStartWithNative() {
        ...
        // Vivaldi (Android Auto).
        if (!isLaunchedFromCarHeadUnit()) return;
        if (mAndroidAutoActivityZoomController == null) {
            mAndroidAutoActivityZoomController =
                    new AndroidAutoActivityZoomController(getActivityTabProvider());
        }
        mAndroidAutoActivityZoomController.onStart();
    }
    ...
    public @Nullable ActorPictureInPictureController maybeCreateActorPipController() {
        if (getProfileProviderSupplier().get() == null
                /*|| !GlicEnabling.isProfileEligible(
                        getProfileProviderSupplier().get().getOriginalProfile()) Vivaldi */
    ...
    public void onStop() {
        ...
        // Vivaldi (Android Auto).
        if (mAndroidAutoActivityZoomController != null) {
            mAndroidAutoActivityZoomController.onStop();
            mAndroidAutoActivityZoomController = null;
        }
    }
    ...
    public void finishNativeInitialization() {
        ...
        if (BuildConfig.IS_VIVALDI
                && !VivaldiPreferences.getSharedPreferencesManager().readBoolean(
                        VivaldiPreferences.PREF_PRIVACY_REPORT_NOTIFICATION_SET, false)) {
            Log.i("PRIVACY_REPORT", "Notification has not been set yet, starting channels");
            ...
            PrivacyReportNotificationReceiver.addNotification(this);
        }
        // End Vivaldi
    }
    ...
    private boolean maybeOnTabletModeChange() {
        // Vivaldi AUTO-334: On AAOS, skip tablet mode check to avoid spurious recreation on
        // headlamp (day/night) toggles.
    ...
    public void performOnConfigurationChanged(Configuration newConfig) {
        ...
        if (mConfig != null) {
            ...
            // Vivaldi
            if (didChangeUiModeType(
                    mConfig.uiMode, newConfig.uiMode, Configuration.UI_MODE_TYPE_DESK)) {
                doRecreateActivity();
                return;
            }
        }
    ...
    public boolean onMenuOrKeyboardAction(
        ...
        if (id == R.id.vivaldi_offline_page_id) {
            DownloadUtils.downloadOfflinePage(this, currentTab, fromMenu);
            return true;
        }
    ...
    /** Vivaldi **/
    public void captureThumbnailForSpeedDial(Tab tab, BookmarkModel model, BookmarkItem item) {
        if (item == null || tab == null) return;
        if (!SpeedDialUtils.thumbnailsEnabled()
                || item.isDefaultBookmark()
                || (item.getThumbnailPath() != null && !item.getThumbnailPath().isEmpty())) {
            return;
        }
    ...
    public @Nullable PanelManager getPanelManager() {

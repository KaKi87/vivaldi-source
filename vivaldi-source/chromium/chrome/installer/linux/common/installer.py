...
def verify_package_deps(expected_deps, actual_deps):
    ...
    if int(os.environ.get("IGNORE_DEPS_CHANGES", 0)) == 1:
      return;
...
class Installer:
    ...
    def initialize(self):
        # Load branding info
        if self.branding == "vivaldi":
          filename = "vivaldi.info"
        else:
    ...
    def prep_staging_common(self):
        ...
        dirs = [
            ...
            self.staging_dir / "usr/share/xfce4/helpers",
        ]
        for d in dirs:
    ...
    def _stage_binaries(self, install_dir):
        ...
        self._install(stripped_file, install_dir / (progname + "-bin"), mode=0o755)
        ...
        self._install(
            stripped_file, install_dir / "vivaldi-sandbox", mode=0o4755)
    ...
    def _stage_resources(self, install_dir):
        ...
        if (self.output_dir / "vivaldi_100_percent.pak").exists():
            self._install_into_dir(
                self.output_dir / "vivaldi_100_percent.pak",
                install_dir,
                mode=0o644,
            )
            self._install_into_dir(
                self.output_dir / "vivaldi_200_percent.pak",
                install_dir,
                mode=0o644,
            )
        else:
    ...
    def _stage_desktop_integration(self, install_dir):
        ...
        desktop_file = (
            self.staging_dir / "usr/share/applications" /
            f"{self.context['USR_BIN_SYMLINK_NAME']}.desktop")
        ...
        process_template(
            self.output_dir / "installer/common/xfce-helper.template",
            man_page,
            self.context,
        )
        man_page.chmod(0o644)
        ...
        for root, dirs, files in os.walk(self.staging_dir):
            ...
            for item in dirs + files:
                ...
                if path.is_dir():
                    ...
                    if not os.path.lexists(expect_exists) and not target.endswith("WidevineCdm"):
                        print(
                ...
                if base_name == "chrome-management-service":
                    expected_perms = 0o755
                elif base_name == "libffmpeg.so":
                    expected_perms = 0o644
                elif base_name == "vivaldi-sandbox":

...
sys.path.append(os.path.join(os.path.dirname(__file__), "..", "..", "..", "..", "..", "installer", "linux", "common"))
import vivaldi_installer as installer
...
def gen_spec(context, spec_file, output_dir):
    if spec_file.exists():
        spec_file.unlink()
    ...
    installer.process_template(output_dir / "installer/rpm/vivaldi.spec.template", spec_file,
                               context)
...
def main():
    ...
    spec_file = tmp_file_dir / "vivaldi.spec"
    ...
    gen_spec(inst.context, spec_file, output_dir)

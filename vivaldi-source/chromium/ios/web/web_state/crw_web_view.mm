  ...
  if (!base::FeatureList::IsEnabled(
    ...
    if (![self canPerformAction:@selector(copy:) withSender:self]) {
      ...
      if (vivaldi::IsVivaldiRunning() &&
          [self.editMenuBuilder respondsToSelector:@selector
                                (buildSelectionlessMenuWithBuilder:)]) {
        // When textfield is empty we still want to
        // show the context menu with custom action
        // such as Add as Search engine.
        [self.editMenuBuilder buildSelectionlessMenuWithBuilder:builder];
      } // End Vivaldi

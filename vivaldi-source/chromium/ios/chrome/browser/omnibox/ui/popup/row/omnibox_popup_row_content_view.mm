...
@implementation OmniboxPopupRowContentView {
  ...
  // Delete button
  ExtendedTouchTargetButton* _deleteButton;

  NSLayoutConstraint* _textTrailingToDeleteButtonConstraint;
  NSLayoutConstraint* _trailingButtonToDeleteButtonConstraint;
  NSLayoutConstraint* _deleteButtonTrailingConstraint;
  // End Vivaldi
...
- (instancetype)initWithConfiguration:
  ...
  if (self) {
    ...
    _deleteButton.isAccessibilityElement = NO;
    [_deleteButton addTarget:self
                        action:@selector(deleteButtonTapped)
              forControlEvents:UIControlEventTouchUpInside];
    _deleteButton.hidden = YES;  // Optional view.
    [self addSubview:_deleteButton];

    // The delete button is optional. Constraint is activated when needed.
    _textTrailingToDeleteButtonConstraint = [_deleteButton.leadingAnchor
        constraintEqualToAnchor:_textStackView.trailingAnchor
                       constant:kTextTrailingMargin];

    _trailingButtonToDeleteButtonConstraint = [_deleteButton.leadingAnchor
        constraintEqualToAnchor:_trailingButton.trailingAnchor
                       constant:kMultilineTextTrailingMargin];

    _deleteButtonTrailingConstraint = [self.trailingAnchor
        constraintEqualToAnchor:_deleteButton.trailingAnchor
    ...
    [NSLayoutConstraint activateConstraints:@[
      ...
      [_deleteButton.centerYAnchor
          constraintEqualToAnchor:self.centerYAnchor],
      _deleteButtonTrailingConstraint,
      // End Vivaldi
...
- (void)setupWithConfiguration:
  ...
  // Important! Note(prio@vivaldi.com) -
  // We have to add the constraint separately because of the delete button.
  // There can be several states - Only texts, texts + trailing button,
  // texts + trailing button + delete button, texts + delete button.
  if (IsVivaldiRunning()) {
    // Trailing Button.
    // TODO: CR140 - Fix this.
    ...
    // Delete Button
    if (configuration.deleteIcon) {
      [_deleteButton setImage:configuration.deleteIcon
                     forState:UIControlStateNormal];
      _deleteButton.hidden = NO;
      _deleteButton.tintColor = configuration.deleteIconTintColor;
      _deleteButton.accessibilityIdentifier =
          configuration.deleteButtonAccessibilityIdentifier;
    } else {
      _deleteButton.hidden = YES;
      _deleteButton.accessibilityIdentifier = nil;
    }

    // Deactivate all possible trailing constraints first.
    _textTrailingConstraint.active = NO;
    _textTrailingToButtonConstraint.active = NO;
    _textTrailingToDeleteButtonConstraint.active = NO;
    _trailingButtonTrailingConstraint.active = NO;
    _deleteButtonTrailingConstraint.active = NO;
    _trailingButtonToDeleteButtonConstraint.active = NO;

    BOOL hasTrailing = (configuration.trailingIconType != TrailingIconType::kNone);
    BOOL hasDelete = (configuration.deleteIcon != nil);

    // Now activate the constraints for whichever scenario applies.
    if (!hasTrailing && !hasDelete) {
      // [A] No buttons: text → trailing edge
      _textTrailingConstraint.active = YES;
    } else if (hasTrailing && !hasDelete) {
      // [B] Only trailing button
      _textTrailingToButtonConstraint.active = YES;
      _trailingButtonTrailingConstraint.active = YES;
    } else if (!hasTrailing && hasDelete) {
      // [C] Only delete button
      _textTrailingToDeleteButtonConstraint.active = YES;
      _deleteButtonTrailingConstraint.active = YES;
    } else {
      // [D] Both trailing button & delete button
      _textTrailingToButtonConstraint.active = YES;
      _trailingButtonToDeleteButtonConstraint.active = YES;
      _deleteButtonTrailingConstraint.active = YES;
    }
    ...
    if (configuration.primaryTextNumberOfLines > 1) {
      ...
      _textTrailingToDeleteButtonConstraint.constant =
          kMultilineTextTrailingMargin;
      ...
      _textTrailingToDeleteButtonConstraint.constant = kTextTrailingMargin;
    ...
    if (configuration.isPopoutOmnibox) {
      ...
      _deleteButtonTrailingConstraint.constant =
          kTrailingButtonTrailingMarginPopout;
      _leadingConstraint.constant = kLeadingSpacePopout;
    } else {
      _trailingButtonTrailingConstraint.constant =
          kTrailingButtonTrailingMargin;
      _deleteButtonTrailingConstraint.constant =
          kTrailingButtonTrailingMargin;
...
/// Handles tap on delete button.
- (void)deleteButtonTapped {
  [self.configuration.delegate
      omniboxPopupRowWithConfiguration:self.configuration
         didTapDeleteButtonAtIndexPath:self.configuration.indexPath];
}

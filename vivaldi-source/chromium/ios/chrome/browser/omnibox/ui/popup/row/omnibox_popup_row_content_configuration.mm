...
// Delete Icon.
@property(nonatomic, strong, readwrite) UIImage* deleteIcon;
@property(nonatomic, strong, readwrite) UIColor* deleteIconTintColor;
@property(nonatomic, strong, readwrite)
    NSString* deleteButtonAccessibilityIdentifier;
// End Vivaldi
...
- (instancetype)updatedConfigurationForState:(id<UIConfigurationState>)state {
  ...
  if (IsVivaldiRunning()) {
    configuration.deleteIconTintColor =
        allowHighlight ? UIColor.whiteColor : [UIColor secondaryLabelColor];
  } // End Vivaldi
...
// Vivaldi
- (BOOL)showXButton {
  PrefService* pref = GetApplicationContext()->GetLocalState();
  return pref->GetBoolean(vivaldiprefs::kVivaldiShowXForSuggestionEnabled);
}

/// Handles tap on the delete button.
- (void)deleteButtonTapped {
  CHECK(_indexPath);
  [self.delegate omniboxPopupRowWithConfiguration:self
                    didTapDeleteButtonAtIndexPath:_indexPath];
}
// End Vivaldi

...
- (void)omniboxTextController:(OmniboxTextController*)omniboxTextController
  ...
  if (IsVivaldiRunning() && isFirstUpdate) {
    // The first zero-suggest update can be a most-visited tile. Its favicon is
    // correct in the popup row, but the focused omnibox should start with the
    // search engine icon and only change after an explicit suggestion preview.
    [self setDefaultLeftImage];
    return;
  } // End Vivaldi
...
#pragma mark - OmniboxMutator (Vivaldi)
...
- (void)insertKeywordToOmnibox:(NSString*)text fromPaste:(BOOL)fromPaste {
  [self.omniboxTextController insertKeywordToOmnibox:text fromPaste:fromPaste];
}

    ...
    if (IsVivaldiRunning()) {
      NSString* formattedURL =
          [VivaldiGlobalHelpers
              formattedURLStringForChromeScheme:textInput.userText];
      [self.textInput setText:formattedURL];
      [self textDidChangeWithUserEvent:YES];
    } // End Vivaldi
...
- (void)insertKeywordToOmnibox:(NSString*)text fromPaste:(BOOL)fromPaste {
  ...
  dispatch_async(dispatch_get_main_queue(), ^{
    UITextPosition *endPosition = weakSelf.textInput.endOfDocument;
    weakSelf.textInput.selectedTextRange =
        [weakSelf.textInput textRangeFromPosition:endPosition

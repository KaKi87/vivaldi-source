...
PlaceholderService::PlaceholderService(
    ...
    VivaldiFaviconLoader* vivaldi_favicon_loader,
    ProfileIOS* profile)
    : favicon_loader_(favicon_loader),
      template_url_service_(template_url_service),
      vivaldi_favicon_loader_(vivaldi_favicon_loader),
      profile_(profile),
...
void PlaceholderService::FetchDefaultSearchEngineIcon(
  ...
  if (profile_->IsOffTheRecord()) {
    default_provider = template_url_service_->GetDefaultSearchProvider(
...
void PlaceholderService::OnTemplateURLServiceChanged() {
  ...
  TemplateURLService::DefaultSearchType type =
      profile_->IsOffTheRecord() ? TemplateURLService::kDefaultSearchPrivate
                                 : TemplateURLService::kDefaultSearchMain;
  if (!template_url_service_ ||
      template_url_service_->GetDefaultSearchProvider(type) == current_dse_) {
    return;
  }
  current_dse_ = template_url_service_->GetDefaultSearchProvider(type);
#else
  if (!template_url_service_ ||
...
void PlaceholderService::OnIconReceivedForTemplateURL(
  ...
  if (icon && template_url_service_ &&
      template_url_service_->GetDefaultSearchProvider(type)->id() ==
          template_url_id &&
  ...
  auto favicon_completion = base::CallbackToBlock(base::BindRepeating(
      [](base::WeakPtr<PlaceholderService> weak_self,
        ...
        if (vivaldi::IsVivaldiRunning()) {
          // Note: (prio@vivaldi.com) - Resize the icon before showing because
          // we do not always load from cache and downloaded icon can be bigger
          // than desired size.
          favicon = ResizeImage(favicon, CGSize(kFaviconSize, kFaviconSize),
...
#if defined(VIVALDI_BUILD)
  if (vivaldi_favicon_loader_) {
    vivaldi_favicon_loader_->GetFaviconForIconUrl(
        template_url->favicon_url(), icon_point_size, icon_point_size,
        ios::VivaldiFaviconOptions::kCacheFirstThenFresh, favicon_completion);
  }
#else

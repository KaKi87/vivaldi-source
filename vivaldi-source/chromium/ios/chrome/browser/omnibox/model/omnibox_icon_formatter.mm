...
OmniboxSuggestionIconType IconTypeFromMatch(const AutocompleteMatch& match) {
  ...
  if (vivaldi::IsVivaldiRunning() &&
      match.type == AutocompleteMatchType::DIRECT_MATCH) {
    return OmniboxSuggestionIconType::kDirectMatch;
  } // End Vivaldi
...
- (instancetype)initWithMatch:(const AutocompleteMatch&)match {
  ...
  if (vivaldi::IsVivaldiRunning() &&
    ...
    OmniboxIconFormatter* formatter =
        [self initWithIconType:iconType
            suggestionIconType:suggestionIconType
                      isAnswer:isAnswer
                      imageURL:[[CrURL alloc] initWithGURL:imageURL]];

    NSString* localPath = base::SysUTF16ToNSString(match.local_favicon_path);
    UIImage* localFavicon = (localPath.length > 0)
        ? [UIImage imageWithContentsOfFile:localPath] : nil;

    // If localFavicon is nil, use the fallback image instead.
    UIImage* chosenImage =
        localFavicon ?: [UIImage imageNamed:vNTPSDFallbackFavicon];

    // Resize and store as directMatchFavicon.
    formatter.directMatchFavicon = ResizeImage(chosenImage,
                                               CGSizeMake(kFaviconSize,
                                                          kFaviconSize),

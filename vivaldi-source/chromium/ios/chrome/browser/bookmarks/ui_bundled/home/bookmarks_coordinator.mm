...
- (void)bookmarkBrowserDismissed {
  ...
  // Note:(prio@vivaldi.com) - Skip this DCHECKs. These will always return
  // false for us since we present the bookmarks home within the panels
  // controller.
  if (!IsVivaldiRunning()) {
...
#pragma mark - PRIVATE
- (void)showHomeViewController:(LegacyChromeTableViewController*)viewController
    withReplacementViewControllers:
        (NSArray<LegacyChromeTableViewController*>*)replacementViewControllers {
  TableViewNavigationController* navController =
      [[TableViewNavigationController alloc] initWithTable:viewController];

...
- (void)generateTableViewDataForRootNode {
  ...
  // Note:(prio@vivaldi.com) - Show permanant nodes even if they don't have
  // any children.
  if (IsVivaldiRunning()) {
    showProfileSection = YES; // End Vivaldi
  }
...
- (void)generateTableViewDataWithPermanentNodes:
  ...
  for (const BookmarkNode* permanentNode : permanentNodes) {
    ...
    if (!permanentNode->IsVisible()) {
      ...
      if (IsVivaldiRunning() &&
          permanentNode != _bookmarkModel->bookmark_bar_node()) {
      continue;
      } // End Vivaldi
  ...
  if (IsVivaldiRunning()) {
    // Add trash folder
    const BookmarkNode* trashNode = _bookmarkModel->trash_node();
    BookmarksHomeNodeItem* trashBarItem =
        [BookmarksHomeNodeItem makeItemWithType:BookmarksHomeItemTypeBookmark
                                   bookmarkNode:trashNode
                    shouldDisplayCloudSlashIcon:NO];
...
- (int)populateNodeItemWithQuery:(const bookmarks::QueryFields&)query {
  ...
  if (IsVivaldiRunning()) {
    const BookmarkNode* trashFolder = _bookmarkModel->trash_node();
    const bool isTrash = vivaldi_bookmark_kit::IsTrash(self.displayedNode);
    int nodeCount = 0;
    for (const BookmarkNode* node : nodes) {
      if (IsNodeInTrash(node, trashFolder) && (!isTrash)) {
        // Skipping the node, because it is already in trash
        continue;
      }
      BookmarksHomeNodeItem* nodeItem =
          [BookmarksHomeNodeItem makeItemWithType:BookmarksHomeItemTypeBookmark
...
// Comparing node with another node (parent node)
// Basically searching here for for the parent node, if it is trash or not
bool IsNodeInTrash(const BookmarkNode* node, const BookmarkNode* trashNode) {

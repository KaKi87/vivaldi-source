...
@implementation SceneCoordinator {
  ...
  // Vivaldi
  // Coordinator for share feedback prompt.
...
- (void)showHistory {
  ...
  if (vivaldi::IsVivaldiRunning()){
    Browser* browser = self.currentBrowser ? self.currentBrowser
                                           : _regularBrowser.get();
...
- (void)
  ...
  if (vivaldi::IsVivaldiRunning()) {
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
      [weakSelf showVivaldiFeedbackPromptFromViewController:baseViewController];
    });
    return;
  } // End Vivaldi
...
- (void)openURLInNewTab:(OpenNewTabCommand*)command {
  ...
  if (vivaldi::IsVivaldiRunning() &&
      !command.inIncognito &&
      !params.web_params.url.is_valid()) {
    ProfileIOS* profile = self.profile;
    PrefService* prefs = profile ? profile->GetPrefs() : nullptr;
    if (prefs) {
      NSString* urlString =
          [VivaldiTabSettingsHelper getNewTabURLWithPref:prefs];
      params.web_params.url =
          url_formatter::FixupURL(base::SysNSStringToUTF8(urlString),
                                  std::string());
    }
  }  // End Vivaldi
...
- (void)showVivaldiFeedbackPromptFromViewController:
  ...
  Browser* browser =
      _regularBrowser ? _regularBrowser.get() : self.currentBrowser;
  if (!browser) {
    return;
  }
  _shareFeedbackCoordinator = [[VivaldiFeedbackViewCoordinator alloc]
      initWithBaseViewController:baseViewController
                         browser:browser

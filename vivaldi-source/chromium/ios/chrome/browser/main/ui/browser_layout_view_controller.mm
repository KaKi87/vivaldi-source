...
@protocol VivaldiTabStripLayoutUpdating <NSObject>
...
@implementation BrowserLayoutViewController {
  ...
  // Vivaldi
  // View acting as the background behind the bottom-positioned Vivaldi tab
  // strip and the bottom safe area.
  UIView* _bottomTabStripBackgroundView;
  // End Vivaldi
...
- (void)updateForFullscreenProgress:(CGFloat)progress
  ...
  if (IsVivaldiRunning() && [self shouldPlaceTabStripAtBottom]) {
    ...
    CGFloat height = [self currentTabStripHeight];
    CGFloat bottomInset = self.safeAreaProvider.safeArea.bottom;
    CGRect frame = self.view.bounds;
    frame.origin.y = CGRectGetHeight(self.view.bounds) - bottomInset - height;
    frame.size.height = height;
    _tabStripViewController.view.frame = frame;
    if (_bottomTabStripBackgroundView &&
        !_bottomTabStripBackgroundView.hidden) {
      _bottomTabStripBackgroundView.frame =
          CGRectMake(0, CGRectGetMinY(frame), CGRectGetWidth(self.view.bounds),
                     height + bottomInset);
    }
    return;
  }  // End Vivaldi
  ...
  if (IsVivaldiRunning()) {
    frame.size.height = [self currentTabStripHeight];
    _tabStripViewController.view.frame = frame;
    return;
  } // End Vivaldi
...
- (void)updateUIOnTraitChange:(UITraitCollection*)previousTraitCollection {
  ...
  if (IsVivaldiRunning()) {
    if (_tabStripViewController) {
      _tabStripViewController.view.hidden = ![self shouldShowTabStrip];
    }
    [self updateVivaldiTabStripLayout];
  } else {
  if (_tabStripViewController) {
...
- (void)ensureStatusBarViewsInstalled {
  ...
  BOOL shouldShowStatusBarBackground =
      _tabStripViewController && [self shouldShowTabStrip];
  if ([self shouldPlaceTabStripAtBottom]) {
    shouldShowStatusBarBackground = NO;
  }
  // Only install status bar background when the top-positioned tab strip is
  // visible.
  if (!shouldShowStatusBarBackground) {
    return;
  }

  UIColor* tabStripBackgroundColor = [self tabStripBackgroundColor];
  self.fadingStatusBarView.backgroundColor = tabStripBackgroundColor;
...
- (void)updateStatusBarBackgroundViews {
  ...
  bool shouldShow = [self shouldShowTabStrip] &&
                    ![self shouldPlaceTabStripAtBottom] &&
                    _tabStripViewController;
#else
...
- (CGFloat)currentTabStripHeight {
  ...
  CGFloat preferredHeight = _tabStripViewController.preferredContentSize.height;
  return preferredHeight > 0 ? preferredHeight
                             : TabStripCollectionViewConstants.height;
}
...
- (void)updateVivaldiTabStripLayout {
  ...
  BOOL shouldShowTabStrip = [self shouldShowTabStrip];
  BOOL shouldPlaceAtBottom = [self shouldPlaceTabStripAtBottom];
  CGFloat tabStripHeight = [self currentTabStripHeight];

  if ([_tabStripViewController
          respondsToSelector:@selector(setBottomOmniboxEnabled:)]) {
    id<VivaldiTabStripLayoutUpdating> tabStripViewController =
        (id<VivaldiTabStripLayoutUpdating>)_tabStripViewController;
    [tabStripViewController setBottomOmniboxEnabled:shouldPlaceAtBottom];
  }

  UIEdgeInsets safeAreaInsets = self.safeAreaProvider.safeArea;
  CGRect frame = self.view.bounds;
  frame.size.height = tabStripHeight;
  if (shouldPlaceAtBottom) {
    frame.origin.y = CGRectGetHeight(self.view.bounds) - safeAreaInsets.bottom -
                     tabStripHeight;
  } else {
    frame.origin.y = safeAreaInsets.top;
  }
  _tabStripViewController.view.frame = frame;
  _tabStripViewController.view.hidden = !shouldShowTabStrip;

  if (shouldShowTabStrip && shouldPlaceAtBottom) {
    if (!_bottomTabStripBackgroundView) {
      _bottomTabStripBackgroundView = [[UIView alloc] init];
      _bottomTabStripBackgroundView.autoresizingMask =
          UIViewAutoresizingFlexibleWidth | UIViewAutoresizingFlexibleTopMargin;
    }
    if (_bottomTabStripBackgroundView.superview != self.view) {
      [self.view addSubview:_bottomTabStripBackgroundView];
    }
    [self.view insertSubview:_bottomTabStripBackgroundView
                belowSubview:_tabStripViewController.view];
    _bottomTabStripBackgroundView.frame =
        CGRectMake(0, CGRectGetMinY(frame), CGRectGetWidth(self.view.bounds),
                   tabStripHeight + safeAreaInsets.bottom);
    _bottomTabStripBackgroundView.backgroundColor =
        [self tabStripBackgroundColor];
    _bottomTabStripBackgroundView.alpha = _tabStripViewController.view.alpha;
    _bottomTabStripBackgroundView.hidden = NO;
  } else {
    _bottomTabStripBackgroundView.hidden = YES;
  }
...
- (void)setVivaldiTabStripAlpha:(CGFloat)alpha {
  if (!IsVivaldiRunning() || !_tabStripViewController) {
    return;
  }

  _tabStripViewController.view.alpha = alpha;
  if ([self shouldPlaceTabStripAtBottom]) {
    _bottomTabStripBackgroundView.alpha = alpha;
  } else {
    _fadingStatusBarView.alpha = alpha;
    _staticStatusBarView.alpha = alpha;
  }
}

- (BOOL)shouldShowTabStrip {
  return
      [self.browserViewController respondsToSelector:@selector(canShowTabStrip)]
          ? [self.browserViewController canShowTabStrip]
          : CanShowTabStrip(self);
}

- (BOOL)shouldPlaceTabStripAtBottom {
  if (![self shouldShowTabStrip]) {
    return NO;
  }

  return [self.browserViewController
             respondsToSelector:@selector(isBottomOmniboxEnabled)] &&
         [self.browserViewController isBottomOmniboxEnabled];
}

- (UIColor*)tabStripBackgroundColor {
  if ([self.browserViewController
          respondsToSelector:@selector(browserLayoutTabStripBackgroundColor)]) {
    UIColor* backgroundColor =
        [self.browserViewController browserLayoutTabStripBackgroundColor];
    if (backgroundColor) {
      return backgroundColor;
    }
  }

  return TabStripHelper.backgroundColor;
}

- (void)vivaldiTabStripHeightDidChange:(NSNotification*)notification {
  [self updateVivaldiTabStripLayout];
}
// Vivaldi
...
- (void)removeTabStripViewController {
  ...
  if (IsVivaldiRunning()) {
    [[NSNotificationCenter defaultCenter]
        removeObserver:self
                  name:TabStripNotificationConstants
                           .vivaldiTabStripHeightDidChange
                object:_tabStripViewController];
    _bottomTabStripBackgroundView.hidden = YES;
  } // End Vivaldi
...
- (void)addTabStripViewController {
  ...
  if (IsVivaldiRunning()) {
    ...
    tabStripView.hidden = ![self shouldShowTabStrip];
  } else {
...
- (void)setTabStripViewController:(UIViewController*)tabStripViewController {
  ...
  if (IsVivaldiRunning() && _tabStripViewController) {
    [[NSNotificationCenter defaultCenter]
        addObserver:self
           selector:@selector(vivaldiTabStripHeightDidChange:)
               name:TabStripNotificationConstants
                        .vivaldiTabStripHeightDidChange
             object:_tabStripViewController];
  } // End Vivaldi

...
- (void)start {
  ...
  // profile.
  if (!vivaldi::IsVivaldiRunning()) { // We do not show incognito badge.
...
#pragma mark - LocationBarViewControllerDelegate (Vivaldi)
- (void)locationBarContentsTintColorDidChange:(UIColor*)tintColor {
  self.badgeViewController.displayedBadgeTintColor = tintColor;
}

- (void)locationBarCopySanitizedLinkTapped {
  web::WebState* web_state = self.webState;
  if (!web_state) {
    return;
  }
  // use canonical URL when valid
  __weak LocationBarCoordinator* weak_self = self;
  activity_services::RetrieveCanonicalUrl(
      web_state,       base::BindOnce(^(const GURL& canonical_url) {
        LocationBarCoordinator* strong_self = weak_self;
        if (!strong_self || !strong_self.browser) {
          return;
        }
        if (strong_self.browser->GetWebStateList()->GetActiveWebState() !=
            strong_self.webState) {
          return;
        }

        web::WebState* active_web_state = strong_self.webState;
        DCHECK(active_web_state);
        const GURL visible = active_web_state->GetVisibleURL();
        const GURL url_to_copy =
            canonical_url.is_valid() ? canonical_url : visible;
        StoreURLInPasteboard(CopyUrlWithoutParameters(url_to_copy));
      }));
}
// End Vivaldi

...
@property(nonatomic, strong, readwrite) LegacyToolbarButton* panelButton;
...
// iPhone.
@property(nonatomic, strong, readwrite) LegacyToolbarButton* vivaldiMoreButton;
// Vivaldi home button
@property(nonatomic, strong, readwrite) LegacyToolbarButton* vivaldiHomeButton;
...
- (void)updateTabGroupIndicatorAvailability {
  ...
  if (IsVivaldiRunning()) {
    // Note: (prio@vivaldi.com) - We never show Tab Group Indicator.
    self.tabGroupIndicatorView.available = NO;
  } else {
...
- (void)redrawStackView:(UIStackView*)stackView
  ...
  // Remove all subviews
  for (LegacyToolbarButton *button in stackView.arrangedSubviews) {
    [stackView removeArrangedSubview:button];
    [button removeFromSuperview];
  }

  // Add new buttons
  for (LegacyToolbarButton *button in buttons) {

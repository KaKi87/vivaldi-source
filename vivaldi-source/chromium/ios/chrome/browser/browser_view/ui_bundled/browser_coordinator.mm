...
// Vivaldi: Coordinator for Text Zoom.
@property(nonatomic, strong)
    VivaldiPageZoomDialogCoordinator* pageZoomCoordinator;
// End Vivaldi
...
@implementation BrowserCoordinator {
  ...
  VivaldiSiteTrackerPrefsCoordinator* _siteTrackerPrefsCoordinator;
  VivaldiSyncErrorDialogCoordinator* _syncErrorDialogCoordinator;
  // End Vivaldi
}
...
- (void)createViewController {
  ...
  // Vivaldi
  _viewController.profile = self.browser->GetProfile();
  // End Vivaldi
...
- (void)updateViewControllerDependencies {
  ...
  _panelInteractionController.toolbarOffsetProvider =
      (id<PanelToolbarOffsetProvider>)self.viewController;
  // End Vivaldi
...
- (void)destroyViewControllerDependencies {
  ...
  [_siteTrackerPrefsCoordinator stop];
  _siteTrackerPrefsCoordinator = nil;
  // End Vivaldi
...
- (void)stopAndStartSharingCoordinatorFromView:(UIView*)shareButton {
  ...
  if (IsVivaldiRunning()) {
    GuideName* preferredToolsGuide =
        vToolsMenuGuide;
    UIView* sourceView = nil;
    CGRect sourceRect = CGRectZero;
    VivaldiResolveShareAnchor(self.viewController, _layoutGuideCenter, shareButton,
                              preferredToolsGuide, kToolsMenuGuide, &sourceView,
                              &sourceRect);
...
- (void)showShareSheetFromShareButton:(UIView*)shareButton {
  ...
  if (IsVivaldiRunning() && self.popupMenuCoordinator) {
    UIViewController* overflowMenuController =
        VivaldiFindPresentedOverflowMenuController(self.viewController);
    BOOL overflowMenuPresented = overflowMenuController != nil;
    if (self.popupMenuCoordinator.isShowingPopupMenu || overflowMenuPresented) {
      __weak __typeof(self) weakSelf = self;
      VivaldiDismissPopupMenuAndRunCompletion(
          self.popupMenuCoordinator, YES, ^{
            [weakSelf showShareSheetFromShareButton:shareButton];
          });
      return;
    }
  } // End Vivaldi
...
- (void)showShareSheetForChromeApp {
  ...
  if (IsVivaldiRunning()) {
    originView = VivaldiFindToolsMenuAnchorView(_layoutGuideCenter,
                                                vToolsMenuGuide,
                                                kToolsMenuGuide);
  } // End Vivaldi
...
- (NSString*)getHomePageURL {
  return [VivaldiTabSettingPrefs
    getHomepageUrlWithPrefService: self.browser->GetProfile()->GetPrefs()];
}

- (void)showHomePage {
  NSString* url = [self getHomePageURL];
  if ([VivaldiGlobalHelpers isValidURL:url]) {
    [self loadURL: url_formatter::FixupURL(base::SysNSStringToUTF8(url),
                                           std::string())];
    return;
  }
  [self openInternalURL:GURL(kChromeUINewTabURL)];
}
...
- (void)openInternalURL:(GURL)URL {
  OpenNewTabCommand* command = [OpenNewTabCommand commandWithURLFromChrome:URL];
  DCHECK([command fromChrome]);
  [self loadURL:[command URL]];
}
// End Vivaldi
...
- (void)showTranslate {
  ...
  VivaldiIOSTranslateClient* translateClient =
      VivaldiIOSTranslateClient::FromWebState(activeWebState);
#else
...
- (void)showSendTabToSelfUI:(const GURL&)url
  ...
  if (self.sceneState.controller.isTabGridVisible) {
    presentingViewController = self.sceneState.controller.activeViewController;
  }
  _sendTabToSelfCoordinator = [[SendTabToSelfCoordinator alloc]
      initWithBaseViewController:presentingViewController
#else // Vivaldi
  _sendTabToSelfCoordinator = [[SendTabToSelfCoordinator alloc]
      initWithBaseViewController:self.viewController
#endif // End Vivaldi
...
- (void)openTextZoom {
  ...
  if (IsVivaldiRunning()) {
    if (_pageZoomCoordinator) {
      [_pageZoomCoordinator stop];
    }
    _pageZoomCoordinator = [self newPageZoomCoordinator];
    [_pageZoomCoordinator start];
  } else { // Vivaldi
...
- (void)closeTextZoom {
  ...
  if (IsVivaldiRunning() && [self stopVivaldiPageZoomIfNeeded]) {
    return;
  } // End Vivaldi
...
- (void)buildSelectionlessEditMenuWithBuilder:(id<UIMenuBuilder>)builder
                               inWebState:(web::WebState*)webState {
  return [self.browserContentCoordinator.editMenuBuilder
          buildSelectionlessEditMenuWithBuilder:builder
          inWebState:webState];
}
// End Vivaldi
...
- (void)showPrimaryAccountReauth {
  ...
  if (IsVivaldiRunning()) {
    [HandlerForProtocol(self.dispatcher, SettingsCommands)
        showSyncSettingsFromViewController:self.viewController];
    return;
  } // End Vivaldi
...
- (void)showQuickDeleteAndCanPerformRadialWipeAnimation:
  ...
  if (IsVivaldiRunning() && self.isOffTheRecord) {
    // When triggered from an off-the-record browser, Quick Delete is
    // allowed but the radial wipe animation is not. The animation path relies
    // on `TabsAnimationCommands`, which is only registered for the regular
    // tab grid.
...
- (void)showTranslateSceneWithText:(NSString*)sourceText {
  // Exit fullscreen if needed to make sure that panel button is visible.
  // Mainly required for Tablet with side panel.
  if ([VivaldiGlobalHelpers
          canShowSidePanelForTrait:self.viewController.traitCollection]) {
    _fullscreenController->ExitFullscreen();
  }

  [_panelInteractionController presentPanel:PanelPage::TranslatePage
                           withSearchString:sourceText];
}

- (void)dismissPanel {
  if (_panelInteractionController.isPresenting &&
      ![_panelInteractionController hasPresentedModalViewController]) {
    [_panelInteractionController dismissPanelModalControllerAnimated:NO];
  }
}

- (void)setToolbarHidden:(BOOL)hidden {
  _toolbarCoordinator.primaryToolbarViewController.view.hidden = hidden;
}

- (void)showDefaultReviewPrompt {
  [HandlerForProtocol(self.dispatcher, SceneCommands)
      showReportAnIssueFromViewController:self.viewController
                                   sender:UserFeedbackSender::ToolsMenu];
}

#pragma mark - ActivityServiceCommands (Vivaldi)
- (void)showTrackerBlockerManager {
  _siteTrackerPrefsCoordinator =
      [[VivaldiSiteTrackerPrefsCoordinator alloc]
          initWithBaseViewController:self.viewController
              browser:self.browser];
  [_siteTrackerPrefsCoordinator start];
}
...
- (VivaldiPageZoomDialogCoordinator*)newPageZoomCoordinator {
  BrowserLayoutViewController* browserLayoutViewController =
      self.browserLayoutViewController;
  // Prefer using infobar modal overlay container as the base view controller
  // for page zoom dialog, if it exists, to make sure the dialog is presented
  // above the tab strip when tab strip is redrawned.
  UIViewController* baseViewController =
      browserLayoutViewController.infobarModalOverlayContainerViewController;
  if (!baseViewController) {
    // Just a fallback. Infobar modal overlay container is expected to always
    // exist.
    baseViewController = browserLayoutViewController;
  }
  VivaldiPageZoomDialogCoordinator* pageZoomCoordinator =
      [[VivaldiPageZoomDialogCoordinator alloc]
          initWithBaseViewController:baseViewController
                             browser:self.browser];
  return pageZoomCoordinator;
}

- (BOOL)stopVivaldiPageZoomIfNeeded {
  VivaldiPageZoomDialogCoordinator* coordinator = self.pageZoomCoordinator;
  if (!coordinator) {
    return YES;
  }
  __weak __typeof(self) weakSelf = self;
  [coordinator stopAnimated:YES
                 completion:^{
                   weakSelf.pageZoomCoordinator = nil;
                 }];
  return YES;
}

- (void)setBrowserForReaderMode {
  // Ensure ReaderModeController has Browser* for UI presentation.
  ReaderModeController::SetBrowser(self.browser);
}
// End Vivaldi

...
#import "components/search_engines/vivaldi_template_url_service.h"
...
@interface SceneController () <AuthenticationServiceObserving,
                               ...
                               VivaldiATBConsumer,
                               VivaldiSearchEngineChangePromptCoordinatorDelegate,
                               VivaldiSyncErrorDialogDelegate
                               // End Vivaldi
  ...
  std::unique_ptr<SearchEngineObserverBridge> _templateServiceObserver;
  TemplateURLService* _templateURLService;  // weak
  adblock_filter::RuleService* _ruleService;  // weak
  // Bridge to register for adblock backend changes.
  std::unique_ptr<vivaldi_adblocker::VivaldiATBManagerBridge> _bridge;

  BOOL _templateURLServiceLoaded;
  BOOL _adblockerRuleServiceLoaded;
  // End Vivaldi
...
@property(nonatomic, strong)
    VivaldiFeedbackViewCoordinator* shareFeedbackCoordinator;

// Coordinator for Search Engine change prompt.
@property(nonatomic, strong)
    VivaldiSearchEngineChangePromptCoordinator* seChangePromptCoordinator;

// Coordinator for Sync Error dialog.
@property(nonatomic, strong)
  VivaldiSyncErrorDialogCoordinator* syncErrorDialogCoordinator;
...
- (void)transitionToSceneActivationLevel:(SceneActivationLevel)level
  ...
  if (level == SceneActivationLevelForegroundActive &&
    ...
    if (IsVivaldiRunning()) {
      ProfileIOS* profile = self.mainInterface.browser->GetProfile();
      if (!_ruleService && profile) {
        _ruleService =
            adblock_filter::RuleServiceFactory::GetForProfile(profile);
        _bridge.reset(new vivaldi_adblocker::VivaldiATBManagerBridge(
            self, _ruleService));
      }
      if (!_templateURLService && profile) {
        _templateURLService =
            ios::TemplateURLServiceFactory::GetForProfile(profile);
        if (!_templateServiceObserver) {
          _templateServiceObserver =
              std::make_unique<SearchEngineObserverBridge>(self,
                                                           _templateURLService);
        }
        _templateURLService->Load();
      } else {
        [self maybePromptSearchEngineChangeDialog];
      }
    } // End Vivaldi
...
- (void)startUpChromeUI {
  ...
  if (vivaldi::IsVivaldiRunning()) {
    ...
    [VivaldiTabSettingPrefs setPrefService:profile->GetPrefs()];
    ...
    [self showSyncErrorDialogAfterInitialDelay];
...
- (void)addAgents {
  ...
  if (IsVivaldiRunning()) {
    [sceneState
        addAgent:[[VivaldiDefaultBrowserStartupPromoSceneAgent alloc] init]];
  } // End Vivaldi
...
- (void)beginActivatingBrowser:(Browser*)browser
  ...
  if (IsVivaldiRunning() && !self.mainCoordinator.isTabGridActive) {
...
- (void)setCurrentInterfaceForMode:(ApplicationMode)mode {
  ...
  if (currentInterface && currentInterface == newInterface) {
    ...
    if (IsVivaldiRunning() && !self.activatingBrowser) {
...
- (void)didChangeWebStateList:(WebStateList*)webStateList
  ...
  switch (change.type()) {
    ...
    case WebStateListChange::Type::kDetach: {
      ...
      if (IsVivaldiRunning()) {
        if (webStateList->regular_tabs_count() <= 0 ||
            webStateList->empty()) {
...
- (void)webStateListBatchOperationEnded:(WebStateList*)webStateList {
  ...
  if (IsVivaldiRunning()) {
    if ((webStateList->regular_tabs_count() <= 0 && iter->second != 0) ||
        webStateList->empty()) {
      [self onLastWebStateClosedForWebStateList:webStateList];
    }
  } else {
...
- (void)onLastWebStateClosedForWebStateList:(WebStateList*)webStateList {
  ...
  if (!IsVivaldiRunning()) { // Skip as this can crash when there is pinned tabs
...
#pragma mark - TabGridCoordinatorDelegate (Vivaldi)
- (void)didSelectLoginFromEmptyStateView {
  [self.mainCoordinator showVivaldiSyncWithCreateAccountFlow:NO];
}

- (void)didSelectRegisterFromEmptyStateView {
  [self.mainCoordinator showVivaldiSyncWithCreateAccountFlow:YES];
}
...
#pragma mark - Vivaldi Custom Home Screen Shortcuts Action
- (void)showUninstallSurvey {
  [self startVivaldiFeedbackPromptWithEntryPoint:
    VivaldiFeedbackViewEntryPointSad baseViewController:
      self.currentInterface.viewController];
}

#pragma mark - Helper
- (BOOL)openNTPOnClosingLastTab {
  if (self.mainInterface.profile &&
      self.mainInterface.profile->GetPrefs()) {
...
- (void)handleNTPInsertEventWithChange:(const WebStateListChange&)change {
  if (self.willOpenNTPFromUserAction) {
    ...
    if (IsVisibleURLNewTabPage(insertChange.inserted_web_state()) &&
      ...
      WrangledBrowser* targetInterface =
          insertChange.inserted_web_state()->GetBrowserState()->IsOffTheRecord()
              ? self.incognitoInterface
              : self.mainInterface;
...
- (void)handleOpenNTPOnClosingLastTab {
  GURL url = url_formatter::FixupURL(base::SysNSStringToUTF8([self getNewTabURL]),
                                  std::string());
  ...
  dispatch_async(dispatch_get_main_queue(), ^{
    ...
    Browser* browser = strongSelf.mainInterface.browser;
    if (!browser) {
      return;
    }
    TabInsertionBrowserAgent::FromBrowser(browser)->InsertWebState(
    ...
    if (strongSelf.mainCoordinator.isTabGridActive &&
        !strongSelf.activatingBrowser) {
      [strongSelf beginActivatingBrowser:browser focusOmnibox:NO];
    }
  });
...
- (void)handleClosingLastTab {
  if ([self openNTPOnClosingLastTab] && !self.willCloseAllTabs) {
    [self handleOpenNTPOnClosingLastTab];
    return;
  }
...
#pragma mark Feedback Prompt
- (void)startVivaldiFeedbackPromptWithEntryPoint:
                                    (VivaldiFeedbackViewEntryPoint)entryPoint
                              baseViewController:
                                    (UIViewController*)baseViewController{
  self.shareFeedbackCoordinator =
      [[VivaldiFeedbackViewCoordinator alloc]
          initWithBaseViewController:baseViewController
                             browser:self.mainInterface.browser
                          entryPoint:entryPoint
                        allowsCancel:YES];
  self.shareFeedbackCoordinator.delegate = self;
  [self.shareFeedbackCoordinator start];
}

- (void)stopVivaldiFeedbackPrompt {
  if (self.shareFeedbackCoordinator) {
    [self.shareFeedbackCoordinator stop];
    self.shareFeedbackCoordinator = nil;
  }
}
...
- (void)searchEngineChanged {
  if (_templateURLServiceLoaded)
    return;
  _templateURLServiceLoaded = YES;
  [self maybePromptSearchEngineChangeDialog];
}

#pragma mark - VivaldiATBConsumer
- (void)ruleServiceStateDidLoad {
  if (_adblockerRuleServiceLoaded)
    return;
  _adblockerRuleServiceLoaded = YES;
  [self maybePromptSearchEngineChangeDialog];
}

#pragma mark - Search engine change prompt private methods.
// Try to show the search engine change dialog if conditions are met.
- (void)maybePromptSearchEngineChangeDialog {

  if (![self shouldPresentSearchEngineChangeDialog])
    return;

  if (!_templateURLService || !_templateURLService->loaded() ||
      !_ruleService || !_ruleService->IsLoaded()) {
    return;
  }
  ...
  BOOL shouldPrompt = prompt_manager->ShouldPrompt(
      profile->GetPrefs(), _templateURLService, _ruleService);
  if (shouldPrompt) {
    const std::vector<TemplateURL*> partner_search_engines =
        prompt_manager->GetPartnerSearchEngines(
            country_codes::GetCurrentCountryID(),
            GetApplicationContext()->GetApplicationLocaleStorage()->Get(),
            *(profile->GetPrefs()), _templateURLService);

    VivaldiSearchEngineChangePromptType promptType =
        [self targetPromptType:partner_search_engines
                    dialogType:prompt_manager->GetDialogType()];

    if (promptType != VivaldiSearchEngineChangePromptTypeNone) {
      self.seChangePromptCoordinator =
          [[VivaldiSearchEngineChangePromptCoordinator alloc]
              initWithBaseViewController:self.mainInterface.viewController
                                 browser:self.mainInterface.browser
                               providers:partner_search_engines
                              promptType:promptType];
      self.seChangePromptCoordinator.delegate = self;
      [self.seChangePromptCoordinator start];
    }
  }
}

- (VivaldiSearchEngineChangePromptType)
    targetPromptType:(const std::vector<TemplateURL*>)partners
          dialogType:(const std::string&)dialogType {

  VivaldiAccountSyncManager* accountManager = [self createAccountManager];

  // If no account manager, assume user is not donating
  // (account manager is only for donation check)
  bool isUserDonating = false;
  if (accountManager) {
    isUserDonating =
        ([accountManager donationBadgeTier] != VivaldiDonationBadgeTierNone);
  }

  return [self determinePromptTypeForDialogType:dialogType
                                       partners:partners
                                 isUserDonating:isUserDonating];
}

/// Creates and returns sync manager, or nil if any dependency is missing
- (VivaldiAccountSyncManager*)createAccountManager {
  Browser* mainBrowser =
      self.sceneState.browserProviderInterface.mainBrowserProvider.browser;
  if (!mainBrowser) {
    return nil;
  ...
  ProfileIOS* profile = mainBrowser->GetProfile()->GetOriginalProfile();
  if (!profile) {
    return nil;
  }
  ...
  vivaldi::VivaldiAccountManager* accountManager =
  ...
  return
      [[VivaldiAccountSyncManager alloc] initWithAccountManager:accountManager
                                                    syncService:syncService];
}

/// Determines the appropriate prompt type based on
/// dialog type and user state
- (VivaldiSearchEngineChangePromptType)
    determinePromptTypeForDialogType:(const std::string&)dialogType
                            partners:(const std::vector<TemplateURL*>)partners
                      isUserDonating:(bool)isUserDonating {
  // We have two types of key, and using string directly here to avoid one
  // extra patche.
  if (dialogType == "MULTICHOICE_PROMPT") {
    return [self handleMultiChoicePrompt:partners
                          isUserDonating:isUserDonating];
  } else if (dialogType == "DONATE_AND_MULTICHOICE_PROMPT") {
    return [self handleDonateAndMultiChoicePrompt:partners
                                   isUserDonating:isUserDonating];
  }

  return VivaldiSearchEngineChangePromptTypeNone;
}

/// Handles logic for `MULTICHOICE_PROMPT` dialog type
- (VivaldiSearchEngineChangePromptType)
    handleMultiChoicePrompt:(const std::vector<TemplateURL*>)partners
             isUserDonating:(bool)isUserDonating {
  if (partners.empty()) {
    if (!isUserDonating) {
      // Partner list is empty AND user is not donating -> Show Donate Dialog
      return VivaldiSearchEngineChangePromptTypeDonate;
    } else {
      // Partner list is empty AND user is donating -> Do not show anything
      return VivaldiSearchEngineChangePromptTypeNone;
    }
  }

  // Partner list has values -> Show multichoice dialog v1
  return VivaldiSearchEngineChangePromptTypeMultiChoice;
}

/// Handles logic for DONATE_AND_MULTICHOICE_PROMPT dialog type
- (VivaldiSearchEngineChangePromptType)
    handleDonateAndMultiChoicePrompt:(const std::vector<TemplateURL*>)partners
                      isUserDonating:(bool)isUserDonating {
  if (partners.empty()) {
    if (!isUserDonating) {
      // Partner list is empty AND user is not donating -> Show Donate Dialog
  ...
  if (isUserDonating) {
    // User is donating -> Show only multichoice dialog v2
    return VivaldiSearchEngineChangePromptTypeDonateMultiChoice;
  } else {
    // User is not donating -> Show Donate Dialog followed by multichoice dialog
    // v2
    return VivaldiSearchEngineChangePromptTypeDonate;
  }
}

// Returns YES if the search engine change dialog should be presented.
- (BOOL)shouldPresentSearchEngineChangeDialog {
  if (self.sceneState.profileState.startupInformation.isFirstRun) {
    return NO;
  }
  ...
  // Don't show the dialog in Incognito mode.
  if (self.currentInterface == self.incognitoInterface) {
    return NO;
  }
  // Don't show the dialog if the window is not active.
  if (self.sceneState.activationLevel < SceneActivationLevelForegroundActive) {
    return NO;
  }

  // Don't show the dialog if already presented.
  if (self.seChangePromptCoordinator) {
    return NO;
  }
  return YES;
}

- (void)setSearchEngineChangePromptWasSeen {
  ProfileIOS* profile = self.mainInterface.browser->GetProfile();
  if (profile && profile->GetPrefs()) {
    SearchEnginesManagersFactory::GetInstance()
        ->GetSearchEnginesPromptManager()
        ->MarkCurrentPromptAsSeen(profile->GetPrefs());
  }
}

- (void)stopSearchEnginePromptCoordinator {
  _templateURLService = nullptr;
  _ruleService = nullptr;
  // Remove observer bridges.
  _templateServiceObserver.reset();
  if (self.seChangePromptCoordinator) {
    self.seChangePromptCoordinator.delegate = nil;
    [self.seChangePromptCoordinator stop];
    self.seChangePromptCoordinator = nil;
  }
}

#pragma mark - VivaldiSearchEngineChangePromptCoordinatorDelegate
...
- (void)coordinatorDidCloseWithSelectingPartner:(const TemplateURL*)partner {
  if (_templateURLService) {
    _templateURLService->SetUserSelectedDefaultSearchProvider(
        const_cast<TemplateURL*>(partner),
        TemplateURLService::kDefaultSearchMain,
        search_engines::ChoiceMadeLocation::kChoiceScreen);
    [self setSearchEngineChangePromptWasSeen];
    [self stopSearchEnginePromptCoordinator];
  }
}

- (void)coordinatorDidCloseWithDonateNow {
  GURL url = GURL(vVivaldiDonateUrl);
  UrlLoadParams params = UrlLoadParams::InNewTab(url);
  params.web_params.transition_type = ui::PAGE_TRANSITION_TYPED;
  UrlLoadingBrowserAgent::FromBrowser(self.mainInterface.browser)->Load(params);
  [self setSearchEngineChangePromptWasSeen];
  [self stopSearchEnginePromptCoordinator];
}
...
#pragma mark - VivaldiSyncErrorDialogDelegate
- (void)showSyncErrorDialog {
  [self.mainCoordinator showVivaldiSyncWithCreateAccountFlow:NO];
  [_syncErrorDialogCoordinator stop];
  _syncErrorDialogCoordinator = nil;
}
...
- (void)showSyncErrorDialogAfterInitialDelay {
  ...
  dispatch_after(dispatch_time(DISPATCH_TIME_NOW,
    ...
    __strong __typeof(weakSelf) strongSelf = weakSelf;
    if (strongSelf) {
      [strongSelf showSyncErrorDialogIfNeeded];
    }
  });
}

- (void)showSyncErrorDialogIfNeeded {
  Browser* mainBrowser =
    self.sceneState.browserProviderInterface.mainBrowserProvider.browser;
  if (!mainBrowser) {
    return;
  ...
  PrefService* prefService = profile->GetPrefs();
  NSDate* lastShownDate =
    [VivaldiSyncErrorDialogPrefs
      getLastSyncErrorDialogShownDateWithPrefService:prefService];
  BOOL shouldAskUserAgain =
      [VivaldiSyncErrorDialogPrefs
        shouldAskUserAgainWithPrefService:prefService];
  if (lastShownDate) {
    if ([[NSCalendar currentCalendar]
         isDate:lastShownDate inSameDayAsDate:[NSDate date]]) {
      return;
    }
  }
  ...
  // Resets the ask user again flag if sync is enabled
  // and engine is initialized
  if (syncService->IsSyncFeatureActive() &&
      syncService->IsEngineInitialized()) {
    PrefService* prefs = profile->GetPrefs();
    [VivaldiSyncErrorDialogPrefs
      setShouldAskUserAgain:YES withPrefService:prefs];
    [VivaldiSyncErrorDialogPrefs
      setLastSyncErrorDialogShownDate:nil withPrefService:prefs];
  }
  // !shouldAskUserAgain should be after syncService->IsSyncFeatureActive() &
  // syncService->IsEngineInitialized(), because in case if it reset
  // We need to show again the dialog
  if (!shouldAskUserAgain) {
    return;
  }
  vivaldi::VivaldiAccountManager* _vivaldiAccountManager =
    vivaldi::VivaldiAccountManagerFactory::GetForProfile(profile);
  ...
  if (syncManager.shouldShowSyncErrorDialog) {
    _syncErrorDialogCoordinator =
        [[VivaldiSyncErrorDialogCoordinator alloc]
            initWithBaseViewController:self.activeViewController
                               browser:mainBrowser];
    _syncErrorDialogCoordinator.delegate = self;
    [_syncErrorDialogCoordinator start];
  }
}
// End Vivaldi

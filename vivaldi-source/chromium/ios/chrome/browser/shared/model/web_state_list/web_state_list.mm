...
int WebStateList::SetWebStatePinnedAtImpl(int index, bool pinned) {
  ...
  if (vivaldi::IsVivaldiRunning()) {
    // Note: (prio@vivaldi.com) When unpinning keep the unpinned tab
    // in the current index to match behaviour from Desktop.
    if (pinned) {
      // When pinning, move to the end of pinned tabs (standard behavior)
      new_index = pinned_tabs_count_;
      // Ensure new_index doesn't exceed total count
      new_index = std::min(new_index, count() - 1);
    } else {
      // When unpinning, we need to handle potential inconsistent states
      // If the tab is currently in the pinned section (index < pinned_tabs_count_)
      if (index < pinned_tabs_count_) {
        // For unpinning tabs in the middle of pinned section, we need to ensure
        // all pinned tabs remain contiguous at the beginning of the list
        // So we move the tab to the first unpinned position
        new_index = pinned_tabs_count_ - 1;
        // If all tabs are pinned, move to the end
        if (pinned_tabs_count_ == count()) {
          new_index = count() - 1;
        }
      } else {
        // For tabs that are already in the unpinned section,

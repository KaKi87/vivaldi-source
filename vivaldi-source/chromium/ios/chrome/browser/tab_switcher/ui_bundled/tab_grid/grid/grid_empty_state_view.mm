...
@property (assign, nonatomic) BOOL isSyncingInProgres;
// End Vivaldi
...
- (void)setAuthBottonsHidden:(BOOL)hidden {
  ...
  _syncButtonSyncingStateWidthConstraint.active = hidden;
}
...
- (void)setEnableSyncButtonWithSyncingState:(BOOL)isSyncing {
  if (self.isSyncingInProgres == isSyncing)
    return;
  self.isSyncingInProgres = isSyncing;

  _syncButtonSyncingStateWidthConstraint.constant = isSyncing ?
      vRecentTabsEmptyStateLoginButtonHeight :
          vRecentTabsEmptyStateEnableSyncButtonWidth;

...
TabGridPage GetPageFromScrollView(UIScrollView* scrollView) {
  ...
  if (UseRTLLayout()) {
    ...
    // Vivaldi
    if (IsVivaldiRunning()) {
      tabGridPage = static_cast<TabGridPage>(TabGridPageClosedTabs - page);
    } else
    // End Vivaldi
...
@property(nonatomic, strong)
    DisabledGridViewController* closedTabsDisabledViewController;
// End Vivaldi
  ...
  if (self) {
    ...
    if (IsVivaldiRunning()) {
      if (_pageConfiguration == TabGridPageConfiguration::kIncognitoPageOnly) {
        ...
        _closedTabsDisabledViewController = [[DisabledGridViewController alloc]
            initWithPage:TabGridPageClosedTabs];
        _closedTabsDisabledViewController.delegate = self;

      } else {
        _remoteTabsViewController =
            [[RecentTabsTableViewController alloc] init];
        _closedTabsViewController =
            [[RecentTabsTableViewController alloc] init];
      }
    } // End Vivaldi
  }
...
- (void)setupChildViewsIfNeeded {
  ...
  if (IsVivaldiRunning()) {
    if (_pageConfiguration == TabGridPageConfiguration::kIncognitoPageOnly) {
      ...
      [self setupDisabledClosedTabsViewController];
    } else {
  ...
  if (IsVivaldiRunning()) {
    [self updateToolbarBackgroundGradientForCurrentPage];
  } // End Vivaldi
  ...
  if (IsVivaldiRunning()) {
    if (IsPinnedTabsEnabled()) {
      const BOOL pinnedTabsAvailable =
          ...
          _mode == TabGridMode::kNormal;
      BOOL shouldDeferPinnedTabsUpdate = pinnedTabsAvailable &&
          (self.scrollView.dragging || self.scrollView.decelerating ||
           self.scrollViewAnimatingContentOffset);
      if (!shouldDeferPinnedTabsUpdate) {
        if (pinnedTabsAvailable) {
          [self updatePinnedTabsViewControllerConstraints];
    ...
    [self updateInactiveTabsFadeVisibility];
    BOOL isPagingTransition =
        self.scrollView.dragging || self.scrollView.decelerating ||
        self.scrollViewAnimatingContentOffset;
    // Keep gradient and main background in sync: defer both while paging.
...
- (void)scrollToPage:(TabGridPage)targetPage animated:(BOOL)animated {
  ...
  // Vivaldi
  [self reloadRemoteTabs:targetPage scrolled:scrolled changed:changed];
  ...
  switch (self.currentPage) {
    ...
    case TabGridPageClosedTabs:
      return self.closedTabsViewController
                 ...
                 : self.closedTabsDisabledViewController;
    // End Vivaldi
...
- (void)setupScrollView {
  ...
  [self addChildViewController:self.closedGridContainerViewController];
  // End Vivaldi
  ...
  UIStackView* gridsStack = [[UIStackView alloc] initWithArrangedSubviews:@[
    ...
    , self.closedGridContainerViewController.view,
    // End Vivaldi
  ...
  NSArray* constraints = @[
    ...
    [self.remoteGridContainerViewController.view.widthAnchor
        constraintEqualToAnchor:self.view.widthAnchor],
    [self.closedGridContainerViewController.view.widthAnchor
        constraintEqualToAnchor:self.view.widthAnchor],
    // End Vivaldi
...
- (void)openNewTabInPage:(TabGridPage)page focusOmnibox:(BOOL)focusOmnibox {
  ...
  switch (page) {
    ...
    case TabGridPageClosedTabs:
      NOTREACHED()
          << "It is invalid to have an active tab in recently closed tabs.";
    // End Vivaldi
...
- (void)gridViewController:(BaseGridViewController*)gridViewController
  ...
  if ((isOnRegularTabsPage && !gridIsRegularTabs) ||
      (isOnIncognitoTabsPage && !gridIsIncognitoTabs) ||
      isOnTabGroupsPage || isOnRecentTabs) {
    return;
  }
#else
  if ((isOnRegularTabsPage && !gridIsRegularTabs) ||
...
- (void)validateCommand:(UICommand*)command {
  ...
  if (command.action == @selector(keyCommand_select1)) {
    ...
    if (IsVivaldiRunning()) {
      command.image = CustomSymbolWithConfiguration(vMenuPrivateTab, nil);
    } // End Vivaldi
  }
  ...
  if (command.action == @selector(keyCommand_select4)) {
    newTitle =
        l10n_util::GetNSStringWithFixup(
           IDS_IOS_KEYBOARD_GO_TO_SYNCED_TABS_LIST);
  }
  if (command.action == @selector(keyCommand_select5)) {
    newTitle =
        l10n_util::GetNSStringWithFixup(
            IDS_IOS_KEYBOARD_GO_TO_CLOSED_TABS_LIST);
  }
  // End Vivaldi
    ...
    if (compactLayout) {
      [pinnedTabsConstraints addObjectsFromArray:@[
        [pinnedView.leadingAnchor
            constraintEqualToAnchor:self.view.leadingAnchor
                           constant:vPinnedViewHorizontalPadding],
...
- (void)vivaldiHandleScrollCompletionForScrollView:(UIScrollView*)scrollView {
  ...
  const BOOL pinnedTabsAvailable =
      self.currentPage == TabGridPageRegularTabs &&
      _mode == TabGridMode::kNormal;
  if (IsPinnedTabsEnabled() && !pinnedTabsAvailable) {
    self.pinnedTabsViewController.view.alpha = 0.0;
    self.pinnedTabsViewController.view.hidden = YES;
  }
  [self vivaldiSetDeferToolbarPageUpdates:NO];
  if (IsPinnedTabsEnabled()) {
    if (pinnedTabsAvailable) {
...
- (void)reloadRemoteTabs:(TabGridPage)targetPage
...
- (void)setupRemoteTabsViewController {
  ...
  self.remoteTabsViewController.emptyStateView =
      [[TabGridEmptyStateView alloc] initWithPage:TabGridPageRemoteTabs];
  self.remoteTabsViewController.emptyStateView.accessibilityIdentifier =
...
- (void)setupClosedTabsViewController {
  ...
  self.closedTabsViewController.emptyStateView =
      [[TabGridEmptyStateView alloc] initWithPage:TabGridPageClosedTabs];
  self.closedTabsViewController.emptyStateView.accessibilityIdentifier =
...
- (void)setClosedTabsViewController:
    (RecentTabsTableViewController*)closedTabsViewController {
  _closedTabsViewController = closedTabsViewController;
  _closedTabsViewController.view.accessibilityElementsHidden =
      self.currentPage != TabGridPageClosedTabs;
}

- (void)setClosedTabsDisabledViewController:
    (DisabledGridViewController*)closedTabsDisabledViewController {
  _closedTabsDisabledViewController = closedTabsDisabledViewController;
  _closedTabsDisabledViewController.view.accessibilityElementsHidden =
      self.currentPage != TabGridPageClosedTabs;
}

// Adds a DisabledGridViewController as a contained view controller for the
// closed tabs.
- (void)setupDisabledClosedTabsViewController {
  self.closedGridContainerViewController.containedViewController =
      self.closedTabsDisabledViewController;
  self.closedTabsDisabledViewController.delegate = self;
...
- (void)setInsetForRemoteTabs {
  ...
  CGFloat bottomInset = self.bottomToolbar.intrinsicContentSize.height;
  UIEdgeInsets inset = UIEdgeInsetsMake(
     kTabGridTopToolbarHeight, 0, bottomInset, 0);
  // Left and right side could be missing correct safe area
  // inset upon rotation. Manually correct it.
...
// Forces dark toolbar edge-effect gradient only on the incognito page.
- (void)updateToolbarBackgroundGradientForCurrentPage {
  if (!@available(iOS 26, *)) {
    return;
  }
  BOOL forceDarkModeGradient = self.currentPage == TabGridPageIncognitoTabs;
  _topToolbarBackground.forceDarkModeGradient = forceDarkModeGradient;
  _bottomToolbarBackground.forceDarkModeGradient = forceDarkModeGradient;
}

...
- (void)prepareLayout {
  ...
  if (IsVivaldiRunning()) {
    horizontalPaddingSum = 2 * vPinnedCellHorizontalLayoutInsets;
  } // End Vivaldi
  ...
  if (IsVivaldiRunning()) {
    self.itemSize = CGSize{itemWidth, vPinnedCellHeight};
  } else {
  ...
  if (IsVivaldiRunning()) {
    availableHorizontalLayoutInsets =
        MAX(availableHorizontalLayoutInsets, vPinnedCellHorizontalLayoutInsets);
  } else {
  availableHorizontalLayoutInsets =

...
void TabSnapshotAndFaviconConfigurator::FetchSnapshotAndFaviconFromWebState(
  ...
  if (IsUrlNtp(url)) {
    tab_snapshot_and_favicon.favicon =
        IsVivaldiRunning() ? [UIImage imageNamed:vToolbarMenu] :
  ...
  UIImage* default_favicon =
      IsVivaldiRunning() ? [UIImage imageNamed:vNTPSDFallbackFavicon] :
...
void TabSnapshotAndFaviconConfigurator::
  ...
  if (IsVivaldiRunning())
    default_favicon = [UIImage imageNamed:vNTPSDFallbackFavicon]; // End Vivaldi

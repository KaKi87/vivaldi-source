...
- (void)initializeModel {
  ...
  if (IsVivaldiRunning()) {
    ...
    self.addPageToAction.submenuExpanded = YES;
...
- (void)updateModelItemsState {
  ...
  if (IsVivaldiRunning()) {
    ...
    // Keep the edit actions collapsed, but add actions should stay expanded
    // so that users can find the way to add pages to bookmarks or startpage
    // easily.
    if ([self.editPageAction.submenuActions count] == 0) {
      self.editPageAction.submenuExpanded = NO;
    }
...
- (OverflowMenuDestination*)destinationForDestinationType:
  ...
  switch (destinationType) {
    ...
    case overflow_menu::Destination::History:
      ...
      if (IsVivaldiRunning())
        return self.historyDestination; // End Vivaldi
...
- (void)shareThisPage {
  ...
  if (IsVivaldiRunning()) {
    UIView* toolMenuView = self.baseViewController.presentedViewController
                               .popoverPresentationController.sourceView;
    if (toolMenuView == self.baseViewController.view || !toolMenuView.window) {
      toolMenuView = VivaldiFindToolsMenuAnchorView(
          _layoutGuideCenter, vToolsMenuGuide, kToolsMenuGuide);
    }

    self.menuHasBeenDismissed = YES;
    id<ActivityServiceCommands> activityServiceHandler =
        self.activityServiceHandler;
    VivaldiDismissPopupMenuAndRunCompletion(self.popupMenuHandler, YES, ^{
      [activityServiceHandler showShareSheetFromShareButton:toolMenuView];
    });
    return;
  } // End Vivaldi
...
- (void)showShareSheetForChromeApp {
  ...
  if (IsVivaldiRunning()) {
    ...
    id<ActivityServiceCommands> activityServiceHandler =
        self.activityServiceHandler;
    VivaldiDismissPopupMenuAndRunCompletion(self.popupMenuHandler, YES, ^{
      [activityServiceHandler showShareSheetForChromeApp];
    });
    return;
  } // End Vivaldi
                            ...
                            overflow_menu::ActionType::vAddPageTo
                             ...
                             monochromeSymbol:NO
                              accessibilityID:vToolsMenuAddPageToId
                                 hideItemText:hideItemText
                                      handler:^{
                                        [weakSelf toggleAddPageToSubmenu];
                                      }];
}
                            ...
                            overflow_menu::ActionType::vEditPage
                             ...
                             monochromeSymbol:NO
                              accessibilityID:vToolsMenuEditPageId
                                 hideItemText:hideItemText
                                      handler:^{
                                        [weakSelf toggleEditPageSubmenu];
                                      }];
}
...
- (void)toggleAddPageToSubmenu {
  [self toggleExpandedSubmenuForAction:self.addPageToAction];
}

- (void)toggleEditPageSubmenu {
  [self toggleExpandedSubmenuForAction:self.editPageAction];
}

- (void)toggleExpandedSubmenuForAction:(OverflowMenuAction*)action {
  if ([action.submenuActions count] == 0) {
    return;
  }
  action.submenuExpanded = !action.submenuExpanded;
  [self.menuOrderer updatePageActions];
}
...
- (void)showSharePage {
  ...
  id<ActivityServiceCommands> activityServiceHandler =
      self.activityServiceHandler;
  VivaldiDismissPopupMenuAndRunCompletion(self.popupMenuHandler, YES, ^{
    [activityServiceHandler showShareSheetFromShareButton:nil];
  });
}

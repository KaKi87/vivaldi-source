...
- (void)updateDestinationsPrefIfNeeded {
  ...
  // Convert back to base::ListValue and write updates
  base::ListValue updatedShownDestinations;
  for (const auto& destination : currentDestinations) {
    updatedShownDestinations.Append(
        overflow_menu::StringNameForDestination(destination));
  }

  base::ListValue updatedHiddenDestinations;
...
- (void)loadVivaldiActionsFromPrefs {
  ...
  if (shownActions) {
    for (const auto& value : *shownActions) {
      ...
      vivaldiActionOrderData.shownActions.push_back(*actionType);
    }
  }
  ...
  if (hiddenActions) {
    for (const auto& value : *hiddenActions) {
      ...
      vivaldiActionOrderData.hiddenActions.push_back(*actionType);
    }
  }

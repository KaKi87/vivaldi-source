...
- (void)didFinishFirstRun {
  ...
  if (vivaldi::IsVivaldiRunning()) {
    // NOTE(tomas@vivaldi.com): VIB-1376 We never show the chrome FRE so our
    // finishing procedure is different and must happen first.
    [self.profileState queueTransitionToNextInitStage];

    [[NSNotificationCenter defaultCenter]
         postNotificationName:
              [VivaldiOnboardingNotifications didFinishOnboardingKey]
                       object:nil
                     userInfo:nil];
  } // End Vivaldi

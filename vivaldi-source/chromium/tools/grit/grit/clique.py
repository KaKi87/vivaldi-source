...
class UberClique:
  ...
  def FindCliqueAndAddTranslation(self, translation, language, gender, override_exisiting=False):
    ...
    if translation.GetId() in self.cliques_:
      for clique in self.cliques_[translation.GetId()]:
        clique.AddTranslation(translation, language, gender, override_exisiting=override_exisiting)
      return True
    else:
      return False
    ...
    def Callback(id, structures_by_gender):
      if id not in self.cliques_:
        if self.keep_additional_translations_:
          self.additional_translations_.setdefault(lang, {})[id] = structures_by_gender
        if debug:
      ...
      for (gender, structure) in structures_by_gender.items():
        ...
        self.FindCliqueAndAddTranslation(translation, lang, gender, override_exisiting=override_exisiting)
    return Callback
...
class MessageClique:
  ...
  def AddTranslation(self, translation, language, gender, override_exisiting=False):
    ...
    assert not (language, gender) in self.clique or override_exisiting, "%s already loaded"% language

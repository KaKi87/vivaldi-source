...
void MenuController::OnMouseMoved(SubmenuView* source,
  ...
  if (vivaldi::IsVivaldiRunning() && VivaldiHandleSynthesizedEvent(event))
    return;
...
void MenuController::SetSelection(MenuItemView* menu_item,
  ...
  if (menu_item && menu_item->GetDelegate() && vivaldi::IsVivaldiRunning())
...
bool MenuController::OnKeyPressed(const ui::KeyEvent& event) {
  ...
  switch (key_code) {
    ...
    case ui::VKEY_RIGHT:
      ...
      if (vivaldi::IsVivaldiRunning() && VivaldiHandleKeyPressed(key_code)) {
        break;
      } // Vivaldi
...
// On Mac, treat space the same as return.
  ...
  if (vivaldi::IsVivaldiRunning()) {
    // We want Alt to close the Vivaldi menu as under Windows. For consistency
    // we enable it for all menus. VB-39080
    switch (key_code) {
      case ui::VKEY_MENU:
      case ui::VKEY_F10:
        Cancel(ExitType::kAll);
...
bool MenuController::ShowSiblingMenu(SubmenuView* source,
  ...
  MenuItemView* alt_menu = vivaldi::IsVivaldiRunning() ?
      source->GetMenuItem()->GetDelegate()->GetVivaldiSiblingMenu(
          ...
          &vivaldi_rect, &has_mnemonics, &anchor) :
  ...
  if (vivaldi::IsVivaldiRunning()) {
    did_capture_ = false;
    UpdateInitialLocation(vivaldi_rect, anchor, state_.menu_type);
  } else {
...
gfx::Rect MenuController::CalculateMenuBounds(
  ...
  *resulting_direction = preferred_open_direction;
  ...
  if (is_child_menu) {
    ...
    if (item->GetDelegate()->VivaldiShouldTryPositioningContextMenu()) {
      item->GetDelegate()->VivaldiGetContextMenuPosition(&menu_bounds,
        monitor_bounds, anchor_bounds);
    }
    ...
    } else if (item->GetDelegate()->VivaldiShouldTryPositioningInMenuBar()) {
      VivaldiAdjustMenubarMenuGeometry(&menu_bounds, monitor_bounds,
                                       anchor_bounds);
    } else {

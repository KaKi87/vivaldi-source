...
char16_t MenuItemView::GetMnemonic() {
  if (vivaldi_has_local_mnemonics_info_) {
    if (!has_mnemonics_ || !MenuConfig::instance().use_mnemonics ||
        !may_have_mnemonics()) {
      return 0;
    }
  } else {
...
int MenuItemView::GetDrawStringFlags() const {
  ...
  if (vivaldi_has_local_mnemonics_info_) {
    if (has_mnemonics_ && may_have_mnemonics()) {
...
#include "browser/menus/vivaldi_menu_item_view.inc"
...
bool ResolveTab(int tab_id, TabInfo* info) {
  CHECK(info);
  ...
  for (Browser* browser : *BrowserList::GetInstance()) {
    ...
    for (int i = 0; i < tab_strip->count(); ++i) {
      ...
      if (sessions::SessionTabHelper::IdForTab(contents).id() == tab_id) {
        info->web_contents = contents;
        info->tab_id = tab_id;
        info->index = i;
        info->tab_strip = tab_strip;
        info->type = GetVivaldiPanelType(contents);
...
bool ResolveTab(content::WebContents* contents, TabInfo* info) {
  CHECK(info);
  if (!contents) {
    return false;
  }
  for (Browser* browser : *BrowserList::GetInstance()) {
    ...
    for (int i = 0; i < tab_strip->count(); ++i) {
      if (contents == tab_strip->GetWebContentsAt(i)) {
        info->web_contents = contents;
        info->tab_id = sessions::SessionTabHelper::IdForTab(contents).id();
        info->index = i;
        info->tab_strip = tab_strip;
        info->type = GetVivaldiPanelType(contents);
        info->browser = browser;

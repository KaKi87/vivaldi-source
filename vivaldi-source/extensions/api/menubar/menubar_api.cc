...
namespace {
std::map<std::string, ItemInfo>& GetTagMap() {
  static base::NoDestructor<std::map<std::string, ItemInfo>> tag_map;
  return *tag_map.get();
}
...
int getIdByAction(const menubar::MenuItem& item) {
  ...
  auto it = GetTagMap().find(item.action);
  if (it != GetTagMap().end()) {
    GetTagMap()[item.action].enabled = enabled;
    return GetTagMap()[item.action].id;
  }
  ...
  if (item.action == "COMMAND_CLIPBOARD_UNDO")
    GetTagMap()[item.action] = {IDC_CONTENT_CONTEXT_UNDO, false, enabled};
  else if (item.action == "COMMAND_CLIPBOARD_REDO")
    GetTagMap()[item.action] = {IDC_CONTENT_CONTEXT_REDO, false, enabled};
  else if (item.action == "COMMAND_CLIPBOARD_CUT")
    GetTagMap()[item.action] = {IDC_CONTENT_CONTEXT_CUT, false, enabled};
  else if (item.action == "COMMAND_CLIPBOARD_COPY")
    GetTagMap()[item.action] = {IDC_CONTENT_CONTEXT_COPY, false, enabled};
  else if (item.action == "COMMAND_CLIPBOARD_PASTE")
    GetTagMap()[item.action] = {IDC_CONTENT_CONTEXT_PASTE, false, enabled};
  else if (item.action == "COMMAND_DELETE")
    GetTagMap()[item.action] = {IDC_CONTENT_CONTEXT_DELETE, false, enabled};
  else if (item.action == "COMMAND_CLIPBOARD_SELECT_ALL")
    GetTagMap()[item.action] = {IDC_CONTENT_CONTEXT_SELECTALL, false, enabled};
  else if (item.action == "COMMAND_HIDE_VIVALDI")
    GetTagMap()[item.action] = {IDC_HIDE_APP, false, enabled};
  else if (item.action == "COMMAND_HIDE_OTHERS")
    GetTagMap()[item.action] = {IDC_VIV_HIDE_OTHERS, true, enabled};
  else if (item.action == "COMMAND_SHOW_ALL")
    GetTagMap()[item.action] = {IDC_VIV_SHOW_ALL, true, enabled};
  // These are ids we test for in app_controller_mac.mm
  else if (item.action == "COMMAND_CHECK_FOR_UPDATES")
    GetTagMap()[item.action] = {IDC_VIV_CHECK_FOR_UPDATES, item.with_no_window,
                                enabled};
  else if (item.action == "COMMAND_QUIT_MAC_MAYBE_WARN")
    GetTagMap()[item.action] = {IDC_VIV_EXIT, item.with_no_window, enabled};
  else if (item.action == "COMMAND_NEW_WINDOW")
    GetTagMap()[item.action] = {IDC_VIV_NEW_WINDOW, item.with_no_window,
                                enabled};
  else if (item.action == "COMMAND_NEW_PRIVATE_WINDOW")
    GetTagMap()[item.action] = {IDC_VIV_NEW_PRIVATE_WINDOW, item.with_no_window,
                                enabled};
  else if (item.action == "COMMAND_NEW_PRIVATE_WINDOW")
    GetTagMap()[item.action] = {IDC_VIV_NEW_PRIVATE_WINDOW, item.with_no_window,
                                enabled};
  else if (item.action == "COMMAND_NEW_TAB")
    GetTagMap()[item.action] = {IDC_VIV_NEW_TAB, item.with_no_window, enabled};
  else if (item.action == "COMMAND_CLOSE_WINDOW")
    GetTagMap()[item.action] = {IDC_VIV_CLOSE_WINDOW, item.with_no_window,
                                enabled};
  else if (item.action == "COMMAND_CLOSE_TAB")
    GetTagMap()[item.action] = {IDC_VIV_CLOSE_TAB, item.with_no_window,
                                enabled};
  else if (item.action == "COMMAND_WINDOW_MINIMIZE")
    GetTagMap()[item.action] = {IDC_VIV_MAC_MINIMIZE, item.with_no_window,
                                enabled};
  else if (item.action == "COMMAND_SHOW_HELP")
    GetTagMap()[item.action] = {IDC_VIV_SHOW_HELP, item.with_no_window,
                                enabled};
  // Some menus
  ...
  // And containers
    ...
    GetTagMap()[item.action] = {IDC_VIV_MAC_SERVICES, item.with_no_window,
                                enabled};
  else if (item.action == "CONTAINER_SHARE_MENU")
    GetTagMap()[item.action] = {IDC_VIV_SHARE_MENU_MAC, item.with_no_window,
                                enabled};
  else if (item.action == "CONTAINER_BOOKMARK") {
    GetTagMap()[item.action] = {IDC_VIV_BOOKMARK_CONTAINER, item.with_no_window,
                                enabled};
  }
  // And something we could do better
  else if (item.action == "JS_LOCAL_ADD_ACTIVE_TAB_TO_BOOKMARKS")
    GetTagMap()[item.action] = {IDC_VIV_ADD_ACTIVE_TAB_TO_BOOKMARKS,
                                item.with_no_window, enabled};
  else
    GetTagMap()[item.action] = {tag_counter++, item.with_no_window, enabled};

  return GetTagMap()[item.action].id;
}

bool SetIds(std::vector<menubar::MenuItem>& items, bool addFixedActions) {
  size_t map_size = GetTagMap().size();
  ...
  return map_size != GetTagMap().size();
}
}  // namespace

std::string GetActionById(int id) {
  for (auto it = GetTagMap().begin(); it != GetTagMap().end(); ++it) {
    if (it->second.id == id) {
      return it->first;
    }
  }
...
ExtensionFunction::ResponseAction MenubarGetHasWindowsFunction::Run() {
  int count =
      VivaldiBrowserComponentWrapper::GetInstance()->BrowserListGetCount();
  return RespondNow(ArgumentList(
      vivaldi::menubar::GetHasWindows::Results::Create(count > 0)));
}

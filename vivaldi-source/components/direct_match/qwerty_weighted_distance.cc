...
namespace {
constexpr auto kNeighbors = base::MakeFixedFlatMap<char, std::string_view>({
    {'q', "wa"},     {'w', "esaq"},   {'e', "rdsw"},   {'r', "tfde"},
    {'t', "ygfr"},   {'y', "uhgt"},   {'u', "ijhy"},   {'i', "okju"},
    {'o', "plki"},   {'p', "lo"},     {'a', "qwsz"},   {'s', "wedxza"},
    {'d', "erfcxs"}, {'f', "rtgvcd"}, {'g', "tyhbvf"}, {'h', "yujnbg"},
    {'j', "uikmnh"}, {'k', "iolmj"},  {'l', "opk"},    {'z', "asx"},
    {'x', "sdcz"},   {'c', "dfvx"},   {'v', "fgbc"},   {'b', "ghnv"},
    {'n', "hjmb"},   {'m', "jkn"},
});
}
...
void QwertyWeightedDistance::InitializeQwertyCosts() {
  ...
  for (const auto& entry : kNeighbors) {
    char ch = entry.first;
...
float QwertyWeightedDistance::BaseDistanceSimilarity(
  ...
  float distance =
      GetDistance(text, typed_text, qwerty_cost_, use_damerau_distance);
  if (!similarity) {
    return distance;
  }
...
float QwertyWeightedDistance::GetDistance(
  ...
  for (int i = 1; i <= text_len; ++i) {
    for (int j = 1; j <= typed_text_len; ++j) {
      ...
      if (use_damerau_distance && i > 1 && j > 1 &&

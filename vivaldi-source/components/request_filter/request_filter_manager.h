 ...
 public:
  ...
  class RequestIDGenerator {
   public:
    ...
    // Generates a Filtered Request ID. If `SaveID()` was previously called with
    // the same (`routing_id`, `request_id_from_client`) pair, returns the saved
    // ID and removes the mapping. Otherwise, generates and returns a new unique
    // ID.
    uint64_t Generate(int32_t routing_id, int32_t request_id_from_client);

    // Maps a Filtered Request ID to a (`routing_id`, `request_id_from_client`)
    // pair when a request is restarted. Callers must subsequently call
    // `Generate()` with the same pair to reclaim the ID and prevent memory
    // leaks.

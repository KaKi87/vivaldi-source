...
RuleBufferHolder::RuleBufferHolder(uint32_t source_id,
...
const re2::RE2& RuleBufferHolder::GetRegexForPattern(
    const flatbuffers::String* flat_string) const {
  const auto& hit = regex_cache_.find(flat_string);
  if (hit != regex_cache_.end()) {
    return *hit->second;
  }

  auto regex = std::make_unique<re2::RE2>(flat_string->string_view());

  // Invalid patterns would have been removed at parsing time
  CHECK(regex->ok());

  auto result = regex_cache_.try_emplace(flat_string, std::move(regex));
  // Insertion should succeed. We just tested whether this element was presetm
  CHECK(result.second);
...
RulesIndexManager::RulesIndexManager(
    ...
    : group_(group),
      ...
      rule_sources_(rule_manager->GetRuleSources(group)),
...
void RulesIndexManager::Shutdown() {
  rule_manager_->RemoveObserver(this);
  rules_index_loaded_callback_.Reset();
  rules_index_change_callback_.Reset();
}
...
void RulesIndexManager::OnRulesRead(uint32_t source_id,
  ...
  rules_buffers_[source_id] = std::make_unique<RuleBufferHolder>(
      source_id, std::move(rules_buffer), checksum);
...
void RulesIndexManager::OnIndexRead(std::string index_buffer) {
  ...
  if (rules_index_loaded_callback_) {
...
void RulesIndexManager::ReadIndex(const std::string& checksum) {
  ...
  if (rules_index_change_callback_) {

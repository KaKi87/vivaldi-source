...
bool DoesTextMatchLastSubpatternInternal(uint8_t anchor_type,
  ...
  const size_t host_end = static_cast<size_t>(url_host.end());

  // Enumerate all possible combinations of |anchor_left| and |anchor_right|.
  if (anchor_type == flat::AnchorType_NONE) {
    size_t position = FindSubpattern(text, subpattern);
    return position != std::string_view::npos &&
           (!must_intersect_host ||
            (position <= host_end && position + subpattern.size() > host_end));
  }

  if (anchor_type == flat::AnchorType_END) {
    if (must_intersect_host &&
        subpattern.size() < base::CheckSub(url_size, host_end).ValueOrDie()) {
      return false;
    }
  ...
  if (anchor_type == flat::AnchorType_START) {
    if (must_intersect_host && subpattern.size() <= host_end) {
      return false;
    }
  ...
  if (anchor_type == flat::AnchorType_HOST) {
    if (must_intersect_host && subpattern.empty()) {
      return false;
    }
...
bool DoesTextMatchLastSubpattern(uint8_t anchor_type,
  ...
  if (DoesTextMatchLastSubpatternInternal(anchor_type, url_size, text, url_host,
                                          subpattern, must_intersect_host)) {
    return true;
  }
  ...
  if (subpattern.back() == url_pattern_index::kSeparatorPlaceholder) {
    ...
    return DoesTextMatchLastSubpatternInternal(
        anchor_type | flat::AnchorType_END, url_size, text, url_host,
        subpattern, must_intersect_host);
  }
...
bool IsCaseSensitiveMatch(std::string_view url_pattern,
  ...
  if (subpattern_it == subpattern_end) {
    return DoesTextMatchLastSubpattern(anchor_type, url_spec.size(), url_spec,
                                       url_host, subpattern,
                                       must_intersect_host);
  }
  ...
  while (++subpattern_it != subpattern_end) {
    ...
    must_intersect_host =
        must_intersect_host && (match_position > host_end ||
                                match_position + subpattern.size() < host_end);
  ...
  return DoesTextMatchLastSubpattern(anchor_type & flat::AnchorType_END,
                                     url_spec.size(), text, url_host,
                                     subpattern, must_intersect_host);
}

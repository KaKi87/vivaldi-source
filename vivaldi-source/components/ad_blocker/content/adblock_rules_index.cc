...
#include "components/ad_blocker/content/adblock_rule_pattern_matcher.h"
#include "components/ad_blocker/content/stylesheet_builder.h"
...
#include "vivaldi/components/ad_blocker/content/flat/adblock_rules_index_generated.h"
...
bool AddActivationsFromRule(RulesIndex::ActivationResults& activations,
  ...
  for (uint8_t i = 1; i < flat::ActivationType_ANY; i <<= 1) {
    ...
    if (existing.priority > GetRulePriority(*rule_and_source.rule)) {
      continue;
    }
    existing.rule_details.emplace();
    existing.rule_details->decision = rule_and_source.rule->decision();
    existing.rule_details->source_id = rule_and_source.source_id;
    result = true;
  }
...
void GetActivationsFromCandidates(
  ...
  for (const flat::RuleId* rule_id : *candidates->Get(0)->rules()) {
    ...
    if (!DoesUrlMatchRulePattern(rule, url, false)) {
      continue;
    }
...
std::optional<RulesIndex::RuleAndSource> FindMatchAmongCandidates(
    ...
    const PartyMatcher& party_matcher,
    bool disable_generic_rules,
    std::optional<RulesIndex::AdAttributionMatches> ad_attribution_matches,
  ...
  for (const flat::RuleId* rule_id : *sorted_candidates) {
    ...
    if (!DoesUrlMatchRulePattern(rule, url, must_intersect_host)) {
      continue;
    }
    ...
    if (rule.ad_domains_and_query_triggers()) {
      ...
      for (const auto* ad_domain_and_query_trigger :
        ...
        if (ad_attribution_matches && !ad_attribution_matches->is_null()) {
...
RulesIndex::ActivationResults RulesIndex::FindActivations(
    base::RepeatingCallback<bool(url::Origin)> is_origin_wanted,
    const url::Origin& parent_origin,
    const GURL& url) {
  CHECK(url.is_valid() && CanFilterUrl(url));

  ActivationResults activations;

  RulePatternMatcher::UrlInfo url_info(url);
  PartyMatcher party_matcher = GetPartyMatcher(url, parent_origin);

  auto handle_matches = [this, &activations, &url_info, parent_origin,
                         party_matcher](
                            const FlatRulesByModifierList* rule_list) {
    GetActivationsFromCandidates(rule_list, rules_buffers_, url_info,
                                 parent_origin, party_matcher, &activations);
    return false;
  };
  ...
  if (!is_origin_wanted.Run(url::Origin::Create(url))) {
...
RulesIndex::FindMatchingBeforeRequestRule(
  ...
  auto handle_matches = [this, &result, &url_info, must_intersect_host,
    ...
    std::optional<RulesIndex::RuleAndSource> rule_and_source =
        FindMatchAmongCandidates(rule_list, rules_buffers_, url_info,
                                 must_intersect_host, document_origin,
                                 resource_type, party_matcher,
                                 disable_generic_rules, ad_attribution_matches,

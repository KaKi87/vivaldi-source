...
std::ostream& operator<<(std::ostream& os, const RequestFilterRule& rule) {
  ...
  os << "\n"
     ...
     << "Decision:" << static_cast<int>(rule.decision) << "\n"
     << std::setw(kAlignemntPosition) << "Modify block:" << rule.modify_block
     << "\n"
     << std::setw(kAlignemntPosition)
     << "Modifier:" << static_cast<int>(rule.modifier) << "\n"
  ...
  os << std::setw(kAlignemntPosition) << PatternTypeToString(rule.pattern_type)
     ...
     << std::setw(kAlignemntPosition) << "Party:"
     << (rule.party.has_value() ? static_cast<int>(*rule.party) : -1) << "\n"
     << std::setw(kAlignemntPosition)
     << "Method:" << rule.request_methods.ToString() << "\n"
     << std::setw(kAlignemntPosition)
     << "Resources:" << rule.resource_types.ToString() << "\n"
     << std::setw(kAlignemntPosition)
     << "Explicit resources:" << rule.explicit_types.ToString() << "\n"
     << std::setw(kAlignemntPosition)
     << "Activations:" << rule.activation_types.ToString() << "\n"
     << std::setw(kAlignemntPosition)

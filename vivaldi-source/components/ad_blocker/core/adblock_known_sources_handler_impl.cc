...
bool KnownRuleSourcesHandlerImpl::EnableSource(RuleGroup group,
  ...
  bool result = rule_manager_->AddRulesSource(group, known_source->second.core);
...
void KnownRuleSourcesHandlerImpl::DisableSource(RuleGroup group,
  ...
  rule_manager_->DeleteRuleSource(group, known_source->second.core);
...
bool KnownRuleSourcesHandlerImpl::IsSourceEnabled(RuleGroup group,
                                                  uint32_t source_id) {
  return rule_manager_->GetRuleSource(group, source_id).has_value();
}

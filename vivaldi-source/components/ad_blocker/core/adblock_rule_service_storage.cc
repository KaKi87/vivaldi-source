...
std::optional<RuleSourceCore> LoadRuleSourceCore(
  ...
  settings.allow_abp_snippets =
      source_dict.FindBool(kAllowAbpSnippets).value_or(false);
  settings.naked_hostname_is_pure_host =
      source_dict.FindBool(kNakedHostnameIsPureHost).value_or(true);
  settings.use_whole_document_allow =
      source_dict.FindBool(kUseWholeDocumentAllow).value_or(true);
  settings.allow_attribution_tracker_rules =
      source_dict.FindBool(kAllowAttributionTrackerRules).value_or(false);
  settings.pure_host_is_document_block =
      source_dict.FindBool(kPureHostIsDocumentBlock).value_or(true);
...
RuleServiceStorageDelegate::LoadResult DoLoad(const base::FilePath& path) {
  RuleServiceStorageDelegate::LoadResult load_result;
...
base::Value::Dict SerializeRuleGroup(RuleServiceStorageDelegate* delegate,
  ...
  rule_group.Set(kProcessListKey, SerializeStringSetToList(
                                      delegate->GetRuleManager()->GetExceptions(
  ...
  rule_group.Set(
      kRuleSourcesKey,
      SerializeSourcesList(delegate->GetRuleManager()->GetRuleSources(group)));
  rule_group.Set(kKnownSourcesKey,
                 SerializeKnownSourcesList(
                     delegate->GetKnownSourcesHandler()->GetSources(group)));
  rule_group.Set(
      kDeletedPresetsKey,
      SerializeUuidSetToList(
          delegate->GetKnownSourcesHandler()->GetDeletedPresets(group)));
  rule_group.Set(kIndexChecksum, delegate->GetRulesIndexChecksum(group));
...
void RuleServiceStorage::OnLoadFinished(
    RuleServiceStorageDelegate::LoadResult load_result) {
  delegate_->OnStorageDoneLoading(std::move(load_result));
}
...
std::optional<std::string> RuleServiceStorage::SerializeData() {
  ...
  root.Set(kAdBlockingRulesKey,
           SerializeRuleGroup(delegate_, RuleGroup::kAdBlockingRules));

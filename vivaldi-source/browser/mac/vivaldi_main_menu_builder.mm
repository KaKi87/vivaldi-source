...
NSMenuItem* BuildAppMenu(NSApplication* nsapp, AppController* app_controller) {
      ...
      Item(IDS_FILE_MENU_MAC)
          ...
          .submenu({
            Item(IDS_NEW_TAB_MAC).command_id(IDC_NEW_TAB),
                ...
                Item().is_separator(), Item(IDS_SHARE_MAC).tag(-1).submenu({}),
      ...
      Item(IDS_EDIT_MENU_MAC)
          ...
          .submenu({
            Item(IDS_EDIT_UNDO_MAC)
                ...
                Item(IDS_SPEECH_MAC).tag(IDC_VIV_SPEECH_MENU_MAC).submenu({
                ...
                Item().is_separator().tag(IDC_VIV_EDIT_SEPARATOR_MAC),
...
NSMenuItem* BuildViewMenu(NSApplication* nsapp, AppController* app_controller) {
  NSMenuItem* item =
...
NSMenuItem* BuildToolsMenu(NSApplication* nsapp,
                           AppController* app_controller) {
  NSMenuItem* item =
      Item(IDS_VIV_TOOLS_MENU).tag(IDC_VIV_TOOLS_MENU).submenu({}).Build();
  return item;
}
...
NSMenuItem* BuildHelpMenu(NSApplication* nsapp, AppController* app_controller) {
  std::u16string productname(l10n_util::GetStringUTF16(IDS_PRODUCT_NAME));

  NSMenuItem* item = Item(IDS_HELP_MENU_MAC)

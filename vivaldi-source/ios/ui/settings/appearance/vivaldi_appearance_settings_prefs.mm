...
+ (void)registerBrowserStatePrefs:(user_prefs::PrefRegistrySyncable*)registry {
  ...
  registry->RegisterIntegerPref(vivaldiprefs::kVivaldiWebsiteAppearanceStyle,
...
+ (void)setBrowserTheme:(NSString*)theme {
  PrefService* prefService = [VivaldiAppearanceSettingPrefs prefService];
  prefService->SetString(vivaldiprefs::kVivaldiAppearanceMode,
...
+ (void)setForceWebsiteDarkThemeEnabled:(BOOL)enabled {
  PrefService* prefService = [VivaldiAppearanceSettingPrefs prefService];
  prefService->SetBoolean(vivaldiprefs::kVivaldiWebsiteAppearanceForceDarkTheme,
                          enabled);
}

+ (void)setCustomAccentColor:(NSString*)accentColor {
  PrefService* prefService = [VivaldiAppearanceSettingPrefs prefService];
  prefService->SetString(vivaldiprefs::kVivaldiCustomAccentColor,

...
CAGradientLayer* GetAnimatedWipeEffect(CGRect frame,
  ...
  CAGradientLayer* gradient_layer = [CAGradientLayer layer];
  gradient_layer.type = kCAGradientLayerRadial;
  UIColor* color = baseColor ?: [UIColor systemRedColor];
  gradient_layer.colors = @[
  ...
  colors_animation.toValue = @[
    ...
    (id)[color colorWithAlphaComponent:0.45].CGColor,
    (id)[color colorWithAlphaComponent:0.0].CGColor,
  ];
...
@implementation VivaldiReaderModeAnimation {
  UIView* _window;
...
- (void)animateWithCompletion:(ProceduralBlock)completion {
  ...
  __weak VivaldiReaderModeAnimation* weakSelf = self;
...
- (void)addGridCellDisapperingAnimationWithMediaTime:(CFTimeInterval)mediaTime {
  for (UIView* cell : _gridCells) {
    bool invertMaskAlpha =
        (self.type == VivaldiReaderModeAnimationType::kRevealView);
...
- (void)onAnimationCompletedWithCompletionBlock:(ProceduralBlock)completion {
  ...
  // view.
  for (UIView* cell : _gridCells) {
    cell.hidden = (self.type == VivaldiReaderModeAnimationType::kHideView);
    cell.layer.mask = nil;
  }

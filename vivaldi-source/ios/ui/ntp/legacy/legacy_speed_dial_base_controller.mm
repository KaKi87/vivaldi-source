...
#import "ios/ui/ntp/legacy/cells/legacy_speed_dial_container_cell.h"
#import "ios/ui/ntp/legacy/legacy_speed_dial_base_controller_flow_layout.h"
#import "ios/ui/ntp/legacy/legacy_speed_dial_view_controller.h"
...
- (void)viewDidLoad {
  ...
  [VivaldiStartPagePrefsHelper refreshDailyMixWallpaperForceFetch];
}
...
- (void)setupNotifications {
  ...
  [[NSNotificationCenter defaultCenter]
      addObserver:self
         selector:@selector(handleSystemWallpaperRefreshTrigger:)
             name:NSCalendarDayChangedNotification
           object:nil];
}
...
- (void)handleSystemWallpaperRefreshTrigger:(NSNotification*)notification {
  (void)notification;
...
// Updates the selected page used by the toolbar, page collection, and sorting
// actions. The mediator already decided which logical item should be selected.
- (void)applySelectedToolbarIndex:(NSInteger)index {
...
- (VivaldiNTPTopToolbarItem*)toolbarItemAtIndex:(NSInteger)index {
...
- (BOOL)toolbarItemShowsAddButton:(VivaldiNTPTopToolbarItem*)item {
  return item && item.pageType == VivaldiSpeedDialPageTypeSpeedDial;
}

- (void)refreshToolbarChrome {
  [self setUpMoreButtonProperties];
...
- (void)restorePageOffsetAfterToolbarReloadFromIndex:
            (NSInteger)previousSelectedIndex
                                             toIndex:(NSInteger)newSelectedIndex
                               previousContentOffset:
                                   (CGPoint)previousContentOffset {
  CGFloat pageWidth = self.collectionView.bounds.size.width;
  CGFloat pageDelta = (newSelectedIndex - previousSelectedIndex) * pageWidth;
  CGFloat maxOffsetX =
      MAX(0, self.collectionView.contentSize.width - pageWidth);
  CGFloat preservedOffsetX =
      MIN(MAX(previousContentOffset.x + pageDelta, 0), maxOffsetX);
  [self.collectionView
      setContentOffset:CGPointMake(preservedOffsetX, previousContentOffset.y)
              animated:NO];
}

- (void)finishToolbarReload {
  [self scrollViewDidScroll:self.collectionView];
  ...
  // Re-apply the settled page state after reload. The preserved content
  // offset can leave the add button progress in a stale in-between state.
  [self syncAddButtonVisibilityForSelectedPage];
}

- (void)reloadPagesForToolbarItemsPreservingSelectedPage:
            (BOOL)preserveSelectedPage
  ...
  [UIView performWithoutAnimation:^{
    ...
    if (preserveSelectedPage) {
      [self
          restorePageOffsetAfterToolbarReloadFromIndex:previousSelectedIndex
                                               toIndex:
                                                   self.selectedToolbarItemIndex
                                 previousContentOffset:previousContentOffset];
    } else {
      [self scrollToItemWithIndex:self.selectedToolbarItemIndex animated:NO];
    }

    [self finishToolbarReload];
  }];
}
...
- (BOOL)shouldHideToolbar {
  ...
  return ((speedDialsHidden && topSitesHidden) || belowThreshold);
}
...
- (VivaldiSpeedDialItem*)currentActivePageItem {
  VivaldiNTPTopToolbarItem* toolbarItem =
      [self toolbarItemAtIndex:self.selectedToolbarItemIndex];
  if (!toolbarItem ||
...
- (void)updateAddButtonVisibilityProgressWithCurrentPage:(CGFloat)currentPage
  ...
  VivaldiNTPTopToolbarItem* lastItem =
      [self toolbarItemAtIndex:self.lastSelectedToolbarItemIndex];
  if (lastItem && (self.scrollFromMenuTap ||
...
- (void)handleAddButtonTransitionFromPreviousToNewIndex:(NSInteger)newIndex {
  ...
  VivaldiNTPTopToolbarItem* fromItem =
      [self toolbarItemAtIndex:self.lastSelectedToolbarItemIndex];
  VivaldiNTPTopToolbarItem* toItem = [self toolbarItemAtIndex:newIndex];
  BOOL fromShowsAddButton = [self toolbarItemShowsAddButton:fromItem];
  BOOL toShowsAddButton = [self toolbarItemShowsAddButton:toItem];
  BOOL shouldAnimateTransition =
      [self showFrequentlyVisited] && [self showSpeedDials];

  // Disable animation when both pages share the same add button state.
  BOOL noAnimation = !shouldAnimateTransition ||
                     (fromShowsAddButton == toShowsAddButton) ||
                     (fromItem.pageType == VivaldiSpeedDialPageTypeAddGroup &&
                      toItem.pageType == VivaldiSpeedDialPageTypeTopSites);

  CGFloat progress = toShowsAddButton ? 1.0 : 0.0;
...
- (void)syncAddButtonVisibilityForSelectedPage {
  ...
  CGFloat progress = [self toolbarItemShowsAddButton:selectedItem] ? 1.0 : 0.0;
  [self.bottomToolbarProvider handleAddButtonVisibilityWithProgress:progress
                                                           animated:NO];
}
...
- (void)fadeInBottomToolbarIfNeeded {
  VivaldiNTPTopToolbarItem* toolbarItem =
      [self toolbarItemAtIndex:self.selectedToolbarItemIndex];
  if (!toolbarItem ||
      toolbarItem.pageType == VivaldiSpeedDialPageTypeAddGroup) {
    return;
  }
...
- (void)scrollViewDidScroll:(UIScrollView*)scrollView {
  ...
  if (!self.traitCollectionChangeInProgress) {
    ...
    if ([self showSpeedDials] || [self showFrequentlyVisited]) {
      ...
      // or hidden state.
      if ([self showFrequentlyVisited] && [self showSpeedDials]) {
...
- (void)scrollViewDidEndDecelerating:(UIScrollView*)scrollView {
  ...
  if (currentIndexPath &&
    ...
    [self applySelectedToolbarIndex:currentIndexPath.row];
    VivaldiNTPTopToolbarItem* selectedItem =
        [self.toolbarItems objectAtIndex:currentIndexPath.row];
    if (selectedItem) {
      // Swiping between root pages also counts as a real selection change.
      [self.delegate didSelectToolbarItem:selectedItem
...
- (void)refreshMenuItems:(NSArray<VivaldiNTPTopToolbarItem*>*)items
  ...
  NSInteger previousSelectedIndex = self.selectedToolbarItemIndex;
  CGPoint previousContentOffset = self.collectionView.contentOffset;
  VivaldiNTPTopToolbarItem* previousSelectedItem =
      [self toolbarItemAtIndex:self.selectedToolbarItemIndex];

  NSInteger appliedSelectedIndex =
      MAX(0, MIN(selectedIndex, (NSInteger)items.count - 1));
  self.toolbarItems = [items copy];
  VivaldiNTPTopToolbarItem* selectedItem =
      [self toolbarItemAtIndex:appliedSelectedIndex];
  BOOL preserveSelectedPage = [VivaldiNTPTopToolbarItemHelper
      representsSamePageForFirst:selectedItem
                          second:previousSelectedItem];

  // The mediator owns selection logic. The controller only applies the
  // final state and preserves the current viewport when the same logical
  // page stays selected after a rebuild.
  [self.topToolbarView updateToolbarWithItems:items
                                selectedIndex:appliedSelectedIndex];

  [self applySelectedToolbarIndex:appliedSelectedIndex];
  // The toolbar items were rebuilt, so keep the selection baseline aligned
  // with the remapped item instead of the old absolute index.
  self.lastSelectedToolbarItemIndex = self.selectedToolbarItemIndex;
  [self refreshToolbarChrome];
  [self reloadPagesForToolbarItemsPreservingSelectedPage:preserveSelectedPage
...
- (void)selectToolbarItemWithIndex:(NSInteger)index animated:(BOOL)animated {
  [self applySelectedToolbarIndex:index];
...
- (NSUInteger)indexOfToolbarItemMatchingNode:
  ...
  NSString* uuidString;
  if (bookmarkNode->uuid().is_valid()) {
    uuidString =
        base::SysUTF8ToNSString(bookmarkNode->uuid().AsLowercaseString());
  }

  for (NSUInteger i = 0; i < self.toolbarItems.count; i++) {
    VivaldiNTPTopToolbarItem* item = self.toolbarItems[i];
    if ((uuidString.length > 0 && [item.uuid isEqualToString:uuidString]) ||
        (primaryId && [item.primaryId isEqualToNumber:primaryId])) {
      return i;
    }
  }
...
- (void)refreshChildItems:(NSArray<VivaldiSpeedDialItem*>*)items
  ...
  if (parent.pageType == VivaldiSpeedDialPageTypeTopSites) {
    ...
    for (NSUInteger i = 0; i < self.toolbarItems.count; i++) {
      ...
      if ([item.uuid isEqualToString:parent.uuid] ||
          (parent.primaryId &&
           [item.primaryId isEqualToNumber:parent.primaryId])) {
        indexToUpdate = i;
        item.children = items;
        break;
      }
  ...
  // Refresh updates for nested folders (not top level Group)
  if (!self.delegate || !self.bookmarks || parent.uuid.length == 0) {
    return;
  }

  std::string uuidString =
      base::SysNSStringToUTF8([parent.uuid lowercaseString]);
  base::Uuid uuid = base::Uuid::ParseLowercase(uuidString);
  const bookmarks::BookmarkNode* changedFolder = self.bookmarks->GetNodeByUuid(
      uuid,
      bookmarks::BookmarkModel::NodeTypeForUuidLookup::kLocalOrSyncableNodes);

  const bookmarks::BookmarkNode* groupAncestor = changedFolder;
  while (groupAncestor && !GetSpeeddial(groupAncestor)) {
    groupAncestor = groupAncestor->parent();
  }

  if (groupAncestor) {
    NSUInteger ancestorIndex =
        [self indexOfToolbarItemMatchingNode:groupAncestor];
    if (ancestorIndex == NSNotFound) {
      return;
    }

    VivaldiSpeedDialItem* ancestorItem =
        [[VivaldiSpeedDialItem alloc] initWithBookmark:groupAncestor];
    [self.delegate computeSpeedDialChildItems:ancestorItem];
  }
}
...
- (void)didSelectItemWithIndex:(NSInteger)index
  ...
  if (index >= 0 && index < long(self.toolbarItems.count)) {
    VivaldiNTPTopToolbarItem* selectedItem =
        [self.toolbarItems objectAtIndex:index];
    if (selectedItem) {
      // Report the real user-driven selection back to the mediator so it can
      // persist "last visited" using stable group identity.
      [self.delegate didSelectToolbarItem:selectedItem atIndex:index];
    }
  }
}

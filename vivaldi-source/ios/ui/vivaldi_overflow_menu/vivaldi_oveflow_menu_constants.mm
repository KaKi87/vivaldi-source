// Copyright 2023 Vivaldi Technologies. All rights reserved.

#import "ios/ui/vivaldi_overflow_menu/vivaldi_oveflow_menu_constants.h"

#pragma mark - ICONS

const char kVivaldiAppStoreUrl[] =
    "https://apps.apple.com/app/vivaldi-powerful-web-browser/id1633234600";

NSString* vOverflowShare = @"vivaldi_overflow_menu_share";
NSString* vOverflowRecentTabs = @"vivaldi_overflow_menu_recent_tabs";
NSString* vOverflowPasswords = @"vivaldi_overflow_menu_passwords";
NSString* vOverflowSettings = @"vivaldi_overflow_menu_settings";

NSString* vOverflowReload = @"vivaldi_overflow_reload";
NSString* vOverflowStop = @"vivaldi_overflow_stop";
NSString* vOverflowNewTab = @"vivaldi_overflow_new_tab";
NSString* vOverflowNewPrivateTab = @"vivaldi_overflow_new_private_tab";
NSString* vOverflowNewWindow = @"vivaldi_overflow_new_window"; // iPad only.

NSString* vOverflowAddTo = @"vivaldi_overflow_add_to";
NSString* vOverflowEdit = @"vivaldi_overflow_edit";
NSString* vOverflowAddStartPage = @"vivaldi_overflow_add_startpage";
NSString* vOverflowEditStartPage = @"vivaldi_overflow_edit_startpage";
NSString* vOverflowAddBookmark = @"vivaldi_overflow_add_bookmark";
NSString* vOverflowEditBookmark = @"vivaldi_overflow_edit_bookmark";
NSString* vOverflowAddReadingList = @"vivaldi_overflow_add_reading_list";
NSString* vOverflowEditReadingList = @"vivaldi_overflow_edit_reading_list";
NSString* vOverflowClearHistory = @"vivaldi_overflow_clear_history";
NSString* vOverflowTranslate = @"vivaldi_overflow_translate";
NSString* vOverflowDesktopSite = @"vivaldi_overflow_desktop_site";
NSString* vOverflowMobileSite = @"vivaldi_overflow_mobile_site";
NSString* vOverflowFindInPage = @"vivaldi_overflow_find_in_page";
NSString* vOverflowZoom = @"vivaldi_overflow_zoom";
NSString* vOverflowSiteInfo = @"vivaldi_overflow_menu_site_info";

NSString* vOverflowBookmarks = @"vivaldi_overflow_bookmarks";
NSString* vOverflowNotes = @"vivaldi_overflow_notes";
NSString* vOverflowHistory = @"vivaldi_overflow_history";
NSString* vOverflowReadingList = @"vivaldi_overflow_reading_list";
NSString* vOverflowDownloads = @"vivaldi_overflow_downloads";

NSString* vOverflowHelp = @"vivaldi_overflow_help";
NSString* vOverflowShareFeedback = @"vivaldi_overflow_share_feedback";
NSString* vOverflowAbout = @"vivaldi_overflow_about";

NSString* vOverflowReaderMode = @"vivaldi_overflow_reader_mode";
...
+ (BOOL)shouldUseDarkTextForImage:(UIImage* _Nonnull)image {
  ...
  CIFilter* filter =
      [CIFilter filterWithName:@"CIAreaAverage"
                 keysAndValues:kCIInputImageKey, inputImage, kCIInputExtentKey,
                               extentVector, nil];
  if (!filter)
    return NO;
...
+ (BOOL)isValidDomain:(NSString* _Nonnull)urlString {
  ...
  NSRegularExpression* domainRegex = [NSRegularExpression
  ...
  NSRegularExpression* ipOrLocalhostRegex = [NSRegularExpression
...
+ (BOOL)areHostsEquivalentForURL:(NSURL* _Nonnull)aURL
  ...
  // If either host string does not have the minimum number of domain
  // components, perform a simple string comparison and return the result.
  ...
  NSString* aDomain =
      [NSString stringWithFormat:@"%@.%@",
                                 aHostComponents[aHostComponents.count -
                                                 kMinDomainComponents],
                                 aHostComponents.lastObject];
  NSString* bDomain =
      [NSString stringWithFormat:@"%@.%@",
                                 bHostComponents[bHostComponents.count -
                                                 kMinDomainComponents],

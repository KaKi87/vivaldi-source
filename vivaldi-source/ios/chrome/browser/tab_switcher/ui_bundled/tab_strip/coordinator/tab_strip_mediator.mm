...
// Vivaldi patch parts are marked with // Vivaldi block.
...
#import "ios/chrome/browser/tab_switcher/ui_bundled/tab_strip/coordinator/vivaldi_tab_strip_mediator_utils.h"
...
TabStripItemData* CreateTabItemData(
  ...
  web::WebState* web_state = web_state_list->GetWebStateAt(index);  // Vivaldi
  TabStripItemData* data = [[TabStripItemData alloc] init];
  if (group) {
    const TabGroupRange range = group->range();
    data.isTabInGroup = YES;  // Vivaldi
  ...
  switch (change.type()) {
    ...
    case WebStateListChange::Type::kInsert: {
      ...
      TabStripItemIdentifier* itemIdentifier =
          CreateTabItemIdentifier(insertedWebState, self.webStateList);
      // End Vivaldi
...
- (void)dragWillBeginForTabGroupItem:(TabGroupItem*)item {
  ...
  for (int childWebStateIndex : item.tabGroup->range()) {
    ...
    TabStripItemIdentifier* childItemIdentifier = CreateTabItemIdentifier(
        _webStateList->GetWebStateAt(childWebStateIndex), self.webStateList);
    // End Vivaldi
...
- (void)populateConsumerItems {
  ...
  for (int index = 0; index < _webStateList->count(); ++index) {
    if (const TabGroup* parentGroup =
      ...
      // Vivaldi
      TabStripItemIdentifier* itemIdentifier = CreateTabItemIdentifier(
          _webStateList->GetWebStateAt(index), self.webStateList);
      // End Vivaldi
...
- (void)reconfigureItemForWebState:(web::WebState*)webState {
  ...
  // Vivaldi
  [self.consumer reconfigureItems:@[ CreateTabItemIdentifier(
                                      webState, self.webStateList) ]];
  // End Vivaldi
}
...
- (TabStripItemIdentifier*)destinationItemAtIndex:(int)index
  ...
  if (parentGroup) {
    if (parentGroup->range().contains(index)) {
      ...
      // Vivaldi
      return CreateTabItemIdentifier(destinationWebState, self.webStateList);
      // End Vivaldi
    }
...
- (void)moveItemForWebState:(web::WebState*)webState
  ...
  // Vivaldi
  TabStripItemIdentifier* itemIdentifier =
      CreateTabItemIdentifier(webState, self.webStateList);
  // End Vivaldi

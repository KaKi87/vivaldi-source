...
VivaldiOverscrollController::VivaldiOverscrollController() {}

VivaldiOverscrollController::~VivaldiOverscrollController() {}

bool VivaldiOverscrollController::ShouldProcessEvent(
...
bool VivaldiOverscrollController::ShouldIgnoreInertialEvent(
...
void VivaldiOverscrollController::OnDidOverscroll(
...
void VivaldiOverscrollController::Reset() {
  overscroll_mode_ = OVERSCROLL_NONE;
...
void VivaldiOverscrollController::Cancel() {
  SetOverscrollMode(OVERSCROLL_NONE, OverscrollSource::NONE);
  overscroll_delta_x_ = overscroll_delta_y_ = 0.f;
  ResetScrollState();
}

bool VivaldiOverscrollController::DispatchEventCompletesAction(
    const blink::WebInputEvent& event) const {
  if (overscroll_mode_ == OVERSCROLL_NONE)
    return false;
...
bool VivaldiOverscrollController::DispatchEventResetsState(
    const blink::WebInputEvent& event) const {
  switch (event.GetType()) {
...
bool VivaldiOverscrollController::ProcessEventForOverscroll(
    const blink::WebInputEvent& event) {
...
void VivaldiOverscrollController::CompleteAction() {
  ignore_following_inertial_events_ = true;
  if (delegate_)
...
void VivaldiOverscrollController::ResetScrollState() {

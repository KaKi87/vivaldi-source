...
#include "installer/mini_installer/setup/user_hive_visitor.h"
#include "installer/mini_installer/util/auto_launch_util.h"
...
InstallStatus UninstallProduct(const ModifyParams& modify_params,
  ...
  vivaldi::UnregisterUpdateNotifier(installer_state);
  ...
  if (!force_uninstall && product_state) {
    ...
    DoPostUninstallOperations(product_state->version());
  }

...
#include "installer/mini_installer/util/beacons.h"
...
#include "installer/mini_installer/util/registry_entry.h"
#include "installer/mini_installer/util/registry_util.h"
#include "installer/mini_installer/util/taskbar_util.h"
...
void GetShellIntegrationEntries(
  ...
  const char* direct_launch_scheme = install_static::GetDirectLaunchUrlScheme();
...
bool ElevateAndRegisterChrome(
  ...
  if (base::CommandLine::ForCurrentProcess()->HasSwitch(
    ...
    if (version_info.get()) {
      exe_path = chrome_exe.DirName().Append(
          base::UTF16ToWide(version_info->product_version()));
...
void ShellUtil::AddDefaultShortcutProperties(const base::FilePath& target_exe,
  ...
  if (!properties->has_arguments()) {
    const std::wstring& arguments = vivaldi::install_static::GetArguments();
    if (!arguments.empty())
...
bool ShellUtil::CreateOrUpdateShortcut(ShortcutLocation location,
  ...
  // pinned. VB-97113
  std::optional<bool> override_pinning =
      IsShortcutPinnedToTaskbar(shortcut_path);
...
bool ShellUtil::ShowMakeChromeDefaultSystemUI(
  ...
  bool is_win11_or_greater =
      base::win::GetVersion() >=
      base::win::Version::WIN10_21H2;  // Vivadi include older windows
  if (!is_default) {
